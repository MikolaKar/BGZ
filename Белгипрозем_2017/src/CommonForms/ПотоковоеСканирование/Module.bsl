...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонецЕсли;	
	Элементы.ФорматХраненияМногостраничныйДляЗагрузкиКаталога.Доступность = ИспользоватьImageMagickДляПреобразованияВPDF;
...
Функция СортироватьМассивФайлов(МассивОписаний)
	...
	ТаблицаЗначенийФайлов.Сортировать("ДатаМодификации Возр");  
...
Процедура ВыполнитьЗагрузкуСоСканераПродолжение(ВыбранноеУстройство, Параметры) Экспорт
	...
	КомпонентаTwain.НачатьСканирование(ПоказыватьДиалогСканера, ВыбранноеУстройство, ФорматКартинки, 
	...
	Для Каждого ШтрихКодТекущий Из Штрихкоды Цикл
		...
		Для Каждого НайденныйОбъект Из МассивНайденных Цикл
			...
			Если ТипЗнч(СсылкаВладельца) = Тип("СправочникСсылка.ВнутренниеДокументы")
				...
				ИЛИ ТипЗнч(СсылкаВладельца) = Тип("СправочникСсылка.ИсходящиеДокументы")
				ИЛИ ТипЗнч(СсылкаВладельца) = Тип("СправочникСсылка.мЭтапыДоговоров") Тогда
...
Функция РаспознатьШтрихКодОдногоФайла(ИмяФайлаКартинки, ПрерватьПроцессСканирования)
	...
	Если НРег(Файл.Расширение) = НРег(".pdf") Тогда
		...
		СтрокаПутей = КомпонентаTwain.РазделитьМногоСтраничныйФайл(ИмяФайлаКартинки, ПутьКПрограммеКонвертации);
		//1С-Минск +
		Если СтрокаПутей = "" Тогда
			// в файле одностраничное изображение
...
Функция РаспознатьШтрихКодОднойКартинки(ИмяФайлаКартинки, ПрерватьПроцессСканирования)
	...
	Если ПоложенияШтрихКода <> ПредопределенноеЗначение("Перечисление.ПоложенияШтрихКода.НеЗадано") Тогда
		...
		КомпонентаTwain.ПолучитьФрагментИзображения(ИмяФайлаКартинки,
...
Процедура РазмещениеВБазе(ВладельцыСНомерами)
	...
	Если РаспределениеИзображенийПоФайлам = ПредопределенноеЗначение("Перечисление.ВариантыРаспределенияИзображенийПоФайлам.ВсеИзображенияВОдинФайл") 
		ИЛИ РаспределениеИзображенийПоФайлам = ПредопределенноеЗначение("Перечисление.ВариантыРаспределенияИзображенийПоФайлам.РазделятьПустойСтраницей") Тогда
		Если МассивПутейКартинок.Количество() <> 0 Тогда
	...
	Для Каждого ПараКлючЗначение Из ВсеВладельцы Цикл
		...
		Для Каждого ОписаниеФайла Из ДанныеВладельца.МассивОписанийФайлов Цикл
			...
			Если МассивПутейКартинок.Количество() = 1 Тогда
				...
				Если ТипЗагрузки = ПредопределенноеЗначение("Перечисление.ТипЗагрузкиПотоковогоСканирования.СоСканера") Тогда
					Если РасширениеРезультата = "pdf" Тогда
						...
						КомпонентаTwain.ОбъединитьВМногостраничныйФайл(СтрокаВсехПутей, ФайлРезультатаВременный, ПутьКПрограммеКонвертации);
			...
			//1С-Минск +
			ПереопределитьИмяФайла(ТекущийВладелец, ИмяФайла);
			
			Если ТипЗнч(ТекущийВладелец) = Тип("СправочникСсылка.мЭтапыДоговоров") Тогда
				_ТекущийВладелец = ПолучитьДоговорЭтапа(ТекущийВладелец);
			Иначе	
				_ТекущийВладелец = ТекущийВладелец;
			КонецЕсли; 
			//1С-Минск -
			
			НеОткрыватьКарточкуПослеСозданияИзФайла = Истина;
			СозданныйФайл = РаботаСФайламиКлиент.СоздатьДокументНаОсновеФайла(Путь, _ТекущийВладелец, ЭтаФорма, 
				НеОткрыватьКарточкуПослеСозданияИзФайла, ИмяФайла);
				
			Если ВестиУчетСканКопийОригиналовДокументов Тогда 
				Делопроизводство.СохранитьСведенияОбОригиналеФайла(СозданныйФайл, _ТекущийВладелец);	
			КонецЕсли;	
	...
	Для Каждого ПараКлючЗначение Из ВсеВладельцы Цикл
		...
		Если ТипЗнч(ТекущийВладелец) = Тип("СправочникСсылка.мЭтапыДоговоров") Тогда
			ТекущийВладелец = ПолучитьДоговорЭтапа(ТекущийВладелец);
		КонецЕсли; 
	...
	Элементы.ДекорацияТекстНеудачно.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Для некоторые изображений (%1) не удалось определить к каким документам они относятся. 
	...
	ЭтаФорма.ПоложениеКоманднойПанели = ПоложениеКоманднойПанелиФормы.Низ;
...
Процедура ОткрытьСписокНераспознанныхИзображений(Команда)
	...
	ПараметрыФормы = Новый Структура("МассивПутейНераспознанныхФайлов, ТекстНастроек, ТипЗагрузки, ПутьКПрограммеКонвертации, ФорматХраненияОдностраничный, ФорматХраненияМногостраничный, УдалятьФайлыПослеЗагрузки", 
...
Процедура ПереопределитьИмяФайла(ТекущийВладелец, ИмяФайла)
	Если ТипЗнч(ТекущийВладелец) = Тип("СправочникСсылка.мЭтапыДоговоров") Тогда
		ВидДок = ТекущийВладелец.Владелец.ВидДокумента;
	    ЭтоЭтапДоговора = Истина;
		
		// Получим количество уже отсканированных и сохраненных смет по этапу
		ИскНаим = "Смета " + ТекущийВладелец.НомерЭтапа + "-";
		КолвоСмет = ПолучитьКоличествоФайловСметПоЭтапуДоговора(ТекущийВладелец.Владелец, ИскНаим);
		СледНомер = КолвоСмет + 1;
		РегНомер = "" + ТекущийВладелец.НомерЭтапа + "-" + СледНомер;
	Иначе
		ВидДок = ТекущийВладелец.ВидДокумента;
	    ЭтоЭтапДоговора = Ложь;
		РегНомер = ТекущийВладелец.РегистрационныйНомер;
	КонецЕсли; 
	
	Если ВидДок = Справочники.ВидыВнутреннихДокументов.АктВыполненныхРабот Тогда
		ПолучитьИмяФайла(РегНомер, ИмяФайла, "Акт ");
		
		// Изменение состояния и реквизитов акта
		//   Если  акт уже имеет состояние не подписан - то менять его на подписан нельзя
		РеквизитыАкта = мРаботаСДоговорами.ПолучитьРеквизитыАктаВыполненныхРабот(ТекущийВладелец);
		ТекСостояние = РеквизитыАкта.СостояниеАкта;
		Если ТекСостояние <> Перечисления.мСостоянияАктовВыполненныхРабот.НеПодписан Тогда
			Реквизиты = Новый Структура();
			Реквизиты.Вставить("СостояниеАкта", Перечисления.мСостоянияАктовВыполненныхРабот.ПодписанЗаказчиком);
			Реквизиты.Вставить("Возвращен", ТекущаяДата());
			мРаботаСДоговорами.УстановитьРеквизитыАктаВыполненныхРабот(ТекущийВладелец, Реквизиты);
		КонецЕсли; 
		
	ИначеЕсли ВидДок = Справочники.ВидыИсходящихДокументов.ОтправкаАктаВыполненныхРабот Тогда	
		ПолучитьИмяФайла(РегНомер, ИмяФайла, "Уведомление ");
		
        Акты = СвязиДокументов.ПолучитьСвязанныеДокументы(ТекущийВладелец, Справочники.ТипыСвязей.ПредметПереписки);
		
		Для каждого АктВыполненныхРабот Из Акты Цикл
			РеквизитыАкта = мРаботаСДоговорами.ПолучитьРеквизитыАктаВыполненныхРабот(АктВыполненныхРабот); 		
			
			Если РеквизитыАкта.СостояниеАкта = Перечисления.мСостоянияАктовВыполненныхРабот.ПодписанЗаказчиком 
				ИЛИ РеквизитыАкта.СостояниеАкта = Перечисления.мСостоянияАктовВыполненныхРабот.ПодписанЗаказчикомСЗамечаниями
				ИЛИ РеквизитыАкта.СостояниеАкта = Перечисления.мСостоянияАктовВыполненныхРабот.НеПодписан Тогда
				// состояние акта менять не надо
			Иначе 
				// Изменение состояния акта
				Реквизиты = Новый Структура();
				Реквизиты.Вставить("СостояниеАкта", Перечисления.мСостоянияАктовВыполненныхРабот.ВрученЗаказчику);
				мРаботаСДоговорами.УстановитьРеквизитыАктаВыполненныхРабот(АктВыполненныхРабот, Реквизиты);
			КонецЕсли; 
		КонецЦикла;
		
	ИначеЕсли ВидДок.Родитель = Справочники.ВидыВнутреннихДокументов.ДоговораЗемлеустроительныхРабот Тогда
		
		Если ЭтоЭтапДоговора Тогда
			ПрефиксИмяФайла = "Смета ";
		Иначе
			ПрефиксИмяФайла = "Дог ";
			
			// Изменение состояния договора
			мРаботаСДоговорами.УстановитьСостояниеДоговора(ТекущийВладелец, Перечисления.мСостоянияДоговоров.Подписан);
			
			// Изменение мДатаНачалаДействияДокумента в договоре
			//   если уже заполнено, то не менять
			РеквДог = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ТекущийВладелец, "мДатаНачалаДействияДокумента");
			Если РеквДог.Свойство("мДатаНачалаДействияДокумента") Тогда
				Если Не ЗначениеЗаполнено(РеквДог.мДатаНачалаДействияДокумента) Тогда
					мРаботаСДоговорами.УстановитьРеквизитДоговора(ТекущийВладелец, "мДатаНачалаДействияДокумента", ТекущаяДата());
					мРаботаСДоговорами.ОбработатьмДатаНачалаДействияДокумента(ТекущийВладелец, ТекущаяДата()); 
				КонецЕсли; 
			КонецЕсли; 
		КонецЕсли;
		
		ПолучитьИмяФайла(РегНомер, ИмяФайла, ПрефиксИмяФайла);
		
	ИначеЕсли ВидДок = Справочники.ВидыВнутреннихДокументов.ДополнительноеСоглашениеДоговораЗемлеустроительныхРабот Тогда
		
		ПолучитьИмяФайла(РегНомер, ИмяФайла, "Доп согл ");
		
		// Изменение состояния Доп согл
		мРаботаСДоговорами.УстановитьСостояниеДоговора(ТекущийВладелец, Перечисления.мСостоянияДоговоров.Подписан);
		
		// Изменение мДатаНачалаДействияДокумента в допсогл
		//   если уже заполнено, то не менять
		РеквДог = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ТекущийВладелец, "мДатаНачалаДействияДокумента");
		Если РеквДог.Свойство("мДатаНачалаДействияДокумента") Тогда
			Если Не ЗначениеЗаполнено(РеквДог.мДатаНачалаДействияДокумента) Тогда
...
&НаСервереБезКонтекста
Функция ПолучитьИмяФайла(РегНомер, ИмяФайла, Префикс)
	ИмяФайла = СтрЗаменить(РегНомер, "/", "_");
	ИмяФайла = СтрЗаменить(ИмяФайла, "\", "_");
	ИмяФайла = СтрЗаменить(ИмяФайла, ".", "_");
	ИмяФайла = Префикс+СокрЛП(ИмяФайла);
	Возврат ИмяФайла;
КонецФункции

&НаСервереБезКонтекста
Функция ПолучитьДоговорЭтапа(ЭтапДоговора)
	Возврат ЭтапДоговора.Владелец;
	
КонецФункции

&НаСервереБезКонтекста
Функция ПолучитьКоличествоФайловСметПоЭтапуДоговора(Договор, ИскНаим)
	...
	Запрос.УстановитьПараметр("Наименование", ИскНаим+"%");
