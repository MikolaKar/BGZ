...
Процедура Подключаемый_СтандартныйПериод_Значение_НачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
	...
	ПараметрыОбработчика.Вставить("Настройки", Настройки);
	ПараметрыОбработчика.Вставить("ВидПериода", ВидПериода);
	ПараметрыОбработчика.Вставить("Элемент", Элемент);

	ОписаниеОповещения = Новый ОписаниеОповещения(
		"Подключаемый_СтандартныйПериод_Значение_НачалоВыбораИзСпискаПродолжение",
		ЭтотОбъект);
	
	ВыбратьПериодИзВыпадающегоСписка(ОписаниеОповещения, Элемент, ВидПериода, НачалоПериода);
		
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_СтандартныйПериод_Значение_НачалоВыбораИзСпискаПродолжение(РезультатВыбора, Параметры) Экспорт
...
Процедура СброситьНастройки(Команда)
	Если ВариантМодифицирован = Истина Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Необходимо сохранить вариант отчета до сброса пользовательских настроек.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОтправитьПоЭлектроннойПочте(Команда)
	...
	ПараметрыОбработчика = Новый Структура;
	ПараметрыОбработчика.Вставить("НеобходимоСформироватьОтчет", Ложь);
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ОтправитьПоЭлектроннойПочтеПродолжение",
		ЭтотОбъект,
	...
	Если ОтображениеСостояния.Видимость = Истина
		...
		ОписаниеОповещения.ДополнительныеПараметры.НеобходимоСформироватьОтчет = Истина;
...
&НаКлиенте
Процедура ОтправитьПоЭлектроннойПочтеПродолжение(Ответ, Параметры) Экспорт
	
	Если Параметры.НеобходимоСформироватьОтчет Тогда
...
Процедура Подключаемый_ЭлементОтбораКомпоновкиДанных_Таблица_Подбор(Команда)
	...
	ПараметрыОбработчика.Вставить("Настройки", Настройки);
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"Подключаемый_ЭлементОтбораКомпоновкиДанных_Таблица_ПодборПродолжение",
		ЭтотОбъект);
	...
	Иначе
		ПоказатьВыборИзМеню(ОписаниеОповещения, СписокВыбора, Элементы[Команда.Имя].Родитель);
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура Подключаемый_ЭлементОтбораКомпоновкиДанных_Таблица_ПодборПродолжение(ВыбранныйЭлемент, Параметры) Экспорт
...
Процедура ВыбратьПериодВОтдельнойФорме(ЭлементИлиКоманда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ВыбратьПериодВОтдельнойФормеПродолжение",
		ЭтотОбъект,
		ПараметрыОбработчика);
	ОткрытьФорму(
		"ОбщаяФорма.ВыборПроизвольногоПериода", 
		ПараметрыФормы, 
		ЭтаФорма,,,,
		ОписаниеОповещения,
...
&НаКлиенте
Процедура ВыбратьПериодВОтдельнойФормеПродолжение(РезультатВыбора, Параметры) Экспорт
...
&НаКлиенте
Функция ВыбратьПериодИзВыпадающегоСписка(ОписаниеОповещения, Элемент, ВидПериода, НачалоПериода, ЛистаниеВверх = Неопределено)
	...
	ПараметрыОбработчика.Вставить("Элемент", Элемент);
	ПараметрыОбработчика.Вставить("ВидПериода", ВидПериода);
	ПараметрыОбработчика.Вставить("НачалоПериода", НачалоПериода);
	ПараметрыОбработчика.Вставить("СписокВыбора", СписокВыбора);

	ОписаниеОповещенияВыбора = Новый ОписаниеОповещения(
		"ВыбратьПериодИзВыпадающегоСпискаПродолжение",
		ЭтотОбъект,
		ПараметрыОбработчика);
		
	ПоказатьВыборИзСписка(ОписаниеОповещенияВыбора, СписокВыбора, Элемент, НачальноеЗначение);
	
КонецФункции 

&НаКлиенте
Процедура ВыбратьПериодИзВыпадающегоСпискаПродолжение(РезультатВыбора, Параметры) Экспорт
	
	Если РезультатВыбора = Неопределено тогда
		ВыполнитьОбработкуОповещения(Параметры.ОписаниеОповещения, Неопределено);
		Возврат;
	КонецЕсли;
	...
	Если Индекс = 0 Тогда
		ВыбратьПериодИзВыпадающегоСписка(ОписаниеОповещения, Элемент, ВидПериода, РезультатВыбора.Значение, Истина);
		Возврат;
	ИначеЕсли Индекс = СписокВыбора.Количество() - 1 Тогда
		ВыбратьПериодИзВыпадающегоСписка(ОписаниеОповещения, Элемент, ВидПериода, РезультатВыбора.Значение, Ложь);
		Возврат;
	КонецЕсли;
...
Процедура БыстрыеНастройкиСоздатьЭлементыУправления()
	...
	ШаблонГруппыСФлажками.Вставить("Отображение", ОтображениеОбычнойГруппы.СлабоеВыделение);
	...
	Для Каждого ПользовательскаяНастройка Из ПользовательскиеНастройкиКД.Элементы Цикл
		...
		ИначеЕсли ТипОбщейНастройки = Тип("СтруктураНастроекКомпоновкиДанных") Тогда
			...
			ГруппаГруппировки.Отображение         = ОтображениеОбычнойГруппы.СлабоеВыделение;
	...
	Если Элементы.Периоды.ПодчиненныеЭлементы.Количество() > 0 Тогда
		Элементы.ПараметрыИОтборы.Отображение = ОтображениеОбычнойГруппы.СлабоеВыделение;
	Иначе
