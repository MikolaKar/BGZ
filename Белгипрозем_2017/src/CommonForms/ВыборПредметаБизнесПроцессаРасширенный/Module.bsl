...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если (РольПредмета = Перечисления.РолиПредметов.Заполняемый И ЗначениеЗаполнено(БизнесПроцесс) 
	   И ЗначениеЗаполнено(ОбщегоНазначения.ПолучитьЗначениеРеквизита(БизнесПроцесс, "ВедущаяЗадача")))
...
Процедура ПолучитьСвязанныеПредметы(Дерево)
	...
	Пока Выборка.Следующий() Цикл
		Если ПравоДоступа("Чтение", Выборка.СвязанныйДокумент.Метаданные()) Тогда
			...
			СтрокаПредмета.Наименование = ОбщегоНазначенияДокументооборот.ПредметСтрокой(Выборка.СвязанныйДокумент);
...
Процедура ПолучитьНедавниеПредметы(Дерево)
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	(ТИПЗНАЧЕНИЯ(ПоследниеОбращения.Объект) <> ТИП(Справочник.Файлы)
		|			ИЛИ НЕ ТИПЗНАЧЕНИЯ(Файлы.ВладелецФайла) В (&ТипыБизнесПроцессов))";
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрокаПредмета = СтрокаЗаголовка.Строки.Добавить();
		СтрокаПредмета.Наименование = ОбщегоНазначенияДокументооборот.ПредметСтрокой(Выборка.Объект);
...
Процедура ПолучитьДругиеПредметы(Дерево)
	...
	Если ТипыПредмета.НайтиПоЗначению("Справочник.Файлы") <> Неопределено И ВыборФайлаСДиска Тогда
		СтрокаПредмета = СтрокаЗаголовка.Строки.Добавить();
...
Процедура УстановитьПредметЗавершение(УстановитьПредмет, ПараметрыРезультата) Экспорт
	...
	Описание = ОбщегоНазначенияДокументооборот.ПредметСтрокой(
...
Функция НовыйМассивСтруктурПредметов(МассивПредметов)
	...
	Для Каждого НовыйПредмет Из МассивПредметов Цикл
		...
		СтруктураПредмета.Вставить("Описание", ОбщегоНазначенияДокументооборот.ПредметСтрокой(НовыйПредмет, НовоеИмяПредмета));
