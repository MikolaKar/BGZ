...
Процедура ТаблицаФайловПриАктивизацииСтроки(Элемент)
	...
	Если ТекущийФайл <> СтрокаТаблицы.Файл Тогда
		...
		Если ПустаяСтрока(СтрокаТаблицы.АдресКартинки) Тогда
			 КодВозврата = РаботаСФайламиВызовСервера.ПолучитьДанныеФайлаИНавигационнуюСсылкуВерсииВоВременномХранилище(СтрокаТаблицы.Файл, Неопределено, УникальныйИдентификатор);
...
Процедура ОткрытьФайлВыполнить()
	...
	ТекущийФайл = Элементы.ТаблицаФайлов.ТекущиеДанные.Файл;
	ДанныеФайла = РаботаСФайламиВызовСервера.ПолучитьДанныеФайлаДляОткрытия(ТекущийФайл, , УникальныйИдентификатор);
