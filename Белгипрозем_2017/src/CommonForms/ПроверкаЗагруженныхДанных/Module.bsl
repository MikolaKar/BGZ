...
Процедура ОтменитьПроверку(Команда)
	...
	ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаКорреспонденты Тогда
		ВыбранныйЭлемент = Элементы.Корреспонденты;
	КонецЕсли;	
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПравоДоступа("Чтение", Метаданные.Справочники.Корреспонденты) Тогда
		Элементы.СтраницаКорреспонденты.Видимость = Ложь;
	КонецЕсли;

	ВыполнятьПроверкуЭПНаСервере = РаботаССВД.ПолучитьВыполнятьПроверкуЭПНаСервере();
...
Процедура УстановитьОтборыВсехСписков()
	...
	УстановитьОтборПроверен(Корреспонденты);
	...
	УстановитьОтборПоПериодуЗагрузки(Корреспонденты);
	...
	УстановитьОтборПоПериодуПроверки(Корреспонденты);
	...
	УстановитьОтборПоПроверил(Корреспонденты);	
	...
	УстановитьОтборПодписан(Корреспонденты);
...
Процедура КорреспондентыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ПараметрыОткрытия.Вставить("Ключ", Элемент.ТекущиеДанные.Ссылка);
	ПараметрыОткрытия.Вставить("ПроверкаЗагруженныхДанных", Истина);
	ОткрытьФорму("Справочник.Корреспонденты.ФормаОбъекта", ПараметрыОткрытия)
КонецПроцедуры
...
Процедура ОбработкаОповещенияСервер()
	...
	Элементы.Корреспонденты.Обновить();
КонецПроцедуры
...
Процедура ПолучитьКоличествоВсехОбъектовСервер()
	...
	КоличествоКорреспондентов = ПолучитьКоличествоОбъектовСервер("Корреспонденты");
КонецПроцедуры
...
Процедура СтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаКорреспонденты Тогда
		ПриАктивизацииСтроки(Элементы.Корреспонденты);
	КонецЕсли;
...
Процедура УстановитьРеквизиты(Команда)
	...
	ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаИсходящиеДокументы Тогда	
		...
		КонецЦикла;
	ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаКорреспонденты Тогда	
		СписокДокументов.Добавить(Элементы.Корреспонденты.ТекущаяСтрока);
	Иначе
		Возврат;
	КонецЕсли;
...
Процедура ФайлыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ДанныеФайла = РаботаСФайламиВызовСервера.ПолучитьДанныеФайлаДляОткрытия(ВыбраннаяСтрока, Неопределено, ЭтаФорма.УникальныйИдентификатор);
...
&НаСервере
Процедура ПроверитьВсеНаСервере(Документ, ТаблицаПодписей)
	РаботаСЭП.ПроверитьВсеНаСервере(ТаблицаПодписей, УникальныйИдентификатор);
	УстановитьСтатусПроверкиДокумента(Документ, ТаблицаПодписей);
КонецПроцедуры

&НаСервере
Процедура УстановитьСтатусПроверкиДокумента(Документ, ТаблицаПодписей)
	
	ПодписываемыеЭлементы = ТаблицаПодписей.ПолучитьЭлементы();
	ОбщаяПодписьВерна = Неопределено;
	...
	Для Каждого ПодписываемыйЭлемент Из ПодписываемыеЭлементы Цикл
		...
		Для Каждого ОднаПодпись Из Подписи Цикл
			Если Не ОднаПодпись.ПодписьВерна Тогда
				...
				Если ТипЗнч(ОднаПодпись.Объект) = Тип("СправочникСсылка.ВерсииФайлов") Тогда
					РаботаСЭП.УстановитьСтатусПроверки(ОднаПодпись.Объект.Владелец, ПредопределенноеЗначение("Перечисление.СтатусПроверкиЭП.ПодписьНедействительна"));	
				Иначе
					РаботаСЭП.УстановитьСтатусПроверки(ОднаПодпись.Объект, ПредопределенноеЗначение("Перечисление.СтатусПроверкиЭП.ПодписьНедействительна"));
				КонецЕсли;
				Если Не СообщениеОтправителюОтправлено Тогда
					//МиСофт+
					//РаботаССВД.ОбработатьОшибкуПроверкиЭПДокумента(Документ.Ссылка, Перечисления.ВидыСообщенийСВД.УведомлениеОбОшибке);
					//МиСофт-
			...
			Иначе
				...
				Иначе
					ОбщаяПодписьВерна = ОбщаяПодписьВерна И ОднаПодпись.ПодписьВерна;
				КонецЕсли;
				Если ТипЗнч(ОднаПодпись.Объект) = Тип("СправочникСсылка.ВерсииФайлов") Тогда
					РаботаСЭП.УстановитьСтатусПроверки(ОднаПодпись.Объект.Владелец, ПредопределенноеЗначение("Перечисление.СтатусПроверкиЭП.ПодписьДействительна"));	
				Иначе
					РаботаСЭП.УстановитьСтатусПроверки(ОднаПодпись.Объект, ПредопределенноеЗначение("Перечисление.СтатусПроверкиЭП.ПодписьДействительна"));
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	Если ОбщаяПодписьВерна = Ложь Тогда
		РаботаСЭП.УстановитьСтатусПроверки(Документ.Ссылка, ПредопределенноеЗначение("Перечисление.СтатусПроверкиЭП.ПодписьНедействительна"));
	КонецЕсли;
	Если ПодписываемыеЭлементы.Количество() > 0 И ОбщаяПодписьВерна = Истина Тогда
		РаботаСЭП.УстановитьСтатусПроверки(Документ.Ссылка, ПредопределенноеЗначение("Перечисление.СтатусПроверкиЭП.ПодписьДействительна"));
	ИначеЕсли ПодписываемыеЭлементы.Количество() > 0 И ОбщаяПодписьВерна = Неопределено Тогда
		РаботаСЭП.УстановитьСтатусПроверки(Документ.Ссылка, ПредопределенноеЗначение("Перечисление.СтатусПроверкиЭП.ПодписьНеПроверена"));
	ИначеЕсли ПодписываемыеЭлементы.Количество() = 0 Или ОбщаяПодписьВерна = Неопределено Тогда
		РаботаСЭП.УстановитьСтатусПроверки(Документ.Ссылка, ПредопределенноеЗначение("Перечисление.СтатусПроверкиЭП.ПодписиНет"));
	КонецЕсли;
	
	Если СообщениеОтправителюОтправлено Тогда
	...
	Для Каждого Документ Из ВыбранныйЭлемент.ВыделенныеСтроки Цикл
		...
		Попытка
			...
			Если ВыполнятьПроверкуЭПНаСервере Тогда
				...
				УстановитьСтатусПроверкиДокумента(Документ, ТаблицаПодписей);
			Иначе
				ПроверитьВсеНаСервере(Документ, ТаблицаПодписей);
			КонецЕсли;
