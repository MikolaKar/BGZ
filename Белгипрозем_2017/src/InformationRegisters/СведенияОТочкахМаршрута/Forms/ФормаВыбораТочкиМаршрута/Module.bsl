...
Процедура ОбработатьВыбор()
	ЭлементДерева = Элементы.ДеревоБизнесПроцессовИТочек.ДанныеСтроки(Элементы.ДеревоБизнесПроцессовИТочек.ТекущаяСтрока);
	Если ЭлементДерева.Ссылка <> Неопределено Тогда
		СтруктураВозврата = Новый Структура("ТочкаМаршрута, БизнесПроцесс", ЭлементДерева.Ссылка, ЭлементДерева.ПолучитьРодителя().Имя);
