...
Процедура ДеревоОбъектовМетаданныхПередНачаломИзменения(Элемент, Отказ)
	...
	Если Элемент.ТекущиеДанные.КлассОбъекта = "01КлассСправочникиКорень"
	 Или Элемент.ТекущиеДанные.КлассОбъекта = "02КлассДокументыКорень"
	 Или Элемент.ТекущиеДанные.КлассОбъекта = "03КлассБизнесПроцессыКорень" Тогда
		Отказ = Истина;
	КонецЕсли;
...
Процедура ЗаполнитьТипыОбъектовВДеревеЗначений()
	...
	ВсеБизнесПроцессы = БизнесПроцессы.ТипВсеСсылки();
	...
	Для Каждого Тип Из МассивТипов Цикл
		...
		ИначеЕсли ВсеБизнесПроцессы.СодержитТип(Тип) Тогда
			Если УзелБизнесПроцессы = НеОпределено Тогда
				УзелБизнесПроцессы = ДеревоОМ.Строки.Добавить();
				УзелБизнесПроцессы.СинонимНаименованияОбъекта = НСтр("ru = 'БизнесПроцессы'");
				УзелБизнесПроцессы.КлассОбъекта = "03КлассБизнесПроцессыКорень";
				УзелБизнесПроцессы.КодКартинки = 2;
			КонецЕсли;
