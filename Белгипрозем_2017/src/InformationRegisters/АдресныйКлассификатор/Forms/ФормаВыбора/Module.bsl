...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого КлючЗначение Из ПолучитьОграниченияПоУровню(Параметры.Уровень) Цикл
		Если КлючЗначение.Ключ = "КодНаселенногоПунктаВКоде" Тогда                                                                                           
			ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Список, КлючЗначение.Ключ, КлючЗначение.Значение, ?(Параметры.Уровень = 4 И ЗначениеЗаполнено(Параметры.РегионКодКлассификатора), ВидСравненияКомпоновкиДанных.НеРавно, ВидСравненияКомпоновкиДанных.Равно));
		Иначе
	...
	Если Параметры.Уровень = 1 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Список, "КодАдресногоОбъектаВКоде", 5, ВидСравненияКомпоновкиДанных.НеРавно);
	КонецЕсли;
...
Процедура УстановитьЗаголовокФормы(Уровень)
	...
	ИначеЕсли Уровень = 3 Тогда
		Заголовок = НСтр("ru = 'Выберите сельский совет'");
...
Функция СформироватьРезультатВыбораСервер(ТекущиеДанные, СкрыватьНеактуальныеАдреса, Уровень, ИспользоватьСохраненныеНастройки)
	...
	//МиСофт+	
	// Вычисляемые
	Если КонтактнаяИнформацияКлиентСервер.СокращениеВпереди(Результат.Сокращение) Тогда
