...
Процедура ИзменениеРуководителяПодразделения(Источник) Экспорт
	...
	Запрос = Новый Запрос(
		...
		|	СоставСубъектовПравДоступа.ОсновнойОбъектАдресации,
		|	СоставСубъектовПравДоступа.ДополнительныйОбъектАдресации,
...
//
// Параметры:
//	НаборИсточник - РегистрСведенийНаборЗаписей.СведенияОПользователях
//
...
Процедура ЗаписьИсполнителейРолей(НаборИсточник) Экспорт
	...
	ОтборИсточника = НаборИсточник.Отбор.ОсновнойОбъектАдресации;
	Если ОтборИсточника.Использование Тогда
		СоставСубъектов.Отбор.ОсновнойОбъектАдресации.Установить(ОтборИсточника.Значение);
	КонецЕсли;
	
	ОтборИсточника = НаборИсточник.Отбор.ДополнительныйОбъектАдресации;
	Если ОтборИсточника.Использование Тогда
		СоставСубъектов.Отбор.ДополнительныйОбъектАдресации.Установить(ОтборИсточника.Значение);
	КонецЕсли;
	...
	Для Каждого СтрИсточника Из НаборИсточник Цикл
		Запись = СоставСубъектов.Добавить();
		Запись.Субъект = Роль;
		Запись.ОсновнойОбъектАдресации = СтрИсточника.ОсновнойОбъектАдресации;
		Запись.ДополнительныйОбъектАдресации = СтрИсточника.ДополнительныйОбъектАдресации;
...
Процедура ЗаписьОбластиДелегирования(Знач ОбластьДелегирования) Экспорт
	...
	Запрос = Новый Запрос(
		...
		|	ЗаписиПоОбластиБезРасшифровки.ОсновнойОбъектАдресации,
		|	ЗаписиПоОбластиБезРасшифровки.ДополнительныйОбъектАдресации,
		|	ЗаписиПоОбластиБезРасшифровки.Пользователь,
		|	ЗаписиПоОбластиБезРасшифровки.ОбъектОснование,
	...
	Если РезультатПустой И Не ОбластьДелегирования.ПометкаУдаления Тогда
...
Процедура РасширитьТаблицуНабораСоставомСубъектовНижнихУровней(ТаблицаНабора, ОбрабатыватьРуководителей = Истина)
	...
	Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	Таб.Субъект,
		|	Таб.ОсновнойОбъектАдресации,
		|	Таб.ДополнительныйОбъектАдресации,
		|	Таб.Пользователь,
		|	Таб.ОбъектОснование,
		...
		|	ТаблицаНабора.ОсновнойОбъектАдресации,
		|	ТаблицаНабора.ДополнительныйОбъектАдресации,
		|	ТаблицаНабора.Пользователь,
		|	ТаблицаНабора.ОбъектОснование,
...
Процедура РасширитьСоставСубъектовВерхнихУровней(ТаблицаНабора, ОбрабатыватьРуководителей = Истина)
	...
	Запрос = Новый Запрос(
		...
		|	ТаблицаНабораСРуководителями.ОсновнойОбъектАдресации,
		|	ТаблицаНабораСРуководителями.ДополнительныйОбъектАдресации,
		|	СоставСубъектовПравДоступа.Пользователь,
		|	СоставСубъектовПравДоступа.ОбъектОснование,
		|	СоставСубъектовПравДоступа.Субъект КАК ПользовательОснование,
		|	СоставСубъектовПравДоступа.ОбластьДелегирования,
		...
		|	ТаблицаСРуководителями.ОсновнойОбъектАдресации,
		|	ТаблицаСРуководителями.ДополнительныйОбъектАдресации,
		...
		|	ТаблицаСДелегатами.ОсновнойОбъектАдресации,
		|	ТаблицаСДелегатами.ДополнительныйОбъектАдресации,
		...
		|			И ТаблицаСДелегатами.ОсновнойОбъектАдресации = ТаблицаНабора.ОсновнойОбъектАдресации
		|			И ТаблицаСДелегатами.ДополнительныйОбъектАдресации = ТаблицаНабора.ДополнительныйОбъектАдресации
...
Процедура УдалитьНеактуальныеЗаписиВерхнихУровней(НомерИтерации = 1) Экспорт
	...
	Запрос = Новый Запрос(
		...
		|	СоставСубъектов_ВерхнийУровень.ОсновнойОбъектАдресации,
		|	СоставСубъектов_ВерхнийУровень.ДополнительныйОбъектАдресации,
		...
		|			И СоставСубъектов_ВерхнийУровень.ОсновнойОбъектАдресации = СоставСубъектов_НижнийУровень.ОсновнойОбъектАдресации
		|			И СоставСубъектов_ВерхнийУровень.ДополнительныйОбъектАдресации = СоставСубъектов_НижнийУровень.ДополнительныйОбъектАдресации
