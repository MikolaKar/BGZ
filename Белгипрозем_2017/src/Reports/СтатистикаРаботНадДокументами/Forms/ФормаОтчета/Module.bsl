...
Функция ПолучитьНовыйОтбор(Расшифровка)
	...
	Для Каждого ЭлементОтбора Из НастройкиКомпоновки.Отбор.Элементы Цикл
		Если ЗаполняемыеПоляОтбора.Найти(ЭлементОтбора.ЛевоеЗначение) <> Неопределено Тогда
			...
			УдалитьЗаполяемоеПолеОтбора(ЗаполняемыеПоляОтбора, Строка(ЭлементОтбора.ЛевоеЗначение));
		КонецЕсли;
	КонецЦикла;
...
Функция РасшифровкаПоПолю(Расшифровка, Поле)
	...
	ЭлементРасширофвки = ДанныеРасшифровкиОбъект.Элементы[Расшифровка];
	ПоляРасшифровки = ЭлементРасширофвки.ПолучитьПоля();
...
&НаСервере
Процедура УдалитьЗаполяемоеПолеОтбора(ЗаполняемыеПоляОтбора, ПолеОтбора)
