...
Процедура СформироватьОтчетПоВерсииОбъекта(ТЧОтчета, ИдентификаторВерсии)
	...
	ИначеЕсли БизнесПроцессы.ТипВсеСсылки().СодержитТип(ТипЗнч(СсылкаНаОбъект)) Тогда
		Если Метаданные.БизнесПроцессы[НаименованиеОбъекта].Макеты.Найти("МакетОбъекта") <> Неопределено Тогда
			Макет = БизнесПроцессы[НаименованиеОбъекта].ПолучитьМакет("МакетОбъекта");
		Иначе
			Макет = Неопределено;
		КонецЕсли;	
...
Функция ПолучитьПредставлениеРеквизитаНаЯзыке(знач ИмяРеквизита)
	...
	ИначеЕсли ИмяРеквизита = "HeadTask" Тогда
		Возврат НСтр("ru = 'ВедущаяЗадача'; en='HeadTask'");
	ИначеЕсли ИмяРеквизита = "Completed" Тогда
		Возврат НСтр("ru = 'Завершен'; en='Completed'");
	ИначеЕсли ИмяРеквизита = "Started" Тогда
		Возврат НСтр("ru = 'Стартован'; en='Started'");	
