...
Функция ОбработатьСтраницуПодключениеКСайту(ВыдаватьСообщения = Истина)
	...
	Если НЕ ЗначениеЗаполнено(Объект.КодПользователяСервераОбновлений) Тогда
		Если ВыдаватьСообщения Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Укажите код пользователя сервера обновлений.'"));
		КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(Объект.ПарольСервераОбновлений) Тогда
		Если ВыдаватьСообщения Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Укажите пароль пользователя сервера обновлений.'"));
		КонецЕсли;
...
Функция ПерейтиКВыборуРежимаОбновления(ЭтоПереходДалее = Ложь)
	...
	Иначе
		...
		ПослеПолученияПараметровАдминистрирования(ПараметрыАдминистрирования, Неопределено);
...
Процедура ПослеПолученияПараметровАдминистрирования(Результат, ЭтоПереходДалее) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		Страницы = Элементы.СтраницыПомощника.ПодчиненныеЭлементы;
		ЭтоФайловаяБаза = СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента().ИнформационнаяБазаФайловая;
		НоваяТекущаяСтраница = ?(ЭтоФайловаяБаза И Не РежимИмитацииКлиентСервернойИБ(), Страницы.ВыборРежимаОбновленияФайл.Имя, Страницы.ВыборРежимаОбновленияСервер.Имя);
		...
		ПередОткрытиемСтраницы(НоваяТекущаяСтраница);
		Элементы.СтраницыПомощника.ТекущаяСтраница = Страницы[НоваяТекущаяСтраница];
...
Функция УстановленWMI(Знач ВыдаватьСообщения = Истина)
	...
	Исключение
		...
		Если ВыдаватьСообщения Тогда
			ОткрытьФорму("Обработка.ОбновлениеКонфигурации.Форма.ПредупреждениеWMI", , ЭтотОбъект);
		КонецЕсли;
		Возврат Ложь;
	КонецПопытки;
КонецФункции
