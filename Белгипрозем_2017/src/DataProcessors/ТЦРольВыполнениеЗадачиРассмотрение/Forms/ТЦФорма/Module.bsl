...
Функция ПолучитьЗадачуРассмотреть(Исполнитель)
	...
	Возврат Результат.Выгрузить().ВыгрузитьКолонку(0)[0];
...
Функция ПолучитьМассивПользователей(Сценарий, Роль)
	...
	Если МассивПользователей.Количество() < КоличествоПользователей Тогда
		ВызватьИсключение "Создайте необходимое количество пользователей для роли ""Рассмотрение задачи""";
	КонецЕсли;
...
Функция ТЦИнициализировать() Экспорт
	...
	#Иначе
		...
		Исполнитель = МассивПользователей[ТЦНомерВРМ()];
...
Функция ТЦВыполнить() Экспорт
	...
	#Иначе
		...
		Для а = 0 По КоличествоПовторов - 1 Цикл
			...
			ЗадачаСсылка = ПолучитьЗадачуРассмотреть(Исполнитель);
			Если Не ЗначениеЗаполнено(ЗадачаСсылка) Тогда
				Продолжить;
			КонецЕсли;
			ПараметрыОткрытия.Ключ = ЗадачаСсылка;
			ОткрытаяФорма = ОткрытьФорму("БизнесПроцесс.Рассмотрение.Форма.ФормаЗадачиИсполнителя", ПараметрыОткрытия);
			ОткрытаяФорма.Выполнено("");
			
			ВремяОкончания = ТестированиеКлиентСервер.ЗначениеТаймера(Инструменты);
			
			ТЦЗаписатьПоказатель("Рассмотреть задачу", (ВремяОкончания - ВремяНачала) / 1000);
