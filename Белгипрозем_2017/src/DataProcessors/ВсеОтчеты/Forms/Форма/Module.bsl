...
Процедура ОбновитьДеревоОтчетов()
	...
	Для Каждого Эл из Метаданные.Отчеты Цикл
		...
		Если (ПустаяСтрока(ИмяПодсистемы) ИЛИ ПодсистемаСодержитОтчет(Подсистема, Эл))
			И МожноВключитьОтчетВСписок(Эл)
			И ПравоДоступа("Использование", Эл)
			И Эл.ОсновнаяСхемаКомпоновкиДанных <> Неопределено Тогда
			...
			КонецЕсли; // Эл.ХранилищеВариантов = Неопределено
