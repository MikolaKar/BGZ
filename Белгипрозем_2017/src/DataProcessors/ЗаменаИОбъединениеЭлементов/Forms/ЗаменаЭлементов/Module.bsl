...
Процедура ИнициализироватьЗаменяемыеСсылки(Знач МассивСсылок)
	...
	Запрос = Новый Запрос("
		...
		|ЛЕВОЕ СОЕДИНЕНИЕ ЗаменяемыеСсылки ПО ЗаменяемыеСсылки.Ссылка = ЦелеваяТаблица.Ссылка
		|ГДЕ ЗаменяемыеСсылки.Ссылка IS NULL");
		
	Если ЕстьВладельцы Тогда
		Запрос.Текст = Запрос.Текст + " И ЦелеваяТаблица.Владелец = ЗаменяемыеСсылки.Владелец";
	КонецЕсли;
	Если ЕстьГруппы Тогда
		Запрос.Текст = Запрос.Текст + " И НЕ ЦелеваяТаблица.ЭтоГруппа";
	КонецЕсли;
