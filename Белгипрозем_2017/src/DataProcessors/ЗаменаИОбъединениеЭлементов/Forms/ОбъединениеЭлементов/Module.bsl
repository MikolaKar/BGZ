...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИнициализироватьМастер(Элементы.ШагиМастера, Элементы.Далее, Элементы.Назад, Элементы.Отмена);
...
&НаКлиенте
Процедура ЗавершитьРаботуФормы(ПоказатьОповещение = Ложь)
	
	ОтменитьДлительнуюОперациюМастера();
	
	Если ПоказатьОповещение <> Неопределено Тогда
		ПоказатьОповещениеПользователя( , ПолучитьНавигационнуюСсылку(ОсновнойЭлемент), 
			СообщениеОЗавершении(), БиблиотекаКартинок.Информация32);
...
Процедура СформироватьПодсказкуОбъединения()
	...
	Если ЕстьПравоБезвозвратногоУдаления Тогда
		Если ТекущийВариантУдаления = "Пометка" Тогда
			ТекстПодсказки = НСтр("ru = 'Все элементы списка (%1) будут <a href = ""ПереключениеРежимаУдаления"">помечены на удаление</a> и заменены во всех местах
				|использования на ""%2"" (отмечен стрелкой).'");
		Иначе
			ТекстПодсказки = НСтр("ru = 'Все элементы списка (%1) будут <a href = ""ПереключениеРежимаУдаления"">удалены безвозвратно</a> и заменены во всех местах
				|использования на ""%2"" (отмечен стрелкой).'");
		КонецЕсли;
	Иначе
		ТекстПодсказки = НСтр("ru = 'Все элементы списка (%1) будут помечены на удаление и заменены во всех местах
			|использования на ""%2"" (отмечен стрелкой).'");
	КонецЕсли;
		
	ТекстПодсказки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстПодсказки, МестаИспользования.Количество(), ОсновнойЭлемент);
...
&НаСервере
Процедура ОпределитьМестаИспользования()
	
	ЗапуститьДлительнуюОперациюМастера("ПоискИУдалениеДублей.ОпределитьМестаИспользования", 
		МассивСсылокИзНабора(МестаИспользования));
...
Функция ПроверитьСостояниеИОбработатьЗаполнениеМестИспользования();
	...
	ПроверитьСостояниеДлительнойОперации();
	Если НастройкиПошаговогоМастера.ЗаданиеВыполнено И НастройкиПошаговогоМастера.ИнформацияОбОшибке = Неопределено Тогда
		Результат = ПолучитьИзВременногоХранилища(НастройкиПошаговогоМастера.АдресРезультатаФоновогоЗадания);
		ЗаполнитьМестаИспользования(Результат);
	КонецЕсли;
	
	Возврат Новый Структура("Завершено,ИнформацияОбОшибке", 
		НастройкиПошаговогоМастера.ЗаданиеВыполнено, НастройкиПошаговогоМастера.ИнформацияОбОшибке);
КонецФункции
...
Процедура ЗапуститьЗаменуСсылок()
	...
	ПолноеИмяМетода = "ПоискИУдалениеДублей.ЗаменитьСсылки";
	...
	ЗапуститьДлительнуюОперациюМастера(ПолноеИмяМетода, ПараметрыМетода);
КонецПроцедуры
...
Функция ПроверитьСостояниеИОбработатьЗаменуСсылок()
	...
	Если НастройкиПошаговогоМастера.ЗаданиеВыполнено И НастройкиПошаговогоМастера.ИнформацияОбОшибке = Неопределено Тогда
		Результат = ПолучитьИзВременногоХранилища(НастройкиПошаговогоМастера.АдресРезультатаФоновогоЗадания);
		Результат = Не ЗаполнитьНеудачныеЗамены(Результат);
	КонецЕсли;
	
	Возврат Новый Структура("Завершено,ИнформацияОбОшибке,Результат", 
		НастройкиПошаговогоМастера.ЗаданиеВыполнено, НастройкиПошаговогоМастера.ИнформацияОбОшибке, Результат);
КонецФункции
...
Процедура ИнициализироватьСценарийМастера()
	...
	ДобавитьШагМастера(Элементы.ШагПоискМестИспользования, 
		...
		КнопкаШагаМастера("Отмена", НСтр("ru = 'Прервать'"), НСтр("ru = 'Отказаться от объединения элементов'"), Истина, Ложь, Ложь))));
	...
	ДобавитьШагМастера(Элементы.ШагВыборОсновногоЭлемента, 
		...
		КнопкаШагаМастера("Назад", "", "", Ложь, Ложь, Ложь, 
		КнопкаШагаМастера("Отмена", НСтр("ru = 'Отмена'"),  НСтр("ru = 'Отказаться от объединения элементов'"), Истина, Истина, Ложь))));
	...
	ДобавитьШагМастера(Элементы.ШагОбъединение, 
		...
		КнопкаШагаМастера("Далее", "", "", Ложь, Ложь, Ложь,
		КнопкаШагаМастера("Назад", "", "", Ложь, Ложь, Ложь,
		КнопкаШагаМастера("Отмена", НСтр("ru = 'Прервать'"), НСтр("ru = 'Прервать объединение элементов'"), Истина,	Истина, Ложь))));
	...
	ДобавитьШагМастера(Элементы.ШагПовторОбъединения,
		...
		КнопкаШагаМастера("Отмена", НСтр("ru = 'Закрыть'"), НСтр("ru = 'Закрыть результаты объединения'"), Истина, Истина, Ложь))));
...
Процедура ШагОбъединениеПередДействиемОтмена(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	// Запрос подтверждения на отмену.
	ТекстВопроса = НСтр("ru = 'Прервать объединение элементов и закрыть?'");
	Описание = Новый ОписаниеОповещения("ЗавершениеОтменыИЗакрытиеФормы", ЭтотОбъект, Новый Структура);
	Описание.ДополнительныеПараметры.Вставить("ПараметрыШага", ПараметрыШага);
...
//
&НаСервере
Процедура ИнициализироватьМастер(Знач ГруппаСтраниц, Знач КнопкаДалее, Знач КнопкаНазад, Знач КнопкаОтмена)
	...
	НастройкиМастера.Вставить("ИнформацияОбОшибке");
	НастройкиМастера.Вставить("ЗаданиеВыполнено", Ложь);
	
	// Для хранения пользовательских параметров.
	НастройкиМастера.Вставить("ПользовательскиеПараметры", Новый Соответствие);
	
	НастройкиПошаговогоМастера = НастройкиМастера;
...
//
&НаСервере
Функция КнопкаШагаМастера(Знач Идентификатор, Знач Заголовок, Знач Подсказка, Знач Видимость, 
	Знач Доступность, Знач КнопкаПоУмолчанию, Знач СлужебныеДанные = Неопределено)
...
Процедура ЗапуститьРаботуМастера()
	НастройкиМастера = НастройкиПошаговогоМастера;
	...
	Иначе
		...
		// Отложенный запуск
		НастройкиПошаговогоМастера.НомерТекущегоШага = НастройкиМастера.СтраницаСтарта;
...
&НаСервере
Функция ЗапуститьДлительнуюОперациюМастера(Знач ИмяМетода, Знач ПараметрыМетода)
	
	// Отменяем возможно незавершенное предыдущее.
	ОтменитьДлительнуюОперациюМастера();
	
	Попытка
		Задание = ДлительныеОперации.ЗапуститьВыполнениеВФоне(УникальныйИдентификатор, ИмяМетода, ПараметрыМетода);
	Исключение
...
&НаСервере
Процедура ПроверитьСостояниеДлительнойОперации()
	...
	Если НастройкиПошаговогоМастера.Завершено Тогда
		НастройкиПошаговогоМастера.ИдентификаторФоновогоЗадания = Неопределено;
	КонецЕсли;
