...
&НаСервере
Процедура ЗагрузитьКомпоновщикНастроек(Настройки = Неопределено, ТипПоиска)
...
Процедура ВыполнитьПоискСервер()
	...
	Если ЗначениеЗаполнено(УсловияОтбора) Тогда
		Если Найти(НРег(Строка(ВидПоиска)), "файл") Тогда
			СтроковоеОписаниеПоисковогоЗапроса = СтроковоеОписаниеПоисковогоЗапроса + ", у которых " + УсловияОтбора;
		Иначе
			Если Элементы.Найти(ИмяЗначенияПеречисления + "СодержимоеФайлов") <> Неопределено
				И ЗначениеЗаполнено(ЭтаФорма[ИмяЗначенияПеречисления + "СодержимоеФайлов"]) Тогда 
		...
		Для Каждого Параметр Из ПараметрыОтбора Цикл
			Если Параметр.Использование Тогда
				...
				Если ТипЗнч(Параметр.Значение) = Тип("Массив") Тогда
					ПравоеЗначение = СтроковыеФункцииКлиентСервер.ПолучитьСтрокуИзМассиваПодстрок(Параметр.Значение, ",");
		...
		Если ТипЗнч(УсловиеОтбора) = Тип("ЭлементОтбораКомпоновкиДанных")
			...
			Если Найти(ВидСравненияЗначений, "заполнено") = 0 Тогда
				...
				Если ТипЗнч(УсловиеОтбора.ПравоеЗначение) = Тип("Дата") Тогда
					ЗначениеСравнения = """" + Формат(УсловиеОтбора.ПравоеЗначение, "ДФ=dd.MM.yyyy") + """";
...
Процедура ПереключитьВидПоиска(ВыбранныйВидПоиска, ПринудительноеПереключение = Ложь)
	...
	Если ВыбранныйВидПоиска <> ВидПоиска
		...
		ОбновитьСоставРекизитовДляПоиска();
	КонецЕсли;
...
&НаСервере
Процедура ОбновитьСоставРекизитовДляПоиска()
...
Процедура ПодписатьЗавершение(Результат, Параметры) Экспорт
	...
	РаботаСЭПКлиент.ИнформироватьОПодписании(МассивДанныхДляЗанесенияВРегистр, 
		Элементы[ИмяСписка + "Список"].ТекущаяСтрока);
...
Процедура ОбщийПоискРеквизитыПередНачаломИзменения(Элемент, Отказ)
	...
	СписокЗначений = ПолучитьСписокПоследнихЗначенийДляРеквизита(СписокПоследнихИспользованныхЗначений, Неопределено, Элемент.ТекущиеДанные.ИмяРеквизита);
	Элемент.ПодчиненныеЭлементы[2].РежимВыбораИзСписка = Ложь;
	Элемент.ПодчиненныеЭлементы[2].КнопкаВыпадающегоСписка = Ложь;
	Элемент.ПодчиненныеЭлементы[2].КнопкаВыбора = Неопределено;
	Элемент.ПодчиненныеЭлементы[2].СписокВыбора.Очистить();
	
	Если СписокЗначений = Неопределено Тогда	
	Иначе
		Элемент.ПодчиненныеЭлементы[2].РежимВыбораИзСписка = Истина;
		Для Каждого ЭлементСписка Из СписокЗначений Цикл
			Элемент.ПодчиненныеЭлементы[2].СписокВыбора.Добавить(ЭлементСписка.Значение);	
		КонецЦикла;
	КонецЕсли;

	Если Элемент.ТекущиеДанные.ТипЗначенияРеквизита = "Булево" Тогда
		Элемент.ПодчиненныеЭлементы[2].РежимВыбораИзСписка = Истина;
		Элемент.ПодчиненныеЭлементы[2].СписокВыбора.Добавить(Истина);
		Элемент.ПодчиненныеЭлементы[2].СписокВыбора.Добавить(Ложь);
	Иначе
		Элемент.ПодчиненныеЭлементы[2].КнопкаВыбора = Неопределено;
	КонецЕсли;
...
Процедура ОткрытьФайлВыполнить()
	...
	ДанныеФайла = РаботаСФайламиВызовСервера.ПолучитьДанныеФайлаДляОткрытия(Элементы.ФайлыСписок.ТекущаяСтрока, Неопределено, УникальныйИдентификатор);
...
Процедура ЗакончитьРедактирование(Команда)
	...
	РаботаСФайламиКлиент.ЗакончитьРедактированиеСОповещением(
		...
		УникальныйИдентификатор,
		Элементы.ФайлыСписок.ТекущиеДанные.ХранитьВерсии,
		Элементы.ФайлыСписок.ТекущиеДанные.РедактируетТекущийПользователь,
		Элементы.ФайлыСписок.ТекущиеДанные.Редактирует,
		Элементы.ФайлыСписок.ТекущиеДанные.Автор);
...
Процедура СохранитьКак(Команда)
	...
	ИначеЕсли Элементы.ФайлыСписок.ВыделенныеСтроки.Количество() = 1 Тогда
		ДанныеФайла = РаботаСФайламиВызовСервера.ПолучитьДанныеФайлаДляСохранения(Элементы.ФайлыСписок.ТекущаяСтрока, Неопределено, УникальныйИдентификатор);
...
Процедура ОбновитьИзФайлаНаДиске(Команда)
	...
	ДанныеФайла = РаботаСФайламиВызовСервера.ПолучитьДанныеФайлаИРабочийКаталог(Элементы.ФайлыСписок.ТекущаяСтрока);
...
Процедура Зашифровать(Команда)
	...
	ОбъектСсылка = Элементы.ФайлыСписок.ТекущаяСтрока;
...
Функция ФайловыеКомандыДоступны()
	...
	Если Элементы.ФайлыСписок.ТекущаяСтрока = Неопределено
		ИЛИ Элементы.ФайлыСписок.ТекущиеДанные = Неопределено Тогда 
		Возврат Ложь;
	КонецЕсли;
