...
&НаСервере
Функция ЗаполнитьДолжников()
	Таблица = РеквизитФормыВЗначение("Должники");
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрНакопления.РасчетыСПокупателями.Остатки(&НаДату, ) КАК РасчетыСПокупателямиОстатки
		|ГДЕ
	...
	ЗначениеВРеквизитФормы(Таблица, "Должники");
КонецФункции
...
Процедура ДолжникиПриАктивизацииСтроки(Элемент)
	...
	Если ТекДанные = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	Корреспондент = ТекДанные.Корреспондент;
	ЗаполнитьДерево();	
КонецПроцедуры
