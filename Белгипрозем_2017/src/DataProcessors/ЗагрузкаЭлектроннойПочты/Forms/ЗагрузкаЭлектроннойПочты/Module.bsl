...
Процедура СоздатьДокумент(Команда)
	Если Команда.Имя = "ВнутреннийДокумент" Тогда
		СоздатьДокументИзСообщения("ВнутренниеДокументы");
	ИначеЕсли Команда.Имя = "ВходящийДокумент" Тогда
		СоздатьДокументИзСообщения("ВходящиеДокументы");
	ИначеЕсли Команда.Имя = "ИсходящийДокумент" Тогда
		СоздатьДокументИзСообщения("ИсходящиеДокументы");
	Иначе
...
Процедура СоздатьДокументИзСообщенияПослеПроверкиПовторногоИмпортаДокумента(
	...
	Если ВидДокумента = "ВнутренниеДокументы" Тогда
		ИмяФормыОбработки = "Обработка.ЗагрузкаЭлектроннойПочты.Форма.ВнутреннийДокумент";
	ИначеЕсли ВидДокумента = "ВходящиеДокументы" Тогда
		ИмяФормыОбработки = "Обработка.ЗагрузкаЭлектроннойПочты.Форма.ВходящийДокумент";
	ИначеЕсли ВидДокумента = "ИсходящиеДокументы" Тогда
		ИмяФормыОбработки = "Обработка.ЗагрузкаЭлектроннойПочты.Форма.ИсходящийДокумент";
	Иначе
		ВызватьИсключение НСтр("ru = 'Вид документа не обрабатывается.'");
	КонецЕсли;
...
Процедура СоздатьДокументИзСообщенияПослеЗаполненияРеквизитовДокумента(
	...
	ДокументИнфо = СоздатьДокументИзСообщенияНаСервере(ВидДокумента, ПараметрыЗаполнения);
	...
	ОткрытьФорму("Справочник." + ВидДокумента + ".ФормаОбъекта", ПараметрыОткрытия);
...
Функция ВидПочтовогоКлиентаПочта()
	Возврат ЗначениеЗаполнено(ПараметрыЗагрузкиПочтовыхСообщений.НастройкиПрофилейДляЗагрузки.Профиль)
		И ПараметрыЗагрузкиПочтовыхСообщений.НастройкиПрофилейДляЗагрузки.Профиль.ВидПочтовогоКлиента = КэшВидПочтовогоКлиентаПочта;
КонецФункции

&НаКлиенте
Функция ВидПочтовогоКлиентаMSOutlook()
	Возврат ЗначениеЗаполнено(ПараметрыЗагрузкиПочтовыхСообщений.НастройкиПрофилейДляЗагрузки.Профиль)
		И ПараметрыЗагрузкиПочтовыхСообщений.НастройкиПрофилейДляЗагрузки.Профиль.ВидПочтовогоКлиента = КэшВидПочтовогоКлиентаMSOutlook;
КонецФункции

&НаКлиенте
Функция ВидПочтовогоКлиентаИнтернетПочта()
	Возврат ЗначениеЗаполнено(ПараметрыЗагрузкиПочтовыхСообщений.НастройкиПрофилейДляЗагрузки.Профиль)
		И ПараметрыЗагрузкиПочтовыхСообщений.НастройкиПрофилейДляЗагрузки.Профиль.ВидПочтовогоКлиента = КэшВидПочтовогоКлиентаИнтернетПочта;
КонецФункции

&НаСервере
Функция СоздатьДокументИзСообщенияНаСервере(ВидДокумента, ПараметрыЗаполнения)
	...
	Попытка
		Документ = Справочники[ВидДокумента].СоздатьЭлемент();
		...
		Если (ВидДокумента = "ИсходящиеДокументы") Тогда
		...
		Если ((ВидДокумента = "ВходящиеДокументы")
			И КэшИспользоватьФайлыУВходящихДокументов)
			Или ((ВидДокумента = "ИсходящиеДокументы")
			И КэшИспользоватьФайлыУИсходящихДокументов)
			Или (ВидДокумента = "ВнутренниеДокументы") Тогда
			...
			Для Каждого ФайлИнфо Из ПараметрыЗаполнения.Файлы Цикл
				...
				Если (ИмяФайла = "1C-53898-2010") И (НРег(РасширениеФайла) = "xml") Тогда
					...
					Если ВидДокумента = "ВходящиеДокументы" Тогда
				...
				Если ФайлИнфо.Свойство("ЭП") Тогда
					...
					Для Каждого ЭПСтрока Из ФайлИнфо.ЭП Цикл
						...
						ЭПИнфо.Вставить("ОбъектСсылка", НовыйФайл);
						ЭПИнфо.Вставить("НоваяПодписьДвоичныеДанные", ЭПДвоичныеДанные);
						ЭПИнфо.Вставить("ДатаПодписи", "");
						ЭПИнфо.Вставить("Комментарий", "");
						ЭПИнфо.Вставить("ИмяФайлаПодписи", ЭПСтрока.Имя);
						Если Не ПустаяСтрока(ЭПСтрока.АдресСертификата) Тогда
							...
							ЭПИнфо.Вставить("ДвоичныеДанныеСертификата", СертификатДвоичныеДанные);
							ЭПИнфо.Вставить("КомуВыданСертификат", ЭПСтрока.КомуВыданСертификат);
						Иначе
							ЭПИнфо.Вставить("Отпечаток", "");
							ЭПИнфо.Вставить("ДвоичныеДанныеСертификата", Неопределено);
							ЭПИнфо.Вставить("КомуВыданСертификат", "");
						КонецЕсли;
						ЭПИнфо.Вставить("АдресФайла", "");
						МассивДанныхОПодписях.Добавить(ЭПИнфо);
					КонецЦикла;
					Если МассивДанныхОПодписях.Количество() > 0 Тогда
						РаботаСЭП.ЗанестиИнформациюОПодписях(МассивДанныхОПодписях, Новый Массив);
					КонецЕсли;
				КонецЕсли;
