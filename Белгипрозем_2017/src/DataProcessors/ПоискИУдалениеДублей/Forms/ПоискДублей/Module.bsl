...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если ПодтверждениеЗакрытияФормы <> Истина Тогда
		...
		Текст = НСтр("ru = 'Закрыть поиск и удаление дублей?'");
...
Процедура ОбновитьМестаИспользованияНеобработанныхДубли(Знач СтрокаДанных)
	...
	Если ДанныеСтроки.Количество = 0 Тогда
		Элементы.ОписаниеТекущейГруппыДублей1.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Элемент ""%1"" успешно обработан'"), 
			ДанныеСтроки.Наименование);
...
Процедура ШагВыполнениеПоискаПередДействиемОтмена(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	...
	ТекстВопроса = НСтр("ru = 'Прервать поиск дублей?'");
...
//                                    последним параметром вызова автоматически добавляется адрес хранилища для
//                                    результата.
...
//     Результат             - Произвольный        - Если длительное задание было запущено с флагом возвращения
//                                                   результата, то содержит текущее значение результата.
//
&НаСервере
Функция СостояниеДлительнойОперацииМастера()
	НастройкиМастера = ЭтотОбъект.НастройкиПошаговогоМастера;
	
	Данные = Новый Структура("Завершено, Результат, ИнформацияОбОшибке, Предупреждение", Истина);
