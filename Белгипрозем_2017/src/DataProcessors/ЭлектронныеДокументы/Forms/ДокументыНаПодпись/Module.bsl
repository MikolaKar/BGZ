...
Процедура ЗаполнитьСводнуюТаблицу(ТаблицаДоступныхСертификатов)
	...
	|	СертификатыЭП.ПроверятьСоставИсполнителей КАК ПроверятьСоставИсполнителей,
	|	СертификатыЭПСоставИсполнителей.Исполнитель
	|ИЗ
	|	Справочник.СертификатыЭП КАК СертификатыЭП
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СертификатыЭП.СоставИсполнителей КАК СертификатыЭПСоставИсполнителей
	|		ПО СертификатыЭПСоставИсполнителей.Ссылка = СертификатыЭП.Ссылка
	|ГДЕ
	|	СертификатыЭП.Ссылка В(&СписокДоступныхСертификатов)";
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЭлектронныеДокументыСлужебныйВызовСервера.ВыполнятьКриптооперацииНаСервере() Тогда
		...
		Если НЕ ЕстьКриптосредстваНаСервере Тогда
			Отказ = Истина;
		КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЭлектронныеДокументыСлужебныйВызовСервера.ПолучитьЗначениеФункциональнойОпции("ИспользоватьЭлектронныеПодписи") Тогда
