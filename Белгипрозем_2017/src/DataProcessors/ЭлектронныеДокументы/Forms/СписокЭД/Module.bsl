...
Функция ПолучитьВыборкуПоРеквизитамДокумента(СсылкаНаОбъект)
	...
	Если ТипЗнч(СсылкаНаОбъект) = Тип("СправочникСсылка.СоглашенияОбИспользованииЭД")
		ИЛИ ТипЗнч(СсылкаНаОбъект) = Тип("СправочникСсылка.ИсходящиеДокументы")
		ИЛИ ТипЗнч(СсылкаНаОбъект) = Тип("СправочникСсылка.ВходящиеДокументы") Тогда
		ТекстЗапроса = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	Ссылка,
		|	Ложь КАК Проведен,
...
Функция ПолучитьСписокЭДПоВладельцу(ОбъектВладелец)
	...
	|ГДЕ
	|	СообщенияЭДО.Документ = &ОбъектВладелец";
...
Процедура ВывестиПодчиненныеДокументы(ТекущийДокумент, ДеревоРодитель)
	...
	|	ПрисоединенныеФайлы.Документ.Дата КАК ДатаВладельца,
	|	ПрисоединенныеФайлы.Документ.Номер КАК НомерВладельца,
...
Процедура ТаблицаОтчетаОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если ТипЗнч(ОбъектРасшифровки) = Тип("СправочникСсылка.СообщенияЭДО") Тогда
