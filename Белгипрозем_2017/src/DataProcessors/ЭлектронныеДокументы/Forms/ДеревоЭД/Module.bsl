...
Функция ЭДВладелец(Знач ТекущийЭД)
	...
	Если ТекущийЭД.ВидЭД = Перечисления.ВидыЭД.ТОРГ12Покупатель
		...
		ВозвращаемоеЗначение = ТекущийЭД.Родитель;
	КонецЕсли;
...
Процедура ВывестиНеАктуальныеЭД(ДеревоОбъект, АктуальныйЭД)
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	СообщенияЭДО.Документ = &ВладелецЭД
		|	И НЕ СообщенияЭДО.ВидЭД В (&ВидыЭДИсключения)
		|	И СообщенияЭДО.Родитель = ЗНАЧЕНИЕ(Справочник.СообщенияЭДО.ПустаяСсылка)
		|	И СообщенияЭДО.Ссылка <> &АктуальныйЭД";
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(КорневойЭлемент) Тогда
			ВывестиПодчиненныеДокументы(Выборка.Документ, КорневойЭлемент, Выборка.Ссылка);
		КонецЕсли;
	КонецЦикла;
				...
				Если ИндексТекСтатуса + 1 < МассивСтатусовЭД.Количество() Тогда
					...
					Если ТекущийСтатусЭД = Перечисления.СтатусыЭД.Сформирован
						...
						Если НЕ (НЕ ЭлектронныеДокументыСлужебный.НемедленнаяОтправкаЭД()
								ИЛИ ЭлектронныеДокументыСлужебныйВызовСервера.ПолучитьЗначениеФункциональнойОпции("ИспользоватьЭлектронныеПодписи")) Тогда
...
Процедура ВывестиПодчиненныеДокументы(ТекущийДокумент, ДеревоРодитель, АктуальныйЭД = Неопределено)
	...
	|	КОНЕЦ КАК Версия
	|ИЗ
	|	Справочник.СообщенияЭДО КАК ПрисоединенныеФайлы
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СообщенияЭДО КАК ПодчиненныеПрисоединенныеФайлы
	|		ПО (ПодчиненныеПрисоединенныеФайлы.Родитель = ПрисоединенныеФайлы.Ссылка)
	|			И (НЕ ПодчиненныеПрисоединенныеФайлы.ВидЭД В (&ВидыЭДИсключения))
	|ГДЕ
	|	(ПрисоединенныеФайлы.Документ = &ОбъектВладелец
	|			ИЛИ ПрисоединенныеФайлы.Родитель = &ОбъектВладелец)
	...
	Если АктуальныйЭД <> Неопределено  Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст,
			"ПрисоединенныеФайлы.Документ = &ОбъектВладелец", "ПрисоединенныеФайлы.Документ = &ОбъектВладелец
...
Функция МассивПакетовКОтправке(СсылкаЭД)
	...
	Запрос.УстановитьПараметр("ОбъектВладелец", СсылкаЭД.Документ);
	...
	Если Элемент.ТекущийЭлемент <> Неопределено Тогда
		...
		Если Элемент.ТекущийЭлемент.Имя = "ДеревоПодчиненныеЭДПредставление" Тогда
			...
			Если Элемент.ТекущиеДанные <> Неопределено
				И ТипЗнч(Элемент.ТекущиеДанные.Ссылка) = Тип("СправочникСсылка.СообщенияЭДО")
...
Процедура ЖурналСобытий(Команда)
	...
	ОткрытьФорму("РегистрСведений.ЖурналСобытийЭД.ФормаСписка", ПараметрыФормы, ЭтаФорма,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
