...
Процедура СписокТиповДокументовНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
	...
	ПараметрыВыполнения = Новый Структура("ТекЭлемент", ТекЭлемент);
	Обработчик = Новый ОписаниеОповещения("СписокТиповДокументовНачалоВыбораИзСпискаЗавершение", ЭтотОбъект);
	ПоказатьВыборИзСписка(Обработчик, Элементы.СписокТиповДокументов.СписокВыбора, Элемент, ТекЭлемент);
	
КонецПроцедуры

&НаКлиенте
Процедура СписокТиповДокументовНачалоВыбораИзСпискаЗавершение(ВыбранныйЭлемент, ПараметрыВыполнения)
	
	Если ВыбранныйЭлемент <> Неопределено И ВыбранныйЭлемент <> ПараметрыВыполнения.ТекЭлемент Тогда
