...
Процедура ОткрытьКаталогОбменаИнформациейУзла()
	...
	Оповещение = Новый ОписаниеОповещения("ОткрытьКаталогОбменаИнформациейУзлаЗавершение", ЭтотОбъект, Новый Структура);
	Оповещение.ДополнительныеПараметры.Вставить("ИмяКаталога", ИмяКаталога);
...
&НаКлиенте
Процедура ОткрытьКаталогОбменаИнформациейУзлаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Не Файл.Существует() Тогда
		ТекстПредупреждения = НСтр("ru = 'Каталог ""%1"" не существует или к нему нет доступа.'");
		
	ИначеЕсли Не Файл.ЭтоКаталог() Тогда
		ТекстПредупреждения = НСтр("ru = '""%1"" является файлом, а не каталогом.'");
		
	КонецЕсли;
	
	Если ПустаяСтрока(ТекстПредупреждения) Тогда
		// Независимое от платформы открытие каталога
...
//
// Параметры:
//  Структура - Структура - Структура элементов, на основании которой необходимо создать новую структуру с ключами без заданного литерала.
