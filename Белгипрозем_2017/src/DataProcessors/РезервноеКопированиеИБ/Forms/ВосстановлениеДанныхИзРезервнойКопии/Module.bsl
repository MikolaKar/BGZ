...
Процедура ИстечениеВремениОжидания()
	...
	ОтключитьОбработчикОжидания("ПроверкаНаЕдинственностьПодключения");
	ТекстПредупреждения = НСтр("ru = 'Не удалось отключить все активные сеансы работы с программой.
		|Резервное копирование будет отменено.'");
	ПоказатьПредупреждение(,ТекстПредупреждения, 30, НСтр("ru = 'Резервное копирование'"));
