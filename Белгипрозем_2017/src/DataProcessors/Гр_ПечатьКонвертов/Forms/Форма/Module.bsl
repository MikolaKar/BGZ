...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	ВидКонверта      = ПредопределенноеЗначение("Перечисление.ФорматыКонвертов.С6");
	МассивДокументов = Параметры.МассивПисем;
	флСразуНаПринтер = Истина;
	...
	Для каждого СтрПолучателя Из ДанныеПолучателей Цикл
		...
		Если ЗначениеЗаполнено(СтрПолучателя.АдресКорр) Тогда//надо поставить флаг печатать,если не было другого по этому же док-ту с адресом
			Если СтрПолучателя.ИсхНомер = ТекНомер Тогда//этот документ уже отметили ранее
				НоваяСтрока.Пометка = Ложь;
			Иначе//оставляем пометку
			КонецЕсли;
			ТекНомер = СтрПолучателя.ИсхНомер;
		Иначе
			НоваяСтрока.Пометка = Ложь;
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьДанныеПолучателей(МассивДокументов)
	...
	Запрос = Новый Запрос;
	//Для каждого типа документов своя выборка данных
	Если ТипЗнч(МассивДокументов[0]) = Тип("СправочникСсылка.ВнутренниеДокументы") Тогда
	
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ВЫБОР
		|		КОГДА КорреспондентыКонтактнаяИнформация.Ссылка.ЮрФизЛицо = ЗНАЧЕНИЕ(Перечисление.ЮрФизЛицо.ЮрЛицо)
		|			ТОГДА ВЫРАЗИТЬ(ВнутренниеДокументыКорреспонденты.Корреспондент.ПолноеНаименование КАК СТРОКА(1000))
		|		ИНАЧЕ ВнутренниеДокументыКорреспонденты.Корреспондент.Наименование
		|	КОНЕЦ                                                         КАК Получатель,
		|	ВнутренниеДокументыКорреспонденты.КонтактноеЛицо.Наименование КАК Адресат,
		...
		|	ВнутренниеДокументыКорреспонденты.Ссылка.РегистрационныйНомер КАК ИсхНомер,
		|	ИСТИНА КАК Пометка
		|ИЗ
		...
		|		ПО (ВнутренниеДокументыКорреспонденты.Корреспондент = КорреспондентыКонтактнаяИнформация.Ссылка И ВЫБОР
		...
		|	КОНЕЦ)
		|ГДЕ
		|	ВнутренниеДокументыКорреспонденты.Ссылка В(&МассивДокументов)";
		
	ИначеЕсли ТипЗнч(МассивДокументов[0]) = Тип("СправочникСсылка.ИсходящиеДокументы") Тогда
		Запрос.Текст = 
		"ВЫБРАТЬ
        |   ВЫБОР
        |       КОГДА КорреспондентыКонтактнаяИнформация.Ссылка.ЮрФизЛицо = ЗНАЧЕНИЕ(Перечисление.ЮрФизЛицо.ЮрЛицо)
        ...
        |       ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Корреспонденты.КонтактнаяИнформация КАК КорреспондентыКонтактнаяИнформация
        |       ПО (ИсходящиеДокументыПолучатели.Получатель = КорреспондентыКонтактнаяИнформация.Ссылка И ВЫБОР
        |           КОГДА КорреспондентыКонтактнаяИнформация.Ссылка.ЮрФизЛицо = ЗНАЧЕНИЕ(Перечисление.ЮрФизЛицо.ФизЛицо)
        |               ТОГДА КорреспондентыКонтактнаяИнформация.Вид = ЗНАЧЕНИЕ(Справочник.ВидыКонтактнойИнформации.ПочтовыйАдресФизическогоЛица)
        |           ИНАЧЕ КорреспондентыКонтактнаяИнформация.Вид = ЗНАЧЕНИЕ(Справочник.ВидыКонтактнойИнформации.ПочтовыйАдресКорреспондента)
        |       КОНЕЦ)
        ...
        |   ";
	Иначе
		Сообщить("Для типа "+ТипЗнч(МассивДокументов[0])+" печать конвертов не предусмотрена",СтатусСообщения.Внимание);
		Возврат "";
	КонецЕсли;
...
Процедура ПечатьКонверта(Команда)
	...
	Если ВидКонверта = ПредопределенноеЗначение("Перечисление.ФорматыКонвертов.С6") Тогда
		ТабДок = СформироватьТабДок("С6", МассивПолучателей);
		ТабДок.РазмерСтраницы   = "A4";
		ТабДок.ОриентацияСтраницы 	= ОриентацияСтраницы.Ландшафт;
		ТабДок.АвтоМасштаб			= Ложь;
		ТабДок.ПолеСлева			= 140;
		ТабДок.ПолеСправа			= 0;
		ТабДок.ПолеСверху			= 50;
		ТабДок.ПолеСнизу			= 0;
	ИначеЕсли ВидКонверта = ПредопределенноеЗначение("Перечисление.ФорматыКонвертов.С5") Тогда
		ТабДок = СформироватьТабДок("С5", МассивПолучателей);
		ТабДок.РазмерСтраницы	= "Envelope C5";
		ТабДок.ОриентацияСтраницы 	= ОриентацияСтраницы.Ландшафт;
...
Функция СформироватьТабДок(ИмяМакета, МассивПолучателей)
	...
	Макет = Обработки.Гр_ПечатьКонвертов.ПолучитьМакет(ИмяМакета);
