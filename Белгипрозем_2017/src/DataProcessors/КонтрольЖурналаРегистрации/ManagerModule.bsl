...
Процедура СформироватьОтчетПоОшибкамИПослатьОтчет(Знач НачалоПериодаВыборки = Неопределено,
	...
	АдресФайла = ПоместитьВоВременноеХранилище(Новый ДвоичныеДанные(ИмяФайлаОтчета));
	Вложение = Новый Структура;
	Вложение.Вставить("Адрес", АдресФайла);
	Вложение.Вставить("ИмяФайла", "report.mxl");
...
// окончание периода в хранилище системных настроек - для использования
...
// Получает время предыдущего процесса сбора информации из журнала регистрации
//
...
// Сохраняет время предыдущего процесса сбора информации из журнала регистрации
//
