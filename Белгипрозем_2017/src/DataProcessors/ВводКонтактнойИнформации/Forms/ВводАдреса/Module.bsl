...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
    ...
    // Скрываем кнопку проверки адреса
    Элементы.ПроверитьЗаполнениеАдреса.Видимость = Ложь;
	ЗначениеВидаКонтактнойИнформации = Параметры.ВидКонтактнойИнформации;
    //}}1C-Минск
	...
	//1С-Минск +
	ВидКонтактнойИнформации.Вставить("ЗначениеВидаКонтактнойИнформации", Параметры.ВидКонтактнойИнформации);
	//1С-Минск -
	...
	Иначе
		...
		Элементы.Переместить(КнопкаПанели, Элементы.ФормаВсеДействия, Элементы.ФормаСтандартныеДействия); 
...
Процедура ЗагрузитьКлассификатор(Команда)
	//1С-Минск +
	ОткрытьФорму("Обработка.ЗагрузкаАдресногоКлассификатора.Форма.Форма");
...
Процедура СформироватьДетальныйНаселенныйПункт(Контекст, XDTOДанныеАдреса=Неопределено)
    ...
    //{{1С-Минск
	Контекст.НаселенныйПунктДетально = КонтактнаяИнформацияСлужебный.СписокРеквизитовНаселенныйПункт(Контекст.КодКлассификатораНаселенногоПункта);
...
Процедура СообщитьОбОшибкахЗаполнения(СписокОшибок, ПредупреждатьОбОтсутствии)
	...
	// Сообщаем список с привызкой к полям
...
Процедура СостояниеВводаПредставленияАдреса(Режим, ФормироватьПредставление=Истина)
	...
	Иначе 
		...
		Элементы.ГруппаПредставления.Отображение = ОтображениеОбычнойГруппы.ОбычноеВыделение;
		Элементы.СтранаАдреса.Отображение        = Элементы.ГруппаПредставления.Отображение;
...
Процедура ПриЗакрытии()
    ...
    Если ЭтоАдресЗемельногоУчастка(ЗначениеВидаКонтактнойИнформации) Тогда
		// для формирования наименования вход. документа (поручения)
		Оповестить("АдресВведен", ПредставлениеАдреса);
	КонецЕсли; 
КонецПроцедуры

// <Описание функции>
&НаСервереБезКонтекста
Функция ЭтоАдресЗемельногоУчастка(ЗначениеВидаКонтактнойИнформации)
    Возврат ЗначениеВидаКонтактнойИнформации = Справочники.ВидыКонтактнойИнформации.АдресЗемельногоУчастка или 
    	ЗначениеВидаКонтактнойИнформации = Справочники.ВидыКонтактнойИнформации.АдресЗемельногоУчасткаОбъектаРабот
КонецФункции // ЭтоАдресЗемельногоУчастка(ВидКонтактнойИнформации)()
