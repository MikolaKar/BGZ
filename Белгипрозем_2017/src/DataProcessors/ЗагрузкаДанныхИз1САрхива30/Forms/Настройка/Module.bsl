...
Процедура ЗагрузкаПользователей()
	...
	СоответствиеИдентификатораПользователяСсылке = Новый ТаблицаЗначений;
	СоответствиеИдентификатораПользователяСсылке.Колонки.Добавить("Идентификатор");
	СоответствиеИдентификатораПользователяСсылке.Колонки.Добавить("Ссылка");
	...
	Пока НЕ Recordset.EOF Цикл
		...
		Если Группа = 0 Тогда 
			...
			НоваяСтрока = СоответствиеИдентификатораПользователяСсылке.Добавить();
	...
	АдресСоответствиеИдентификатораПользователяСсылке = ПоместитьВоВременноеХранилище(
		СоответствиеИдентификатораПользователяСсылке, ЭтаФорма.УникальныйИдентификатор);
...
Процедура ЗагрузкаПодверсии()
	...
	СоответствиеИдентификатораПользователяСсылке = Новый ТаблицаЗначений;
	СоответствиеИдентификатораПользователяСсылке = ПолучитьИзВременногоХранилища(АдресСоответствиеИдентификатораПользователяСсылке);
	
	СоответствиеИдентификатораПапкиСсылке = Новый ТаблицаЗначений;
	СоответствиеИдентификатораПапкиСсылке = ПолучитьИзВременногоХранилища(АдресСоответствиеИдентификатораПапкиСсылке);
	...
	Пока НЕ Recordset.EOF Цикл
		...
		Попытка
			...
			НайденнаяСсылка = СоответствиеИдентификатораПользователяСсылке.Найти(Идентификатор, "Идентификатор"); 
			...
			НоваяВерсия = РаботаСФайламиВызовСервера.СоздатьВерсию(
	...
	СоответствиеИдентификатораВерсииФайлаСcылке = ПоместитьВоВременноеХранилище(СоответствиеИдентификатораПапкиСсылке, ЭтаФорма.УникальныйИдентификатор);
...
Процедура ЗагрузкаИсходящиеДокументы(МассивСвязей)
	...
	Пока НЕ Recordset.EOF Цикл
		...
		НовыйДокумент.Зарегистрировал = СоответствиеИдентификатораПользователяСсылке.Найти(Автор, "Идентификатор");
		...
		НоваяСтрока.Получатель = ПолучитьКорреспондента(ИмяКорреспондента);
	...
	Пока НЕ Recordset.EOF Цикл
		...
		Если ТекущаяВерсия <> Null Тогда
			...
			ИначеЕсли КатегорияФайла = 3 Тогда // Папка файлов	
				...
				Если   Идентификатор <>  Null  Тогда
					НайденнаяСсылка = СоответствиеИдентификатораПапкиСсылке.Найти(Идентификатор, "Идентификатор"); 
					Если НайденнаяСсылка <> Null И НайденнаяСсылка <> Неопределено Тогда
						НовыйДокумент.ВладелецФайла = НайденнаяСсылка.Ссылка;
					КонецЕсли;	
				КонецЕсли;
			...
			НайденнаяСсылкаАвтор = СоответствиеИдентификатораПользователяСсылке.Найти(Идентификатор,"Идентификатор"); 
...
Процедура  ЗагрузкаПапок()
	...
	СоответствиеИдентификатораПапкиСсылке = Новый ТаблицаЗначений;
	СоответствиеИдентификатораПапкиСсылке.Колонки.Добавить("Идентификатор");
	СоответствиеИдентификатораПапкиСсылке.Колонки.Добавить("Ссылка");
	...
	Пока НЕ Recordset.EOF Цикл
		...
		НоваяСтрока = СоответствиеИдентификатораПапкиСсылке.Добавить();
	...
	Пока НЕ Recordset.EOF Цикл
		...
		НайденнаяСсылкаРодитель = СоответствиеИдентификатораПапкиСсылке.Найти(ИдентификаторРодитель, "Идентификатор"); 
	...
	АдресСоответствиеИдентификатораПапкиСсылке = ПоместитьВоВременноеХранилище(СоответствиеИдентификатораПапкиСсылке, ЭтаФорма.УникальныйИдентификатор);
...
Процедура ЗагрузкаРабочиеКаталогиФайлов()
	...
	Пока НЕ Recordset.EOF Цикл
		...
		Идентификатор = Recordset.Fields("hFolder").Value;
		СсылкаПапки = СоответствиеИдентификатораПапкиСсылке.Найти(Идентификатор, "Идентификатор"); 
		Если СсылкаПапки <> Неопределено Тогда
			...
			Если НайденнаяСсылка <> Неопределено Тогда
				...
				НаборЗаписей.Отбор.Пользователь.Установить(НайденнаяСсылка.Ссылка);
...
&НаКлиенте
Процедура ЗагрузкаРабочихКаталоговФайлов(Команда)
