...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИнтервалВремениВыполнения = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("АвтоматическоеРаспознаваниеИзображений", "ИнтервалВремениВыполнения");
	Если ИнтервалВремениВыполнения = 0 Тогда
		ИнтервалВремениВыполнения = 60;
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("АвтоматическоеРаспознаваниеИзображений", "ИнтервалВремениВыполнения",  ИнтервалВремениВыполнения);
	КонецЕсли;	
	
	ПриложениеЗапускаCuneiForm = ПолучитьОбщийМакет("ПриложениеЗапускаCuneiForm");
	АдресПриложенияЗапускаCuneiForm = ПоместитьВоВременноеХранилище(ПриложениеЗапускаCuneiForm, УникальныйИдентификатор);
	...
	Если ИспользоватьImageMagickДляРаспознаванияPDF Тогда
		ИнформационнаяНадпись = НСтр("ru = 'Включена настройка ""Использовать ImageMagick для распознавания отсканированных документов формата PDF"". Путь к программе преобразования PDF: ""'");
	...
	КоличествоФайловВПорции = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("АвтоматическоеРаспознаваниеИзображений", "КоличествоФайловВПорции");
	Если КоличествоФайловВПорции = 0 Тогда
		КоличествоФайловВПорции = 100;
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("АвтоматическоеРаспознаваниеИзображений", "КоличествоФайловВПорции",  КоличествоФайловВПорции);
	КонецЕсли;	
...
Процедура ЗаписьЖурналаРегистрацииСервер(ТекстСообщения)
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Распознавание изображения'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
...
#Если Не ВебКлиент Тогда	
	...
	РаспознанныйТекст = "";
	ИмяФайлаРезультата = "";
	СтрокаВозврата = РаспознатьИзображение(ИмяФайлаКартинки, ЯзыкРаспознавания, РасширениеФайлаРезультата, ОписаниеОшибки, РаспознанныйТекст, ИмяФайлаРезультата);
...
#Если Не ВебКлиент Тогда	
	...
	КодВозврата = КомпонентаЗагрузкиCuneiForm.РаспознатьКартинку(ПутьПриложенияЗапускаCuneiForm, ИмяФайлаКартинки, ИмяФайлаРезультата, 
		Число(ЯзыкРаспознавания), ВремяОжиданияРаспознавания, ПутьКПрограммеКонвертацииPDF);
...
Процедура ОткрытьПерсональныеНастройкиПродолжение(КодВозврата, Параметры) Экспорт
	...
	Если КодВозврата = КодВозвратаДиалога.ОК Тогда
		...
		ПутьКПрограммеКонвертацииPDF = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("НастройкиПрограммы", "ПутьКПрограммеКонвертацииPDF");
		Если ПустаяСтрока(ПутьКПрограммеКонвертацииPDF) Тогда
			ПутьКПрограммеКонвертацииPDF = "convert.exe"; // ImageMagick
		КонецЕсли;	
