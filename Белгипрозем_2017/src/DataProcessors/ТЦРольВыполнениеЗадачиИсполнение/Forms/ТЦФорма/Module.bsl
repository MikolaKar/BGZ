...
Функция ПолучитьЗадачуИсполнить(Исполнитель)
...
Функция ПолучитьМассивПользователей(Сценарий, Роль)
	...
	Если МассивПользователей.Количество() < КоличествоПользователей Тогда
		ВызватьИсключение "Создайте необходимое количество пользователей для роли ""Исполнение задачи""";
	КонецЕсли;
...
Функция ТЦВыполнить() Экспорт
	...
	#Иначе
		...
		Для а = 0 По КоличествоПовторов - 1 Цикл
			...
			ЗадачаСсылка = ПолучитьЗадачуИсполнить(Исполнитель);
			Если Не ЗначениеЗаполнено(ЗадачаСсылка) Тогда
				Продолжить;
			КонецЕсли;
			ПараметрыОткрытия.Ключ = ЗадачаСсылка;
			ОткрытаяФорма = ОткрытьФорму("БизнесПроцесс.Исполнение.Форма.ФормаЗадачиИсполнителя", ПараметрыОткрытия);
			ОткрытаяФорма.Исполнено("");
			
			ВремяОкончания = ТестированиеКлиентСервер.ЗначениеТаймера(Инструменты);
			
			ТЦЗаписатьПоказатель("Исполнить задачу", (ВремяОкончания - ВремяНачала) / 1000);
