...
Процедура НайтиНаКлиенте(Направление)
	...
	Если Направление = 0 Тогда
		ОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени(
			"ПолнотекстовыйПоискВыполнениеКомандыНайти");
	КонецЕсли;
...
Функция ПолучитьHTMLТекст(СписокПоиска)
	...
	Если Объект.ОтображатьДополнительнуюИнформацию Тогда
		Для Счетчик = 0 По КоличествоРезультатов - 1 Цикл
			СтрокаЗамены = "%" + Формат(Счетчик, "ЧН=; ЧГ=0") + "%";
			ПапкиКатегорииСтр = ПолнотекстовыйПоискСерверДокументооборот.ПолучитьДополнительнуюИнформациюПоОбъекту(СписокПоиска[Счетчик].Значение);
...
Функция СтрокаПоискаСодержитСпецСимволы(СтрокаПоиска)
	...
	СпецСимволы = """,&|?~/()*#!";
	Для Счетчик = 1 По СтрДлина(СпецСимволы) Цикл
		Символ = Сред(СпецСимволы, Счетчик, 1);
