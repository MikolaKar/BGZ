...
Процедура HTMLДокументПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Если Найти(НРег(СокрЛП(КлассАктивногоЭлемента)),"openupdate") <> 0 Тогда
			...
			Если НЕ ПустаяСтрока(ИмяОбработки) Тогда
				...
				Иначе
					Если НЕ ПустаяСтрока(СообщениеОбОшибке) Тогда
						ПоказатьПредупреждение(, СообщениеОбОшибке, , НСтр("ru = 'Ошибка'"));
						Возврат;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		...
		Исключение
			...
			ШаблонСообщения = НСтр("ru = 'Ошибка при запуске обработки обновления конфигурации. %1'");
			...
			ПоказатьПредупреждение(, ТекстОшибкиПользователю, , НСтр("ru = 'Ошибка'"));
...
Функция ИмяОсновнойФормыОбработкиОбновленияКонфигурации(Знач ИмяОбработки, СообщениеОбОшибке)
	...
	Если МетаданныеОбработка = Неопределено Тогда
		...
		СообщениеОбОшибке = НСтр("ru = 'Отсутствует обработка обновления конфигурации.'");
		ПодробноеОписаниеОшибки = СтрЗаменить(НСтр("ru = 'В метаданных не найдена обработка обновления конфигурации (%1).'"),
			"%1",
			ИмяОбработки);
	...
	Иначе
		...
		Иначе
			СообщениеОбОшибке = НСтр("ru = 'Ошибка открытия обработки обновления конфигурации. Отсутствует основная форма.'");
			ПодробноеОписаниеОшибки = СтрЗаменить(НСтр("ru = 'Отсутствует основная форма обработки обновления конфигурации (%1).'"),
				"%1",
				ИмяОбработки);
			ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(ПодробноеОписаниеОшибки);
			Возврат Неопределено;
