...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтрСертификатЭЦП       = Параметры.IDCertificateED;
	СтрОрганизация         = Параметры.IDOrganizationED;
	...
	Организация            = ЗначениеИзСтрокиВнутр(СтрОрганизация);
...
Процедура ПриЗакрытии()
	...
	Если НЕ ПрограммноеЗакрытие
		И НЕ ИнтернетПоддержкаПользователейКлиент.ФормаОткрыта(КонтекстВзаимодействия, "txЛичныйКабинет") Тогда
...
Процедура ОбработкаНавигационнойСсылкиТехПоддержка(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылка = "TechSupport" Тогда
		...
		ПараметрыСообщения = Новый Структура("ТипСообщения", 9);
...
Процедура УстановитьСтатусФормы()
	...
	ИначеЕсли СтатусЗаявки = "rejected" Тогда
		...
		Элементы.ВыполнитьДействие.Заголовок = "Создать заявку";
		Элементы.ВыполнитьДействие.Видимость = Истина;
...
// Устанавливает надпись на гиперссылке обновления статуса заявки - количество секудн до автоматического обновления
&НаКлиенте
