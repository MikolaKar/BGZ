...
Процедура ЗагрузитьПапкиМероприятий()
	...
	Выборка = ПолучитьВыборкуСтрокМакета("ПапкиМероприятий", "C1:C11");
	ИмяТип_ГруппыПользователей = "СправочникСсылка.ГруппыПользователей";
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.ИмяГруппыПользователей) И ЗначениеЗаполнено(СсылкаНаПапку) Тогда
			
			ГруппаПользователей = ПолучитьСсылкуИзКэша(
				Выборка.ИмяГруппыПользователей, ИмяТип_ГруппыПользователей);
			...
			Строка.Пользователь = ГруппаПользователей;
...
Процедура ЗагрузитьПапкиФорума()
	...
	Выборка = ПолучитьВыборкуСтрокМакета("ПапкиФорума", "C1:C11");
	ИмяТип_ГруппыПользователей = "СправочникСсылка.ГруппыПользователей";
	ЗаполняемыеСвойства = "Наименование";
...
// Создает организацию по умолчанию.
// Добавляет ее в КэшСсылокСозданныхОбъектов и возвращает на нее ссылку.
//
Функция СоздатьОрганизацию()
	
	СтруктураОрганизации = 
		Справочники.Организации.ПолучитьСтруктуруОрганизации();
		
	СтруктураОрганизации.Наименование = НСтр("ru = 'Наша организация'");
	СтруктураОрганизации.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо;
	Организация = Справочники.Организации.СоздатьОрганизацию(СтруктураОрганизации);
...
Процедура ЗагрузитьНастройкиУведомлений()
	...
	Пока Выборка.Следующий() Цикл
		...
		РаботаСУведомлениями.УстановитьНастройкуПоУмолчанию(Настройка, Событие, Значение);
...
Процедура ЗагрузитьРолиИсполнителей()
	...
	Пока ВыборкаПользователей.Следующий() Цикл
		...
		Если Пользователи.ЭтоПолноправныйПользователь(ВыборкаПользователей.Ссылка) Тогда
			...
			Запись.РольИсполнителя = РольАдминистратор;
...
// Создает новые группы пользователей на основе макета ГруппыПользователей.
//
Процедура ЗагрузитьГруппыПользователей()
	
	МенеджерОбъекта = Справочники.ГруппыПользователей;
	Выборка = ПолучитьВыборкуСтрокМакета("ГруппыПользователей", "C1:C5");
	ИмяТекущегоТипа = "СправочникСсылка.ГруппыПользователей";
	...
	Пока Выборка.Следующий() Цикл
		...
		СтруктураСвойств = МенеджерОбъекта.ПолучитьСтруктуруГруппПользователей();
		...
		СсылкаНаСозданныйОбъект = МенеджерОбъекта.СоздатьГруппуПользователей(СтруктураСвойств);
		ДобавитьСсылкуВКэш(Выборка.Имя, СсылкаНаСозданныйОбъект);
		
		Если ЗначениеЗаполнено(Выборка.ИмяГруппаДоступа) Тогда
			
			ДобавитьПользователяВГруппуДоступа(Выборка.ИмяГруппаДоступа, СсылкаНаСозданныйОбъект);
...
Процедура ДобавитьПользователяВГруппуДоступа(ИмяГруппыДоступа, Пользователь)
...
Процедура ЗагрузитьПапкиФайлов()
	...
	Выборка = ПолучитьВыборкуСтрокМакета("ПапкиФайлов", "C1:C11");
	ИмяТип_ГруппыПользователей = "СправочникСсылка.ГруппыПользователей";
	ЗаполняемыеСвойства = "Наименование";
...
Процедура ЗагрузитьВидыВнутреннихДокументов()
	...
	Выборка = ПолучитьВыборкуСтрокМакета("ВидыВнутреннихДокументов", "C1:C18");
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			СтруктураВидаВнутреннихДокументов.АвтоматическиВестиСоставУчастниковРабочейГруппы =
				ПреобразоватьЗначениеВБулево(Выборка.АвтоматическиВестиСоставУчастниковРабочейГруппы);
			СтруктураВидаВнутреннихДокументов.ВестиУчетПоКорреспондентам =
				ПреобразоватьЗначениеВБулево(Выборка.ВестиУчетПоКорреспондентам);
...
Процедура ЗагрузитьШаблоныПроцессов()
	...
	Пока Выборка.Следующий() Цикл
		...
		СтруктураШаблона.РазныеСроки = Выборка.РазныеСроки = "да";
		ЗаполнитьСтруктуруШаблона(СтруктураШаблона, Выборка, ИмяТипа_Шаблона, Истина);
...
Процедура ЗаполнитьСтруктуруШаблона(
	...
	ЗаполнитьЗначенияСвойств(СтруктураШаблона, Выборка, 
		"Наименование, НаименованиеБизнесПроцесса, Описание, СрокИсполнения");
	...
	Если НЕ НесколькоИсполнителей Тогда
		...
		Если Выборка.Исполнитель <> "" Тогда
			...
			Исполнитель = ПолучитьСсылкуИзКэша(Выборка.Исполнитель, "СправочникСсылка.РолиИсполнителей");
		...
		Если НесколькоИсполнителей Тогда
			...
			Если ЗначениеЗаполнено(Исполнитель) Тогда
				...
				Если ИмяТипа_Шаблона = "СправочникСсылка.ШаблоныИсполнения" Тогда
					...
					НоваяСтрокаИсполнитель.СрокИсполнения = Выборка.СрокИсполнения;
...
Процедура ЗаполнитьПривязкиШаблонаКВидамДокументов(СтруктураШаблона, Выборка, ПустаяСтрока)
	...
	Если Выборка.НастрокиШаблонаИмяТипаВидаДокумента <> "" Тогда
		
		НоваяНастройка = СтруктураШаблона.НастрокиШаблона.Добавить();
		ВидДокумента = ПолучитьСсылкуИзКэша(
		Выборка.НастрокиШаблонаВидДокумента, Выборка.НастрокиШаблонаИмяТипаВидаДокумента);
		НоваяНастройка.ВидДокумента = ВидДокумента;
		
		Если Выборка.НастрокиШаблонаВидИнтерактивногоСобытия <> "" Тогда
			НоваяНастройка.ИнтерактивныйЗапуск = Истина;
			НоваяНастройка.ВидИнтерактивногоСобытия = Перечисления.ВидыИнтерактивныхДействий[
			Выборка.НастрокиШаблонаВидИнтерактивногоСобытия];
		КонецЕсли;
		...
		Если Выборка.НастрокиШаблонаУсловие <> "" Тогда
			НоваяНастройка.Условие = ПолучитьСсылкуИзКэша(
				Выборка.НастрокиШаблонаУсловие,
...
Процедура ЗагрузитьПапкиВнутреннихДокументов()
	...
	ИмяТипа_СтруктураПредприятия = "СправочникСсылка.СтруктураПредприятия";
	ИмяТипа_ГруппыПользователей = "СправочникСсылка.ГруппыПользователей";
	
	МенеджерОбъекта = Справочники.ПапкиВнутреннихДокументов;
	Выборка = ПолучитьВыборкуСтрокМакета("ПапкиВнутреннихДокументов", "C1:C11");
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.ИмяГруппыПользователей) И ЗначениеЗаполнено(СсылкаНаПапку) Тогда
			...
			ГруппаПользователей = ПолучитьСсылкуИзКэша(
				Выборка.ИмяГруппыПользователей, ИмяТипа_ГруппыПользователей);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.GUIDИнструкции) Тогда
			...
			Если ЗначениеЗаполнено(Выборка.ДоступнаДляРоли) Тогда
				...
				Роль = ПолучитьСсылкуИзКэша(Выборка.ДоступнаДляРоли, "СправочникСсылка.РолиИсполнителей");
...
//
// Параметры:
//   - ПредметИнструкции - Строка - пердмет инструкции
...
Процедура ЗагрузитьШаблоныВнутреннихДокументов()
	...
	Выборка = ПолучитьВыборкуСтрокМакета("ШаблоныВнутреннихДокументов", "C1:C13");
	Пока Выборка.Следующий() Цикл
		
		Имя = Выборка.Имя;
		
		Если Выборка.ЭтоГруппа = "да" Тогда
			
			СтруктураГруппыШаблонов = 
				Справочники.ШаблоныВнутреннихДокументов.ПолучитьСтруктуруГруппыШаблонов();
				
			СтруктураГруппыШаблонов.Наименование = Выборка.Наименование;
			
			Если ЗначениеЗаполнено(Выборка.Родитель) Тогда
				СтруктураГруппыШаблонов.Родитель = ПолучитьСсылкуИзКэша(
					Выборка.Родитель, "СправочникСсылка.ШаблоныВнутреннихДокументов");
			КонецЕсли;
			
			СсылкаНаНовуюГруппуШаблона = 
				Справочники.ШаблоныВнутреннихДокументов.СоздатьГруппуШаблонов(СтруктураГруппыШаблонов);
			
			ДобавитьСсылкуВКэш(Имя, СсылкаНаНовуюГруппуШаблона);
		...
		Иначе
			...
			Если ЗначениеЗаполнено(Выборка.Родитель) Тогда
				СтруктураШаблона.Родитель = ПолучитьСсылкуИзКэша(
					Выборка.Родитель, "СправочникСсылка.ШаблоныВнутреннихДокументов");
			КонецЕсли;
			...
			// Обход строк табличной части Файлы
			Пока Выборка.Следующий() Цикл
				
				ПустаяСтрока = Истина;
				
				Если Выборка.ШаблонФайла <> "" Тогда
					
					ФайлСсылка = ПолучитьСсылкуИзКэша(Выборка.ШаблонФайла, "СправочникСсылка.Файлы");
					НоваяСтрока = СтруктураШаблона.ПрикрепленныеФайлы.Добавить();
					НоваяСтрока.ФайлСсылка = ФайлСсылка;
...
Процедура ЗагрузитьШаблоныИсходящихДокументов()
	...
	Выборка = ПолучитьВыборкуСтрокМакета("ШаблоныИсходящихДокументов", "C1:C9");
	Пока Выборка.Следующий() Цикл
