...
Процедура НайтиСервер(ВозможныеСтрокиПоиска, СтрокКоличество)
	
	ГСЧ = Новый ГенераторСлучайныхЧисел;
	СтрокаПоиска = ВозможныеСтрокиПоиска[ГСЧ.СлучайноеЧисло(0, СтрокКоличество - 1)];
	РазмерПорции = 20;
...
Функция ТЦВыполнить() Экспорт
	...
	#Иначе
		...
		ВозможныеСтрокиПоиска = Новый Массив;
		Для Каждого СтрокаПоиска Из ТЦОбъект.СтрокиПоиска Цикл
			ВозможныеСтрокиПоиска.Добавить(СтрокаПоиска.СтрокаПоиска);
		КонецЦикла;
		КоличествоПовторов = 5;
		
		Для а = 0 По КоличествоПовторов - 1 Цикл
			
			ТестированиеКлиентСервер.Пауза(Инструменты, 60000);
			
			ВремяНачала = ТестированиеКлиентСервер.ЗначениеТаймера(Инструменты);
			
			НайтиСервер(ВозможныеСтрокиПоиска, ВозможныеСтрокиПоиска.Количество());
			
			ВремяОкончания = ТестированиеКлиентСервер.ЗначениеТаймера(Инструменты);
			
			ТЦЗаписатьПоказатель("Полнотекстовый поиск", (ВремяОкончания - ВремяНачала) / 1000);
