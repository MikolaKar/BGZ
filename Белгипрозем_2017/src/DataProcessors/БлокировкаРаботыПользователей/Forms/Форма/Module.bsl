...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Иначе	
		...
		Если НЕ ТолькоКлиентскиеПриложения И Не ОбщегоНазначенияПовтИсп.РазделениеВключено() И КлиентПодключенЧерезВебСервер
			И Не СоединенияИБВызовСервераПовтИсп.ПараметрыОтключенияСеансов().WindowsПлатформаНаСервере Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура ПрименитьЗавершение(Ответ, Вариант) Экспорт
	...
	Если Не ЭтоФайловаяБаза И Не ВведеныКорректныеПараметрыАдминистрирования Тогда
