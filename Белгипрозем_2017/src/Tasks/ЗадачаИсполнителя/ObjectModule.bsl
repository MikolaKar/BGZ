...
Процедура ПередЗаписью(Отказ)
	...
	Если НЕ Ссылка.Пустая() Тогда
		Если Мультипредметность.ИзмененыПредметыЗадачи(ЭтотОбъект) Тогда
	...
	Если НЕ Ссылка.Пустая() Тогда
		...
		Адресация = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(Ссылка, 
		...
		Если Адресация.Исполнитель <> Исполнитель Тогда
			...
			Если ЗначениеЗаполнено(Адресация.РольИсполнителя) Тогда
				...
				ИсполнителиРоли = БизнесПроцессыИЗадачиСервер.ИсполнителиРоли(
					Адресация.РольИсполнителя,
					Адресация.ОсновнойОбъектАдресации,
					Адресация.ДополнительныйОбъектАдресации);
		...
		Если Адресация.РольИсполнителя <> РольИсполнителя
			ИЛИ Адресация.ОсновнойОбъектАдресации <> ОсновнойОбъектАдресации
			ИЛИ Адресация.ДополнительныйОбъектАдресации <> ДополнительныйОбъектАдресации Тогда
	...
	ИсполнительИлиРоль = ?(ЗначениеЗаполнено(Исполнитель), Исполнитель, РольИсполнителя);
	Если ТекущийИсполнитель <> ИсполнительИлиРоль Тогда
		ТекущийИсполнитель = ИсполнительИлиРоль;
		Если ТекущийИсполнитель = Исполнитель Тогда
			ТекущийОсновнойОбъектАдресации = Неопределено;
			ТекущийДополнительныйОбъектАдресации = Неопределено;
		Иначе
			ТекущийОсновнойОбъектАдресации = ОсновнойОбъектАдресации;
			ТекущийДополнительныйОбъектАдресации = ДополнительныйОбъектАдресации;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриЗаписи(Отказ)
	...
	Если ДополнительныеСвойства.Свойство("ЗадачаВыполнена") Тогда
		...
		Если ДополнительныеСвойства.ЗадачаВыполнена Тогда
			БизнесСобытияВызовСервера.ЗарегистрироватьСобытие(Ссылка, Справочники.ВидыБизнесСобытий.ВыполнениеЗадачи);
		КонецЕсли;
	КонецЕсли;
	...
	Если ЕстьЗаполненныеПредметы Тогда 
			...
			Если ЗначениеЗаполнено(СтарыеРеквизитыАдресации) Тогда
				...
				Если ЗначениеЗаполнено(СтарыеРеквизитыАдресации.РольИсполнителя) Тогда
					...
					РаботаСРабочимиГруппами.ДобавитьУчастникаВТаблицуНабора(
						...
						СтарыеРеквизитыАдресации.ОсновнойОбъектАдресации,
						СтарыеРеквизитыАдресации.ДополнительныйОбъектАдресации);
...
Процедура ЗарегистрироватьПеренаправлениеЗадачи()
	...
	ПараметрыСобытия = Новый Структура("БылИсполнитель, СталИсполнитель", БылИсполнитель, СталИсполнитель);
