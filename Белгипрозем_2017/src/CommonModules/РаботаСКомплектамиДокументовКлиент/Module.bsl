...
Процедура УстановитьДоступностьКомандРаботыСЭлементамиКомплекта(Форма)
	...
	УстановитьДоступностьКоманды(ДоступныеПоля, Элементы.СоставКомплектаПодобратьВнутренниеДокументы, Найти(Форма.ПраваДоступа, "ВнутренниеДокументы.Чтение") > 0);
	УстановитьДоступностьКоманды(ДоступныеПоля, Элементы.СоставКомплектаПодобратьВходящиеДокументы, Найти(Форма.ПраваДоступа, "ВходящиеДокументы.Чтение") > 0);
	УстановитьДоступностьКоманды(ДоступныеПоля, Элементы.СоставКомплектаПодобратьИсходящиеДокументы, Найти(Форма.ПраваДоступа, "ИсходящиеДокументы.Чтение") > 0);
	УстановитьДоступностьКоманды(ДоступныеПоля, Элементы.СоставКомплектаПодобратьФайлы, Найти(Форма.ПраваДоступа, "Файлы.Чтение") > 0);
	...
	Иначе
		...
		УстановитьДоступностьКоманды(ДоступныеПоля, Элементы.ЭлементКомплектаУдалить, ЭтоПервыйУровеньДерева);
		УстановитьДоступностьКоманды(ДоступныеПоля, Элементы.КонтекстноеМенюЭлементКомплектаУдалить, ЭтоПервыйУровеньДерева);
		...
		УстановитьДоступностьКоманды(ДоступныеПоля, Элементы.КонтекстноеМенюЭлементКомплектаПереместитьВверх, ЭтоПервыйУровеньДерева И Не ЭтоПерваяСтрока);
		УстановитьДоступностьКоманды(ДоступныеПоля, Элементы.КонтекстноеМенюЭлементКомплектаПереместитьВниз, ЭтоПервыйУровеньДерева И Не ЭтоПоследняяСтрока);
...
Процедура СоставКомплектаВыбор(Форма, Элемент, Поле, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ЭтоФайл(Ссылка) Тогда
		...
		Если КакОткрывать = "ОткрыватьКарточку" Тогда
			...
			Возврат;
		КонецЕсли;
		ДанныеФайла = РаботаСФайламиВызовСервера.ПолучитьДанныеФайлаДляОткрытия(Ссылка, Неопределено, Форма.УникальныйИдентификатор);
...
Процедура ЭлементКомплектаОбновитьИзФайлаНаДиске(Форма) Экспорт
	...
	ДанныеФайла = РаботаСФайламиВызовСервера.ПолучитьДанныеФайлаИРабочийКаталог(Ссылка);
...
Процедура ЭлементКомплектаОтменитьРедактированиеФайла(Форма) Экспорт
	...
	РаботаСФайламиКлиент.ОсвободитьФайлСОповещением(
		Обработчик,
		Ссылка,,
		Форма.Элементы.СоставКомплекта.ТекущиеДанные.РедактируетТекущийПользователь,
		Форма.Элементы.СоставКомплекта.ТекущиеДанные.Редактирует);
...
Процедура ЭлементКомплектаСохранитьФайлКак(Форма) Экспорт
	...
	Если Форма.Элементы.СоставКомплекта.ВыделенныеСтроки.Количество() = 1 Тогда
		...
		ДанныеФайла = РаботаСФайламиВызовСервера.ПолучитьДанныеФайлаДляСохранения(Ссылка, Неопределено, Форма.УникальныйИдентификатор);
