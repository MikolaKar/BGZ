...
Процедура СформироватьЧисловойНомерДокумента(СтруктураПараметров, СформированныйНомер) Экспорт 
	...
	// измерения нумерации
	ПериодНумерации = НачалоПериодаНумерации(Нумератор.Периодичность, СтруктураПараметров.ДатаРегистрации);
	...
	Если Нумератор.НезависимаяНумерацияПоСвязанномуДокументу Тогда 
		Если ЗначениеЗаполнено(СтруктураПараметров.Ссылка) Тогда 
			СвязанныйДокумент = СвязиДокументов.ПолучитьСвязанныйДокумент(СтруктураПараметров.Ссылка, Нумератор.ТипСвязи);
		Иначе
	...
	Если Нумератор.НезависимаяНумерацияПоПроекту И ПолучитьФункциональнуюОпцию("ВестиУчетПоПроектам") Тогда 
		Проект = СтруктураПараметров.Проект;
	Иначе
		Проект = Справочники.Проекты.ПустаяСсылка();
	КонецЕсли;
	
	Если Нумератор.НезависимаяНумерацияПоВопросуДеятельности И ПолучитьФункциональнуюОпцию("ИспользоватьВопросыДеятельности") Тогда 
...
Процедура СформироватьСтроковыйНомерДокумента(СтруктураПараметров, СформированныйНомер, ОписанияОшибок) Экспорт 
	...
	// автоматическая нумерация
	Если Не РазобратьФорматНомера(Нумератор.ФорматНомера, ОписаниеОшибки, СтруктураФорматаНомера) Тогда 
		СформированныйНомер = "";
...
Функция ПолучитьНумераторДокумента(Объект) Экспорт
    ...
    |			ИЛИ Настройки.ВидДокумента НЕ В (&ПустыеВидыДокументов)
    |				И Настройки.ВидДокумента = &ВидДокумента
    |				И &ФОВидДокумента = ИСТИНА)
...
Функция СформироватьПолеНумеровать(СпособНумерации, Нумератор) Экспорт 
	...
	Если ЗначениеЗаполнено(СпособНумерации) Тогда 
		...
		Если ЗначениеЗаполнено(Нумератор) Тогда 
			...
			ФорматНомера = ОбщегоНазначения.ПолучитьЗначениеРеквизита(Нумератор, "ФорматНомера"); 
...
//Разбирает строковый номер объекта по его нумератору, игнорируя ошибочно внесенные пробелы, и пытается выделить числовой номер
//если невозможно это сделать, то вернет -1
Функция ВыделитьЧисловойНомерИзСтрокового(Объект, ОписаниеОшибки) Экспорт
	Перем ТЗПараметровФорматаНомера, СтруктураЗначенийПараметровНомера, ЧисловойНомер;
	
	Нумератор = Нумерация.ПолучитьНумераторДокумента(Объект);
	//заполняем ТЗПараметровФорматаНомера
	Нумерация.РазобратьФорматНомера(Нумератор.ФорматНомера, ОписаниеОшибки, ТЗПараметровФорматаНомера);
	//заполняем СтруктураЗначенийПараметровНомера
	СтруктураЗначенийПараметровНомера = ПолучитьЗначенияПараметровНомера(Объект, ТЗПараметровФорматаНомера, ОписаниеОшибки);
	Если СтруктураЗначенийПараметровНомера.Свойство("Номер") = Ложь Тогда
		ДополнитьСтрокуСписокОшибокНовой(ОписаниеОшибки,НСтр("ru = 'В нумераторе отсутствует параметр Номер '"));
		Возврат -1;
	КонецЕсли;
	
	//заполняем ТЗПараметров их значениями
	ТЗПараметровФорматаНомера.Колонки.Добавить("ЗначениеП", Новый ОписаниеТипов("Строка")); // значение параметра
	Для Каждого СтрТЗ из ТЗПараметровФорматаНомера Цикл
		Если СтрТЗ.Ключ = "Разделитель" Тогда
			СтрТЗ.ЗначениеП = СтрТЗ.Значение;
		Иначе
			ЗначениеП       = "";
			СтруктураЗначенийПараметровНомера.Свойство(СтрТЗ.Значение,ЗначениеП);
			СтрТЗ.ЗначениеП = ?(ЗначениеП = Неопределено,"",ЗначениеП);//не должно быть Неопределено!
		КонецЕсли;
	КонецЦикла;
	
	СтроковыйНомер = Объект.РегистрационныйНомер;
	Если НЕ МожноВыделитьПараметрыНумератораОбъектаИзСтрокиНомера(СтроковыйНомер, ТЗПараметровФорматаНомера, ОписаниеОшибки) Тогда
		//не смогли разобрать строку по параметрам нумератора
		Возврат -1;
	КонецЕсли;
	
	//СтроковыйНомер теперь должен содержать только ЧисловойНомер
	ЧисловойНомер = ОбщегоНазначения.ВЧисло(СтроковыйНомер);
	Если СокрЛП(СтроковыйНомер) = Формат(ЧисловойНомер,"ЧГ = 0") Тогда
		Возврат ЧисловойНомер;
	Иначе
		ДополнитьСтрокуСписокОшибокНовой(ОписаниеОшибки,НСтр("ru = 'Представление числового номера ["+СтроковыйНомер+"] содержит, помимо цифр, символы '"));
		Возврат -1;
	КонецЕсли;
	
КонецФункции

//вспомогательные для ВыделитьЧисловойНомерИзСтрокового
//склеивает строки ошибок переводами строк
Процедура ДополнитьСтрокуСписокОшибокНовой(ОписаниеОшибок,НоваяОшибка)
	ОписаниеОшибок = ОписаниеОшибок + ?(ПустаяСтрока(ОписаниеОшибок),"",Символы.ПС + Символы.ВК) + НоваяОшибка;
КонецПроцедуры

//пытается разобрать префикс и суффикс номера и если удачно - возвращает Истину 
//в параметре СтроковыйНомер остается неразобранная часть
Функция МожноВыделитьПараметрыНумератораОбъектаИзСтрокиНомера(СтроковыйНомер, ТЗПараметровФорматаНомера, ОписаниеОшибки)
	//выделяем из строкового номера параметры (по параметр Номер!)
	//если они совпадают, то удаляем из исходной строки
	КолСтрТЗ = ТЗПараметровФорматаНомера.Количество();
	Для н = 0 По КолСтрТЗ - 1 Цикл
		СтрТЗ = ТЗПараметровФорматаНомера.Получить(н);
		Если СтрТЗ.Значение = "Номер" Тогда
			Прервать;
		Иначе//ищем в строковом номере
			СтрТЗ.ЗначениеП = СокрЛ(СтрТЗ.ЗначениеП);
			СтроковыйНомер  = СокрЛ(СтроковыйНомер);
			Если СтрНачинаетсяС(СтроковыйНомер,СтрТЗ.ЗначениеП) Тогда
				//все круто, удаляем обработанный параметр из ТЗ и СтроковогоНомера
				СтроковыйНомер = Сред(СтроковыйНомер,СтрДлина(СтрТЗ.ЗначениеП) + 1);
				ТЗПараметровФорматаНомера.Удалить(СтрТЗ);
				
				н        = н - 1;
				КолСтрТЗ = КолСтрТЗ - 1;
			Иначе//значение параметра не найдено
				ДополнитьСтрокуСписокОшибокНовой(ОписаниеОшибки,НСтр("ru = 'Не найден параметр "+ СтрТЗ.Значение + "'"));
				Возврат Ложь;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	//выделяем из строкового номера параметры (по параметр Номер!) идем с конца
	//если они совпадают, то удаляем из исходной строки
	н = ТЗПараметровФорматаНомера.Количество() - 1;
	Пока н >= 0  Цикл
		СтрТЗ = ТЗПараметровФорматаНомера.Получить(н);
		Если СтрТЗ.Значение = "Номер" Тогда
			Прервать;
		Иначе//ищем в строковом номере
			СтрТЗ.ЗначениеП = СокрП(СтрТЗ.ЗначениеП);
			СтроковыйНомер  = СокрП(СтроковыйНомер);
			Если СтрЗаканчиваетсяНа(СтроковыйНомер,СтрТЗ.ЗначениеП) Тогда
				//все круто, удаляем обработанный параметр из ТЗ и СтроковогоНомера
				СтроковыйНомер = Лев(СтроковыйНомер,СтрДлина(СтроковыйНомер) - СтрДлина(СтрТЗ.ЗначениеП));
				ТЗПараметровФорматаНомера.Удалить(СтрТЗ);
			Иначе//значение параметра не найдено
				ДополнитьСтрокуСписокОшибокНовой(ОписаниеОшибки,НСтр("ru = 'Не найден параметр "+ СтрТЗ.Значение + "'"));
				Возврат Ложь;
