...
Функция ПолучитьПолныйПутьКатегорииДанных(Категория) Экспорт
	...
	Попытка
	    ...
	    ТекКатегория = Категория.Родитель;
		Если НЕ ЗначениеЗаполнено(ТекКатегория.Наименование) Тогда
			Возврат ПолныйПуть;
		КонецЕсли;
		Пока ЗначениеЗаполнено(ТекКатегория) Цикл
			Если НЕ ТекКатегория = Справочники.КатегорииДанных.ВсеКатегории Тогда
				...
				Попытка
					Если ЗначениеЗаполнено(ТекКатегория.Родитель) Тогда
						ТекКатегория = ТекКатегория.Родитель;
					КонецЕсли;
				Исключение
					Прервать;
				КонецПопытки;	
...
Функция ПостроитьДеревоКатегорий(Дерево, 
	...
	ВключатьЭлементБезКатегорий = Истина,
	СчитатьОбращенияГраждан = Ложь) Экспорт
...
Процедура ЗначениеКатегорииВЗапросСписка(Список, СУчетомПодкатегорий, МаксимальноеКоличествоПараметров, Категория) Экспорт
	...
	Если СУчетомПодкатегорий Тогда
		...
		Список.Параметры.УстановитьЗначениеПараметра("КатегорияИИЛИПередана", Истина);
	Иначе
		КатегорияИЛИ.Добавить(Категория);
		Список.Параметры.УстановитьЗначениеПараметра("КатегорияИЛИ", КатегорияИЛИ);
		Список.Параметры.УстановитьЗначениеПараметра("КатегорияИЛИПередана", Истина);
	КонецЕсли;	
...
Процедура ЗаписатьСписокКатегорийУОбъекта(СписокКатегорий, Объект) Экспорт
	...
	Для каждого Запись Из Записи Цикл
		Если Категории.Найти(Запись.КатегорияДанных) = Неопределено Тогда
			...
			ИначеЕсли НЕ Запись.КатегорияДанных.Персональная И РольДоступнаДобавлениеИзменениеОбщихКатегорий Тогда
...
Функция ВыполнитьПроверкуВыраженияВУсловии(Источник, Выражение)
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Категории данных.Выполнение автоматической категоризации'"),
...
Функция ВыполнитьПроверкуПрименимостиПравилаАвтоКатегоризацииНаОбъекте(Правило, Объект) Экспорт
	...
	Для Каждого Условие Из Правило.Условия Цикл
		Если Условие.ВидУсловия = Перечисления.ВидыУсловийПравилАвтоматическойКатегоризации.ВычисляемоеВыражение Тогда
			Результат = ВыполнитьПроверкуВыраженияВУсловии(Объект, Условие.Выражение);	
...
Процедура ЗагрузитьДанныеОКатегориях(Элементы, Объект, СписокКатегорийДанных, ИспользоватьАвтоматическуюКатегоризацию, Параметры) Экспорт
	...
	СписокКатегорийОбъекта = ПолучитьСписокКатегорийОбъекта(Объект.Ссылка);
...
Процедура ПрисвоитьКатегориюМассивуОбъектов(Объекты, Категория) Экспорт
	...
	Для Каждого Объект Из Объекты Цикл
		...
		Если НЕ Найдена Тогда
			УстановитьКатегориюУОбъекта(ПараметрыСеанса.ТекущийПользователь,
				Категория, 
				Объект);
		КонецЕсли;
	КонецЦикла;
...
Процедура УстановитьСнятьПометкуУдаленияУКатегории(Категория) Экспорт
	...
	Если НЕ Категория.Персональная
		...
		Если ЗаписьОКатегории.Выбран() Тогда
			Если НЕ РольДоступна(Метаданные.Роли.ДобавлениеИзменениеОбщихКатегорий)
				...
				И (ТипЗнч(ЗаписьОКатегории.Автор) = Тип("СправочникСсылка.ПравилаАвтоматическойКатегоризацииДанных")
					ИЛИ ЗаписьОКатегории.Автор <> ПараметрыСеанса.ТекущийПользователь) Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			Если НЕ РольДоступна(Метаданные.Роли.ДобавлениеИзменениеОбщихКатегорий)
				И НЕ РольДоступна(Метаданные.Роли.ПолныеПрава) 
				И НЕ (ЗаписьОКатегории.КатегорияДанных.Персональная И ЗаписьОКатегории.КатегорияДанных.Автор = ПараметрыСеанса.ТекущийПользователь)
				И НЕ (НЕ ЗаписьОКатегории.КатегорияДанных.Персональная И РольДоступна(Метаданные.Роли.ДобавлениеИзменениеОбщихКатегорий)) Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Недостаточно прав для удаления категории ""%1"".'"),
					ДанныеСтроки.Значение);
				ВызватьИсключение ТекстСообщения;
...
Функция ПолучитьПолныйСписокДоступаКатегории(Категория, ПустойСписокДоступа = Ложь) 
	...
	Если Категория.Доступ.Количество() = 0 Тогда
		...
		Возврат РаботаСПользователями.ПолучитьПользователейГруппыПользователей(Справочники.ГруппыПользователей.ВсеПользователи);	
	КонецЕсли;
	...
	Для Каждого ЭлементДоступа Из Категория.Доступ Цикл
		Если ТипЗнч(ЭлементДоступа.Пользователь) = Тип("СправочникСсылка.Пользователи") Тогда
			...
			КонецЕсли;
		КонецЕсли;
		Если ТипЗнч(ЭлементДоступа.Пользователь) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
			СписокПользователейГруппы = РаботаСПользователями.ПолучитьПользователейГруппыПользователей(ЭлементДоступа.Пользователь);	
...
Функция ПроверитьОграниченностьДоступаКатегории(Категория) Экспорт
	...
	СписокВсехПользователей = РаботаСПользователями.ПолучитьПользователейГруппыПользователей(Справочники.ГруппыПользователей.ВсеПользователи);
