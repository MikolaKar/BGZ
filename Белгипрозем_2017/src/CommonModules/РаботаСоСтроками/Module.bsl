...
// Если передана однострочная строка, то если её длина привышает переданную ширину,
// то преобразует переданную строку в многострочную строку определенной ширины.
// Длина значимой содержательной части строки (без символов кворитрования) будет минимум половина 
...
// Возвращает индекс первого значащего символа строки. Незначащами символами 
// считаются стандартные незначащие символы и переданные в параметре СписокНезначащихСимволов.
Функция НайтиПервыйЗначащийСимволСтроки(Строка, СписокНезначащихСимволов) Экспорт
	...
	Пока Не НайденЗначащийСимвол Цикл
		...
		ДлинаСтрокиСНезначащамиСимволами = СтрДлина(ОбрабатываемаяСтрока);
		ОбрабатываемаяСтрока = СокрЛ(ОбрабатываемаяСтрока);
		ПоложениеПервогоСимвола = 
			ПоложениеПервогоСимвола 
			+ ДлинаСтрокиСНезначащамиСимволами 
...
// Функция возвращает значение Истина, если первый значащай символ в строке является
...
// Если обработанноя строка заканчивается на обрабатываемый знак, тогда процедура переносит
...
// из переданных символов переписки. В соответвии с шириной полученного начала строки уменьшается
// значение  
