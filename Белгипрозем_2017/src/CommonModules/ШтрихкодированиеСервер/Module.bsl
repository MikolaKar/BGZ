...
Функция НайтиОбъектыПоШтрихкоду(Код, ИскатьЗадачиПоОбъектам = Ложь) Экспорт
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ТипЗнч(Владелец) <> Тип("СправочникСсылка.мЭтапыДоговоров") Тогда
		...
		//ИначеЕсли ТипЗнч(Владелец) = Тип("СправочникСсылка.мЭтапыДоговоров") Тогда
		//	// Для смет ШК печатается из этапа, а файл будет подшиваться к договору
		//	ДанныеВозврата.Вставить("Ключ", Владелец.Владелец.Ссылка);
		//	ДанныеВозврата.Вставить("Метаданные", Владелец.Владелец.Ссылка.Метаданные().Имя);
		// 	ДанныеВозврата.Вставить("ПредставлениеОбъекта", Владелец.Владелец.Ссылка.Метаданные().ПредставлениеОбъекта);
	...
	ИначеЕсли ИскатьЗадачиПоОбъектам И МассивНайденных.Количество() = 1 Тогда 
		...
		Запрос.Текст = 
			...
			|		ПО (ЗадачаИсполнителяПредметы.Ссылка = ЗадачиПоИсполнителю.Ссылка)
			|ГДЕ
			...
			|	И ЗадачиПоИсполнителю.Выполнена = ЛОЖЬ
...
Функция ВставитьРегистрационныйШтампВФайлDocxСУказаниемПоложения(
	...
	Если ЗначениеЗаполнено(ТекстНадписи) Тогда
		...
		Иначе
			МакетДляВставки = ПолучитьОбщийМакет("ВставкаРегистрационногоШтампаВDocxБезШтихкода");
	    КонецЕсли;
	Иначе
