...
Процедура ПроинициализироватьПараметрыУчетаВремени(
	...
	Если ТипЗнч(Ссылка) = Тип("ЗадачаСсылка.ЗадачаИсполнителя") Тогда
		...
		Если ЗначениеЗаполнено(Ссылка.Исполнитель) Тогда
			Если ИсполнителиЗадач.Найти(Ссылка.Исполнитель) = Неопределено Тогда
				ЭлементПереключитьХронометраж.Доступность = Ложь;
		...
		ИначеЕсли ЗначениеЗаполнено(Ссылка.РольИсполнителя) Тогда
			...
			Запрос.УстановитьПараметр("ОсновнойОбъектАдресации", Ссылка.ОсновнойОбъектАдресации);
			Запрос.УстановитьПараметр("ДополнительныйОбъектАдресации", Ссылка.ДополнительныйОбъектАдресации);
		...
		Если ДлительностьРаботы >= 60 Тогда // больше 1 минуты
			...
			Если ПолучитьФункциональнуюОпцию("ВестиУчетПоПроектам") Тогда 
				...
				ИначеЕсли ТипЗнч(Источник) = Тип("СправочникСсылка.Контроль") Тогда
					Если ЗначениеЗаполнено(Источник.Предмет) Тогда  
