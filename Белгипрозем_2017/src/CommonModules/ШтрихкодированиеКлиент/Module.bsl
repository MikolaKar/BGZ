...
Процедура ПоискПоШтрихкоду(СтрокаШК, ОкноВладелец) Экспорт
	...
	ИначеЕсли МассивНайденных.Количество() > 1 Тогда 
		...
		Иначе
			ПараметрыФормы.Вставить("ПредметБизнесПроцесса", МассивНайденных[0].ПредметБизнесПроцесса);
			ОткрытьФорму("Задача.ЗадачаИсполнителя.Форма.МоиЗадачи", ПараметрыФормы, ОкноВладелец, , , , , 
				РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
		КонецЕсли;
	Иначе
