...
Функция ВыполнитьПоиск(Знач ТаблицаУсловий, ИскатьВсе = Ложь) Экспорт
	...
	Для Каждого Строка Из ТаблицаУсловий Цикл
		...
		Если Строка.ВидУсловия = Перечисления.ВидыУсловийПоискаПисем.ПолученоОт Тогда
			...
			Массив.Добавить(Тип("СправочникСсылка.Корреспонденты"));
			...
			Запрос.Текст = 
				...
				|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаОт КАК ВременнаяТаблица
				|		ПО (СведенияОбАдресатах.Представление ПОДОБНО ВременнаяТаблица.Значение СПЕЦСИМВОЛ ""~"")
				|ГДЕ
				...
				|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаОт КАК ВременнаяТаблица1
				|		ПО (Адресаты.Наименование ПОДОБНО ВременнаяТаблица1.Значение СПЕЦСИМВОЛ ""~"")
				|
				|ОБЪЕДИНИТЬ ВСЕ
				|
				|ВЫБРАТЬ
				...
				|	Справочник.АдресатыПочтовыхСообщений КАК Адресаты
				|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаОт КАК ВременнаяТаблица1
				|		ПО (Адресаты.Адрес ПОДОБНО ВременнаяТаблица1.Значение СПЕЦСИМВОЛ ""~"")";
				
			Запрос.УстановитьПараметр("Данные", ТаблицаСтроки);
		...
		ИначеЕсли Строка.ВидУсловия = Перечисления.ВидыУсловийПоискаПисем.КомментарийСодержитСлова Тогда	
			...
			Запрос.Текст = 
				"ВЫБРАТЬ
				|	Таблица.Значение КАК Значение
				|ПОМЕСТИТЬ ВременнаяТаблицаКомментарийСодержитСлова
				|ИЗ
				|	&Данные КАК Таблица
				|";
			...
			ТекстЗапроса = 	
				...
				|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаКомментарийСодержитСлова КАК ВременнаяТаблица
				|			ПО (ВходящееПисьмо.Комментарий ПОДОБНО ВременнаяТаблица.Значение СПЕЦСИМВОЛ ""~"")
				|		ПО ЭлектроннаяПочта.Ссылка = ВходящееПисьмо.Ссылка
				|ГДЕ
				|	НЕ ВходящееПисьмо.Ссылка ЕСТЬ NULL 
				|
				|ОБЪЕДИНИТЬ ВСЕ
				|
				|ВЫБРАТЬ
				...
				|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаКомментарийСодержитСлова КАК ВременнаяТаблица
				|			ПО (ИсходящееПисьмо.Комментарий ПОДОБНО ВременнаяТаблица.Значение СПЕЦСИМВОЛ ""~"")
				|		ПО ЭлектроннаяПочта.Ссылка = ИсходящееПисьмо.Ссылка
				|ГДЕ
				|	НЕ ИсходящееПисьмо.Ссылка ЕСТЬ NULL ";
				
			СтруктураПодзапроса = Новый Структура();
			СтруктураПодзапроса.Вставить("Текст", ТекстЗапроса);
			СтруктураПодзапроса.Вставить("Название", "ВТКомментарийСодержитСлова"); 
			ИтоговыйМассив.Добавить(СтруктураПодзапроса);
		...
		ИначеЕсли Строка.ВидУсловия = Перечисления.ВидыУсловийПоискаПисем.ТемаСодержитУказанныеСлова Тогда	
			...
			Запрос.Текст = 
				"ВЫБРАТЬ
				|	Таблица.Значение КАК Значение
				|ПОМЕСТИТЬ ВременнаяТаблицаТемаСодержитУказанныеСлова
				|ИЗ
				|	&Данные КАК Таблица
				|";
			...
			ТекстЗапроса = 	
				...
				|ИЗ
				|	ЖурналДокументов.ЭлектроннаяПочта КАК ЭлектроннаяПочта
				|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаТемаСодержитУказанныеСлова КАК ВременнаяТаблица
				|		ПО (ЭлектроннаяПочта.Тема ПОДОБНО ВременнаяТаблица.Значение СПЕЦСИМВОЛ ""~"")";
			СтруктураПодзапроса = Новый Структура();
			СтруктураПодзапроса.Вставить("Текст", ТекстЗапроса);
			СтруктураПодзапроса.Вставить("Название", "ВТТемаСодержитУказанныеСлова"); 
			ИтоговыйМассив.Добавить(СтруктураПодзапроса);
...
Процедура УдалитьЗависшиеВременныеРезультатыПоиска() Экспорт
	...
	МассивИдентификаторов = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку(0);
	Для Каждого Ид Из МассивИдентификаторов Цикл
		...
		Набор.Отбор.ИдентификаторПоиска.Установить(Ид);
