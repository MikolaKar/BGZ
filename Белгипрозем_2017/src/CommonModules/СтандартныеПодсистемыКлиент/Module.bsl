...
#КонецЕсли
	...
	// Стандартным образом обрабатывается только предупреждение о резервном копировании
	ПредупрежденийВсего = Предупреждения.Количество();
	Для Сч = 1 По ПредупрежденийВсего Цикл
		ТекущееПредупреждение = Предупреждения[ПредупрежденийВсего - Сч];
		Если Не ТекущееПредупреждение.Свойство("ТекстФлажка")
			Или ТекущееПредупреждение.ТекстФлажка <> "Выполнить резервное копирование" Тогда
			Предупреждения.Удалить(ПредупрежденийВсего - Сч);
		КонецЕсли;
	КонецЦикла;
