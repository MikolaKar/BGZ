...
Процедура СоздатьКарточкуКонтроляПоПроцессу(БизнесПроцесс) Экспорт 
	...
	Для Каждого КарточкаКонтроля Из ВсеКарточкиКонтроля Цикл
		ДанныеКарточкиКонтроля = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(КарточкаКонтроля, 
			"Контролер, 
		...
		Если ДанныеКарточкиКонтроля.Контролер = БизнесПроцесс.Контролер
			И ДанныеКарточкиКонтроля.ОсновнойОбъектАдресацииКонтролера = БизнесПроцесс.ОсновнойОбъектАдресацииКонтролера
			И ДанныеКарточкиКонтроля.ДополнительныйОбъектАдресацииКонтролера = БизнесПроцесс.ДополнительныйОбъектАдресацииКонтролера
			И ДанныеКарточкиКонтроля.ПоставленНаКонтроль 
			И Не ДанныеКарточкиКонтроля.СнятСКонтроля Тогда
			Возврат;
		КонецЕсли;
	КонецЦикла;	
	...
	НовыйЭлемент.ОсновнойОбъектАдресацииКонтролера = БизнесПроцесс.ОсновнойОбъектАдресацииКонтролера;
	НовыйЭлемент.ДополнительныйОбъектАдресацииКонтролера = БизнесПроцесс.ДополнительныйОбъектАдресацииКонтролера;
...
//  ОсновнойОбъектАдресацииКонтролера - ссылка на основной объект адресации контролера
//  ДополнительныйОбъектАдресацииКонтролера - ссылка на дополнительный объект адресации контролера
//
// Возвращаемое значение
...
Функция НайтиКарточкуКонтроляПоПредметуИКонтролеру(Предмет, 
	...
	Запрос.Текст = 
		...
		|	И Контроль.ОсновнойОбъектАдресацииКонтролера = &ОсновнойОбъектАдресацииКонтролера
		|	И Контроль.ДополнительныйОбъектАдресацииКонтролера = &ДополнительныйОбъектАдресацииКонтролера
	...
	Запрос.УстановитьПараметр("ОсновнойОбъектАдресацииКонтролера", ОсновнойОбъектАдресацииКонтролера);
	Запрос.УстановитьПараметр("ДополнительныйОбъектАдресацииКонтролера", ДополнительныйОбъектАдресацииКонтролера);
...
//
Процедура СоздатьСтрокуКонтроляПоЗадаче(Задача) Экспорт 
	...
	Если Не ЗначениеЗаполнено(Задача.Исполнитель)
		...
		И Не ЗначениеЗаполнено(Задача.ОсновнойОбъектАдресации)
		И Не ЗначениеЗаполнено(Задача.ДополнительныйОбъектАдресации) Тогда 
		Возврат;
	КонецЕсли;
	
	БизнесПроцесс = Задача.БизнесПроцесс;
	ВедущаяЗадача = ОбщегоНазначения.ПолучитьЗначениеРеквизита(БизнесПроцесс, "ВедущаяЗадача");
	Если ЗначениеЗаполнено(ВедущаяЗадача) Тогда 
		БизнесПроцесс = ОбщегоНазначения.ПолучитьЗначениеРеквизита(ВедущаяЗадача, "БизнесПроцесс");
	КонецЕсли;
	
	КарточкиКонтроляПоПредмету = ВсеКарточкиКонтроляПоПредмету(БизнесПроцесс);
	Если КарточкиКонтроляПоПредмету.Количество() = 0 Тогда 
		Возврат;
	КонецЕсли;
	...
	Для Каждого КонтрольСсылка Из КарточкиКонтроляПоПредмету Цикл
		...
		Иначе
			...
			НоваяСтрока.ОсновнойОбъектАдресации = Задача.ОсновнойОбъектАдресации;
			НоваяСтрока.ДополнительныйОбъектАдресации = Задача.ДополнительныйОбъектАдресации;
		КонецЕсли;	
...
//
Процедура ЗаполнитьСтрокуКонтроляПоЗадаче(Задача) Экспорт 
	...
	Для Каждого КонтрольСсылка Из КарточкиКонтроляПоПредмету Цикл
		...
		Если НайденнаяСтрока = Неопределено Тогда 
			Для Каждого Строка Из КонтрольОбъект.Исполнители Цикл
				...
				Если ТипЗнч(Строка.Исполнитель) = Тип("СправочникСсылка.Пользователи") И Строка.Исполнитель = Задача.Исполнитель Тогда 
					...
					Прервать;
				ИначеЕсли ТипЗнч(Строка.Исполнитель) = Тип("СправочникСсылка.РолиИсполнителей") 
					И Строка.Исполнитель = Задача.РольИсполнителя 
					И Строка.ОсновнойОбъектАдресации = Задача.ОсновнойОбъектАдресации
					И Строка.ДополнительныйОбъектАдресации = Задача.ДополнительныйОбъектАдресации Тогда
					НайденнаяСтрока = Строка;
					Прервать;
				КонецЕсли;
		...
		Если НайденнаяСтрока = Неопределено Тогда 
			Если ТипЗнч(КонтрольОбъект.Контролер) = Тип("СправочникСсылка.Пользователи") И КонтрольОбъект.Контролер = Задача.Исполнитель Тогда 
				Продолжить;
			ИначеЕсли ТипЗнч(КонтрольОбъект.Контролер) = Тип("СправочникСсылка.РолиИсполнителей")	
				И КонтрольОбъект.Контролер = Задача.РольИсполнителя 
				И КонтрольОбъект.ОсновнойОбъектАдресацииКонтролера = Задача.ОсновнойОбъектАдресации
				И КонтрольОбъект.ДополнительныйОбъектАдресацииКонтролера = Задача.ДополнительныйОбъектАдресации Тогда
				Продолжить;
			КонецЕсли;
		...
		Иначе
			...
			НайденнаяСтрока.ОсновнойОбъектАдресации = Задача.ОсновнойОбъектАдресации;
			НайденнаяСтрока.ДополнительныйОбъектАдресации = Задача.ДополнительныйОбъектАдресации;
		КонецЕсли;	
...
Процедура ЗаполнитьКонтрольПоЗадаче(Задача) Экспорт 
	...
	Для Каждого КонтрольСсылка Из КарточкиКонтроляПоПредмету Цикл
		...
		Иначе
			НайденнаяСтрока = КонтрольОбъект.Добавить();
		КонецЕсли;	
...
//
Процедура ОтметитьУдалениеСтрокиКонтроляПоЗадаче(Задача, Отметка) Экспорт 
	...
	Для Каждого КонтрольСсылка Из КарточкиКонтроляПоПредмету Цикл
		...
		Если Отметка Тогда 
			...
			КонецЕсли;
		Иначе
			СоздатьСтрокуКонтроляПоЗадаче(Задача);
		КонецЕсли;	
...
// Параметры:
//  Предмет - ссылка на предмет справочника Контроль - проверяемый предмет
//  СтруктураИсполнитель - Структура - структура с ключами Исполнитель, ОсновнойОбъектАдресации,
...
Процедура УстановитьСвойстваКнопкиКонтроляПисьма(Форма) Экспорт 
	...
	ЭлементКонтроль = Форма.Элементы.ДекорацияКонтроль;
	ТекущаяДата = НачалоДня(ТекущаяДатаСеанса());
	
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьКонтрольОбъектов") Тогда 
		ЭлементКонтроль.Видимость = Ложь;
		Возврат;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ВеткаПереписки) Тогда 
		ЭлементКонтроль.Картинка = БиблиотекаКартинок.КартинкаНеНаКонтроле24;
...
Процедура ЗаполнитьСписокКонтролеров(Контролеры, ЭлементКонтролер, ЭлементКонтролерБезПользовательскойВидимости) Экспорт 
	...
	Если (Контролеры.Количество() = 0) Или 
		(ЕстьПодчиненные И Не ЕстьПроделегированные И Не ЕстьРоли) Тогда 
...
Процедура НайтиСтрокуКонтроляПоАдресату(КарточкаКонтроля, Адресат, ВозврНайденнаяСтрока) Экспорт 
	...
	АдресАдресата = ОбщегоНазначения.ПолучитьЗначениеРеквизита(Адресат, "Адрес");
	...
	Для Каждого Строка Из КарточкаКонтроля.Исполнители Цикл
		Если ТипЗнч(Строка.Исполнитель) = Тип("СправочникСсылка.АдресатыПочтовыхСообщений") Тогда 
			АдресИсполнителя = ОбщегоНазначения.ПолучитьЗначениеРеквизита(Строка.Исполнитель, "Адрес");
	...
	Для Каждого Строка Из КарточкаКонтроля.Исполнители Цикл
		Если ТипЗнч(СтруктураИсполнитель.Исполнитель) = Тип("СправочникСсылка.Пользователи") Тогда 
			Если Строка.Исполнитель = СтруктураИсполнитель.Исполнитель Тогда 
				...
				Прервать;
			КонецЕсли;	
		ИначеЕсли ТипЗнч(СтруктураИсполнитель.Исполнитель) = Тип("СправочникСсылка.РолиИсполнителей") Тогда 
			Если ТипЗнч(Строка.Исполнитель) = Тип("СправочникСсылка.РолиИсполнителей") Тогда 
				Если Строка.Исполнитель = СтруктураИсполнитель.Исполнитель
		   			И Строка.ОсновнойОбъектАдресации = СтруктураИсполнитель.ОсновнойОбъектАдресации
		   			И Строка.ДополнительныйОбъектАдресации = СтруктураИсполнитель.ДополнительныйОбъектАдресации
				Тогда 
					ВозврНайденнаяСтрока = Строка;
					Прервать;
				КонецЕсли;
			ИначеЕсли ЗначениеЗаполнено(Строка.Источник) И ТипЗнч(Строка.Источник) = Тип("ЗадачаСсылка.ЗадачаИсполнителя") Тогда 
				ДанныеИсточника = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(Строка.Источник, 
					"РольИсполнителя, ОсновнойОбъектАдресации, ДополнительныйОбъектАдресации");
				Если ДанныеИсточника.РольИсполнителя = СтруктураИсполнитель.Исполнитель
		   			И ДанныеИсточника.ОсновнойОбъектАдресации = СтруктураИсполнитель.ОсновнойОбъектАдресации
		   			И ДанныеИсточника.ДополнительныйОбъектАдресации = СтруктураИсполнитель.ДополнительныйОбъектАдресации
				Тогда 
					ВозврНайденнаяСтрока = Строка;
					Прервать;
				КонецЕсли;
...
Процедура ПриЗаписиПроцессаСКонтролером(Объект) Экспорт 
	...
	ОсновнойОбъектАдресацииКонтролера = Объект.ОсновнойОбъектАдресацииКонтролера;
	ДополнительныйОбъектАдресацииКонтролера = Объект.ДополнительныйОбъектАдресацииКонтролера;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьКонтрольОбъектов") И Стартован Тогда 
		...
		ИначеЕсли ЗначениеЗаполнено(ДополнительныеСвойства.СтарыйКонтролер) И Не ЗначениеЗаполнено(Контролер) Тогда 	
			...
			КарточкаКонтроля = Контроль.НайтиКарточкуКонтроляПоПредметуИКонтролеру(Ссылка,
				...
				ДополнительныеСвойства.СтарыйОсновнойОбъектАдресацииКонтролера, 
				ДополнительныеСвойства.СтарыйДополнительныйОбъектАдресацииКонтролера,
				ПометкаУдаления);
		...
		ИначеЕсли ДополнительныеСвойства.СтарыйКонтролер <> Контролер Тогда 	
			...
			Если ЗначениеЗаполнено(КарточкаКонтроля) Тогда 
				...
				КарточкаКонтроляОбъект.ОсновнойОбъектАдресацииКонтролера = ОсновнойОбъектАдресацииКонтролера;
				КарточкаКонтроляОбъект.ОсновнойОбъектАдресацииКонтролера = ДополнительныйОбъектАдресацииКонтролера;
