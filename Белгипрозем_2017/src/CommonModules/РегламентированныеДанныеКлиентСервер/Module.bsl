...
//1С-Минск
    Возврат ИННСоответствуетТребованиямРБ(ИНН, ЭтоЮрЛицо, ТекстСообщения);    
//Конец 1С-Минск  
...
Функция КодПоОКПОСоответствуетТребованиям(Знач ПроверяемыйКод, ЭтоЮрЛицо, ТекстСообщения = "", ОтбрасыватьВедущиеНули = Ложь) Экспорт
	...
	Если ЭтоЮрЛицо И ДлинаКода <> 8 И ДлинаКода <> 12 Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ru = 'Код ОКПО юридического лица должен состоять из 8 цифр  или 12 цифр.'") + Символы.ПС;
...
// 
Функция КодПоОКПОСоответствуетТребованиямРБ(КодПоОКПО, ЭтоЮрЛицо, ТекстСообщения = "", ОтбрасыватьВедущиеНули = Ложь) Экспорт
	...
	Если  ЭтоЮрЛицо И ДлинаКодаПоОКПО <> 8 И ДлинаКодаПоОКПО <> 12 Тогда
		СоответствуетТребованиям = Ложь;
		ТекстСообщения = ТекстСообщения + ?(ЗначениеЗаполнено(ТекстСообщения), Символы.ПС, "")
		+ НСтр("ru = 'Код по ОКПО юридического лица должен состоять из 8 или 12 цифр.'");
...
//  Ложь         - ИНН не соответствует требованиям.
//
Функция ИННСоответствуетТребованиямРБ(Знач ИНН, ЭтоЮрЛицо, ТекстСообщения) Экспорт
	...
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(УНП) Тогда
		СоответствуетТребованиям = Ложь;
		ТекстСообщения = ТекстСообщения + НСтр("ru = 'УНП должен состоять только из цифр.'");
    ИначеЕсли СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(УНП) и НЕ ЭтоЮрЛицо  Тогда 
		СоответствуетТребованиям = Ложь;
		ТекстСообщения = ТекстСообщения + НСтр("ru = 'УНП должен включать буквы для физ. лиц.'");
	КонецЕсли;

	Если  ЭтоЮрЛицо  И ДлинаИНН <> 9 Тогда
		СоответствуетТребованиям = Ложь;
		ТекстСообщения = ТекстСообщения + ?(ЗначениеЗаполнено(ТекстСообщения), Символы.ПС, "")
		               + НСтр("ru = 'УНП юридического лица должен состоять из 9 цифр.'");
	КонецЕсли;
    
    Если СоответствуетТребованиям  и ЭтоЮрЛицо Тогда
        ...
        Если КонтрольныйРазряд <> Число(Сред(УНП,9,1))или КонтрольныйРазряд = 10 Тогда
            СоответствуетТребованиям = Ложь;
            ТекстСообщения = ТекстСообщения + ?(ЗначениеЗаполнено(ТекстСообщения), Символы.ПС, "")
            + НСтр("ru = 'Контрольное число для УНП не совпадает с рассчитанным.'");
          //Возврат Ложь;
        КонецЕсли; 
        
    ИначеЕсли СоответствуетТребованиям и НЕ ЭтоЮрЛицо Тогда
