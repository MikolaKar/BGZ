...
Процедура ПослеНачалаРаботыСистемы() Экспорт
	...
	Если Не ПараметрыРаботыКлиента.ОткрытьПомощникСозданияОбменаДаннымиДляНастройкиПодчиненногоУзла
		И ПараметрыРаботыКлиента.ПроверитьНеобходимостьОбновленияКонфигурацииПодчиненногоУзла Тогда
...
Процедура ОбработчикВыбораФайловогоКаталога(Объект, Знач ИмяСвойства, СтандартнаяОбработка = Ложь, Знач ПараметрыДиалога = Неопределено, ОповещениеЗавершения = Неопределено) Экспорт
	...
	ТекстПредупреждения = НСтр("ru = 'Для данной операции необходимо установить расширение работы с файлами.'");
...
Процедура ПередатьФайлыНаСервер(ОповещениеЗавершения, Знач ИменаФайлов, Знач ИдентификаторФормы = Неопределено, Знач ТекстПредупреждения = Неопределено) Экспорт
	...
	Если ТекстПредупреждения = Неопределено Тогда
		Если ИменаФайлов.Количество() = 1 Тогда
			ТекстПредупреждения = НСтр("ru = 'Для передачи файла на сервер необходимо установить расширение работы с файлами.'");
		Иначе
			ТекстПредупреждения = НСтр("ru = 'Для передачи файлов на сервер необходимо установить расширение работы с файлами.'");
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого Элемент Из ДанныеФайлов Цикл
		...
		Если Не Файл.Существует() Или Файл.ЭтоКаталог() Тогда
			...
			Элемент.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Файл ""%1"" не существует или к нему нет доступа'"), ИмяФайла);
		Иначе
