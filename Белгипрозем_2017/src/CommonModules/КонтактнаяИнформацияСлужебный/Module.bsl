...
//
//  Параметры:
//      Текст                      - Строка - строка автоподбора
...
Функция ПредставлениеАдреса(XDTOАдрес, ВидИнформации) Экспорт
    ...
    //{{1С-Минск
    Возврат ПредставлениеАдресаРБ(XDTOАдрес, ВидИнформации);    
    //}}1C-Минск	
...
//     РезультатГруппами - Булево - если указана Истина, то будет возвращее массив групп ошибок, иначе - список значений.
//
// Возвращаемое значение:
//     Массив, СписокЗначений - в зависимости от параметра РезультатГруппами:
//         Если параметр РезультатГруппами равен Ложь, то возвращается CписокЗначений в котором 
//            ** Представление - текст ошибки, 
//            ** Значение      - XPath ошибочного поля
//         Если параметр РезультатГруппами равен Истина, то возвращается массив, содержащий структуры с полями:
...
//1С-Минск
	НазваниеРоссии  = ВРег(Справочники.СтраныМира.Беларусь.Наименование);
//1С-Минск
	...
	Для Каждого ЭлементСписка Из СписокПолей Цикл
		...
		ИначеЕсли ИмяПоля = "РАЙОН" Тогда
			...
			КонецЕсли;
            //{{1С-Минск
			АдресРФ.СвРайМО.Район = ЭлементСписка.Значение + " р-н";
            //}}1C-Минск
		...
		ИначеЕсли ИмяПоля = "НАСЕЛЕННЫЙПУНКТ" Тогда
			...
			АдресРФ.НаселПункт = Строка(АдресРФ.НаселПункт) + ЭлементСписка.Значение;;
			//АдресРФ.НаселПункт = АдресРФ.НаселПункт + ЭлементСписка.Значение;;
            //}}1C-Минск
			
		ИначеЕсли ИмяПоля = "УЛИЦА" Тогда
            //{{1С-Минск
			//АдресРФ.Улица = ЭлементСписка.Значение;
			АдресРФ.Улица = Строка(АдресРФ.Улица) + ЭлементСписка.Значение;;
            //}}1C-Минск
		...
		ИначеЕсли ИмяПоля="ОБЛАСТЬ" Тогда
            АдресРФ.СубъектРФ = ЭлементСписка.Значение + " обл."; 
		ИначеЕсли ИмяПоля="СЕЛЬСОВЕТ" Тогда
			АдресРФ.Город = ЭлементСписка.Значение + " с/с";
        ИначеЕсли ИмяПоля="ТИПНАСЕЛЕННОГОПУНКТА" Тогда
            АдресРФ.НаселПункт = СокрЛП(ЭлементСписка.Значение) + " " + АдресРФ.НаселПункт;
        ИначеЕсли ИмяПоля="НАИМЕНОВАНИЕТИПАУЛИЦЫ" Тогда
            АдресРФ.Улица = СокрЛП(ЭлементСписка.Значение) + " " + АдресРФ.Улица;
        //}}1C-Минск	
		КонецЕсли;
...
Функция ДесериализацияАдресаПоПредставлениюКЛАДР(Знач Представление)
	...
	МодульКЛАДР = СерверныйМодульКЛАДР();
	Запрос = МодульКЛАДР.ЗапросДесериализацияАдресаПоПредставлениюКЛАДР("Справочник.СтраныМира");
...
//1С-Минск
Функция ПредставлениеАдресаРБ(XDTOАдрес, ВидИнформации) Экспорт
    ...
    // Если страна БЕЛАРУСЬ, то не печатаем, иначе добавляем страну в адрес
    Если Найти(ВРЕГ(XDTOАдрес.Страна), "БЕЛАРУСЬ")>0 Тогда
    	Страна = ?(ВидИнформации.ВключатьСтрануВПредставление, СокрЛП(XDTOАдрес.Страна), "");
    Иначе 
    	Страна = СокрЛП(XDTOАдрес.Страна);
    КонецЕсли; 
    ...
    Если ТипЗнч(АдресРФ)<>Тип("ОбъектXDTO") Тогда
		...
		Возврат ?(ПустаяСтрока(Страна), "", Страна + ", ") + СокрЛП(АдресРФ);
	КонецЕсли;
	
	ПространствоИмен = КонтактнаяИнформацияКлиентСерверПовтИсп.ПространствоИмен();
	Если АдресРФ.Тип()=ФабрикаXDTO.Тип(ПространствоИмен, "АдресРФ") Тогда
		...
		// Область, Сельсовет в адресе не отображаем с 21.08.17
		ВидКИ = ВидИнформации.ЗначениеВидаКонтактнойИнформации;
		
		Если ВидКИ = Справочники.ВидыКонтактнойИнформации.АдресЗемельногоУчастка
			ИЛИ ВидКИ = Справочники.ВидыКонтактнойИнформации.АдресЗемельногоУчасткаОбъектаРабот Тогда
			// с указанием сельсовета
			Область = ?(АдресРФ.СубъектРФ = "Минск", "", АдресРФ.СубъектРФ);
		...
		//// сбрасываем фиктивный регион, что бы не выводился в представлении
		//Область = ?(АдресРФ.СубъектРФ = "Минск", "", АдресРФ.СубъектРФ);
		//
		//ЭтоГород = ?(Найти(АдресРФ.НаселПункт, "г.")>0, Истина, Ложь);
		//Если ЭтоГород Тогда
		//    Представление = КонтактнаяИнформацияКлиентСервер.ПолноеНаименование(
		//    Представление, "",
		//    ПочтовыйИндексАдреса(АдресРФ), "",
