	...
	Для Каждого Предмет Из Предметы Цикл
		...
		Если Предмет.Свойство("Организация")
			...
			Если НЕ Предмет.Организация.Пустая() Тогда
				Запрос.Текст = Запрос.Текст + "
					|	И (
					|		Инструкции.Организация = &Организация" + ИндексПредмета + "
					|		ИЛИ Инструкции.Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
					|	)";
				Запрос.УстановитьПараметр("Организация" + ИндексПредмета, Предмет.Организация);
			Иначе
				Запрос.Текст = Запрос.Текст + "
					|	И Инструкции.Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)";
			КонецЕсли;
...
Функция ТекстыИнструкций(Инструкции, УстановитьСтильОформления = Истина) Экспорт
	...
	Если МассивТекстовИнструкций.Количество() > 0 Тогда
		...
		ИначеЕсли МассивТекстовИнструкций.Количество() > 1 Тогда
			Ссылки = СтроковыеФункцииКлиентСервер.ПолучитьСтрокуИзМассиваПодстрок(МассивСсылокНаИнструкции,
				"");
			...
			СтрокаВНачало = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				НСтр("ru = 'В начало'"));
			Результат = СтроковыеФункцииКлиентСервер.ПолучитьСтрокуИзМассиваПодстрок(
				МассивТекстовИнструкций,
				СтрокаВНачало)
				+ СтрокаВНачало;
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
