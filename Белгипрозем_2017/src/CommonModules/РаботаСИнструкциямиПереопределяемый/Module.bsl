...
Функция СоответствияФормИПредметовИнструкций() Экспорт
	...
	Соответствие.Вставить("БизнесПроцесс.Исполнение.Форма.ФормаЗадачиОтвИсполнителя", Справочники.ПредметыИнструкций.ЗадачиПроцессаИсполненияЗадачаОтвественногоИсполнителя);
	...
	Соответствие.Вставить("БизнесПроцесс.Поручение.Форма.ФормаВнешнейЗадачиИсполнителя", Справочники.ПредметыИнструкций.ЗадачиПроцессаПоручениеВнешняяЗадачаИсполнителя);
...
//1С-Минск	
	Соответствие.Вставить("Справочник.мЭтапыДоговоров.Форма.ФормаЭлемента", Справочники.ПредметыИнструкций.ЭтапыДоговоров);
//1С-Минск
Возврат Соответствие;
	
КонецФункции

// Формирует массив описаний предметов инструкций.
//
Процедура ПредметыИнструкцийСоответствующиеФорме(Предметы, ЭтаФорма) Экспорт
	...
	Если Предмет = ПредметыИнструкций.ВнутренниеДокументы
		...
		ЗаполнитьХарактеристикиПредметаПоРеквизитамДокумента(ОписаниеПредмета, ЭтаФорма.Объект, ЭтаФорма);
	...
	ИначеЕсли Предмет = ПредметыИнструкций.ЗадачиПроцессаИсполненияЗадачаИсполнителя
		...
		ИЛИ Предмет = ПредметыИнструкций.ЗадачиПроцессаИсполненияЗадачаОтвественногоИсполнителя
		...
		ИЛИ Предмет = ПредметыИнструкций.ЗадачиПроцессаПоручениеВнешняяЗадачаИсполнителя
...
//1С-Минск		
	ИначеЕсли Предмет = ПредметыИнструкций.ЭтапыДоговоров Тогда
		
		ОписаниеПредмета = РаботаСИнструкциями.ОписаниеПредметаИнструкции(Предмет);
		Предметы.Добавить(ОписаниеПредмета);
		
		ОписаниеПредмета = РаботаСИнструкциями.ОписаниеПредметаИнструкции(ПредметыИнструкций.Белгипрозем);
		Предметы.Добавить(ОписаниеПредмета);
//1С-Минск		
...
//  ЭтаФорма         -
//
Процедура ЗаполнитьХарактеристикиПредметаПоРеквизитамДокумента(ОписаниеПредмета, Документ, ЭтаФорма = Неопределено) Экспорт
...
Процедура ЗаполнитьПредметыИнструкцийПоПредметамЗадачи(Предметы, ПредметыЗадачи) Экспорт
	...
	Для Каждого ТекСтрока ИЗ ПредметыЗадачи Цикл
		...
		ПраваНаПредмет = ДокументооборотПраваДоступа.ПолучитьПраваПоОбъекту(ТекСтрока.Предмет);
		Если Не ПраваНаПредмет.Чтение Тогда
			Продолжить;
		КонецЕсли;
		
		Если ТипЗнч(ТекСтрока.Предмет) = Тип("СправочникСсылка.ВнутренниеДокументы") Тогда
			
			ОписаниеПредмета = РаботаСИнструкциями.ОписаниеПредметаИнструкции(
				Справочники.ПредметыИнструкций.ВнутренниеДокументы);
			ЗаполнитьХарактеристикиПредметаПоРеквизитамДокумента(ОписаниеПредмета, ТекСтрока.Предмет);
			Предметы.Добавить(ОписаниеПредмета);
			
		ИначеЕсли ТипЗнч(ТекСтрока.Предмет) = Тип("СправочникСсылка.ВходящиеДокументы") Тогда
		...
		ИначеЕсли ТипЗнч(ТекСтрока.Предмет) = Тип("СправочникСсылка.ИсходящиеДокументы") Тогда
