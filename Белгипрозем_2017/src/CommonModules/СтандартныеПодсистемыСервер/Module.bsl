...
Процедура ПриНастройкеВариантовОтчетов(Настройки) Экспорт
	...
	Отчет = МодульВариантыОтчетов.ОписаниеОтчета(Настройки, Метаданные.Отчеты.МестаИспользованияСсылок);
	
	МодульВариантыОтчетов.УстановитьРежимВыводаВПанеляхОтчетов(Настройки, Отчет, Ложь);
	
	Вариант = МодульВариантыОтчетов.ОписаниеВарианта(Настройки, Отчет, "Основной");
	Вариант.Описание = НСтр("ru = 'Поиск мест использования объектов приложения.'");
...
Процедура ОбновитьПараметрыАдминистрированияИнформационнойБазы() Экспорт
	...
	Если СтароеЗначениеПараметров <> Неопределено Тогда
		...
		Если СтароеЗначениеПараметров.Свойство("ПортАгентаСервера") И
				ЗначениеЗаполнено(СтароеЗначениеПараметров.ПортАгентаСервера) Тогда
			НовоеЗначениеПараметров.ПортАгентаСервера = СтароеЗначениеПараметров.ПортАгентаСервера;
		КонецЕсли;
		Если СтароеЗначениеПараметров.Свойство("ПортКластераСерверов") И
				ЗначениеЗаполнено(СтароеЗначениеПараметров.ПортКластераСерверов) Тогда
			НовоеЗначениеПараметров.ПортКластера = СтароеЗначениеПараметров.ПортКластераСерверов;
		КонецЕсли;
		Если СтароеЗначениеПараметров.Свойство("ИмяАдминистратораКластера") И
				Не ПустаяСтрока(СтароеЗначениеПараметров.ИмяАдминистратораКластера) Тогда
			НовоеЗначениеПараметров.ИмяАдминистратораКластера = СтароеЗначениеПараметров.ИмяАдминистратораКластера;
		КонецЕсли;
		Если СтароеЗначениеПараметров.Свойство("ИмяАдминистратораИБ") И
				Не ПустаяСтрока(СтароеЗначениеПараметров.ИмяАдминистратораИБ) Тогда
...
Процедура ПередЗапускомПрограммы()
	...
	ОбщегоНазначенияКлиентСервер.Проверить(ОбщегоНазначенияКлиентСервер.СравнитьВерсии(
		СистемнаяИнформация.ВерсияПриложения, "8.3.5.1517") >= 0,
		НСтр("ru = 'Для запуска необходима версия платформы 1С:Предприятие 8.3.5.1517 или выше.'"));
