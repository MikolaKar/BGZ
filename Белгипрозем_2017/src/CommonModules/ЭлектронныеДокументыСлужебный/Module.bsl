...
Процедура ПередЗаписьюЭлектронногоДокумента(Источник, Отказ) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Источник.НаправлениеЭД)
		...
		ИЛИ НЕ ЗначениеЗаполнено(Источник.Документ)
		ИЛИ ТипЗнч(Источник.Документ) = Тип("ДокументСсылка.ПроизвольныйЭД") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПриЗаписиЭлектронногоДокумента(Источник, Отказ) Экспорт
	...
	Если Источник.ДополнительныеСвойства.Свойство("СостояниеВерсииЭД") Тогда
		...
		СтруктураТекВерсииЭД  = ПолучитьСтруктуруВерсииЭД(Источник.Документ);
...
// Получает выборку элементов справочника СообщенияЭДО по отбору
//
// Параметры:
...
Функция ПолучитьВыборкуЭДПоОтбору(Параметры) Экспорт
	...
	Для Каждого Элемент Из Параметры Цикл
		Запрос.Текст = Запрос.Текст + ?(НЕ ПервыйЭлемент," И ","") + " СообщенияЭДО." + Элемент.Ключ + "=&" + Элемент.Ключ;
		ПервыйЭлемент = Ложь;
...
Процедура РегламентноеПолучениеНовыхЭД() Экспорт
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Получение новых эл.документов'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,,,
		НСтр("ru = 'Начато регламентное задание по получению новых эл.документов.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	...
	Исключение
		...
		ОписаниеОшибки =
			НСтр("ru = 'Во время регламентного получения новых эл.документов произошла ошибка.
...
Процедура РегламентнаяОтправкаОформленныхЭД() Экспорт
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Отправка оформленных эл.документов'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,,,
	...
	Исключение
		ОписаниеОшибки =
			НСтр("ru = 'Во время регламентной отправки оформленных эл.документов произошла ошибка.
                  |Дополнительное описание:
                  |%ДополнительноеОписание%'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ДополнительноеОписание%", ИнформацияОбОшибке().Описание);
...
Процедура СохранитьВместеСПоследнейЭП(ПрисоединенныйФайл,
	...
	МассивСтруктурПодписей = ЭлектроннаяПодпись.ПолучитьВсеПодписи(ФайлЭД, УникальныйИдентификатор);
...
Процедура СохранитьВместеСЭП(ПрисоединенныйФайл,
	...
	УникальныйИдентификатор = Новый УникальныйИдентификатор;
	МассивСтруктурПодписей = ЭлектроннаяПодпись.ПолучитьВсеПодписи(ФайлЭД.ТекущаяВерсия, УникальныйИдентификатор);
	Если ТипЗнч(МассивСтруктурПодписей) = Тип("Массив") И МассивСтруктурПодписей.Количество() > 0 Тогда
...
Функция ЕстьТакаяПодпись(ДвоичныеДанныеПроверки, ЭлектронныйДокумент) Экспорт
	...
	Пока ПодписиФайлаЭД.Следующий() Цикл
		ДвоичныеДанныеПодписи = ПодписиФайлаЭД.Подпись.Получить();
...
Функция ТаблицаДоступныхДляПодписиСертификатов(МассивСтруктурСертификатов, СоглашениеЭД = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(СоглашениеЭД)
		...
		|ГДЕ
		|	СоглашенияЭДСертификаты.Ссылка = &СоглашениеОбИспользованииЭД";
	...
	ТекстЗапроса = ТекстЗапроса + "
		|	И (СертификатыЭП.Пользователь = &ТекущийПользователь
		|			ИЛИ СертификатыЭП.Пользователь = &ПустойПользователь)
		|	И (СертификатыЭП.Отпечаток В (&МассивОтпечатков)
		|			ИЛИ НЕ СертификатыЭП.Идентификатор = """")";
...
Функция ПолучитьСтруктуруВерсииЭД(СсылкаНаВладельца) Экспорт
	...
	|	ЕСТЬNULL(СостоянияЭД.ЭлектронныйДокумент, ЗНАЧЕНИЕ(Справочник.СообщенияЭДО.ПустаяСсылка)) КАК ЭлектронныйДокумент,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	МАКСИМУМ(СообщенияЭДО.НомерВерсииЭД) КАК НомерВерсииЭД
	|ПОМЕСТИТЬ МаксВерсия
	|ИЗ
	|	Справочник.СообщенияЭДО КАК СообщенияЭДО
	|ГДЕ
	|	СообщенияЭДО.Документ = &СсылкаНаОбъект
	...
	|ВЫБРАТЬ
	|	СообщенияЭДО.Ссылка,
	|	СообщенияЭДО.НомерВерсииЭД,
	|	СообщенияЭДО.СтатусЭД
	|ИЗ
	|	Справочник.СообщенияЭДО КАК СообщенияЭДО
	|ГДЕ
	|	СообщенияЭДО.Документ = &СсылкаНаОбъект
	|	И СообщенияЭДО.НомерВерсииЭД В
	|			(ВЫБРАТЬ
	...
	Иначе
		...
		СсылкаНаДокумент  = Справочники.СообщенияЭДО.ПустаяСсылка();
...
Функция СоздатьДокументыПакетовЭД(МассивЭД, ПризнакПодписи) Экспорт
	...
	Для Каждого ЭД Из МассивЭД Цикл
		...
		Если РеквизитыСоглашения.СпособОбменаЭД = Перечисления.СпособыОбменаЭД.ЧерезВебРесурсБанка
			И РеквизитыСоглашения.ПрограммаБанка = Перечисления.ПрограммыБанка.iBank2 Тогда
		...
		ПараметрыСоглашения = ОпределитьНастройкиОбменаЭДПоИсточнику(ЭД.Документ, , , ЭД);
		...
		Попытка
			...
			НоваяСтрока.ОбъектВладелец      = ЭД.Документ;
	...
	Для Каждого ТекСтрока Из ТЗ_ПЭД Цикл
		...
		ИначеЕсли ТекСтрока.Соглашение.СпособОбменаЭД = Перечисления.СпособыОбменаЭД.ЧерезВебРесурсБанка Тогда
			Если ТекСтрока.Соглашение.ПрограммаБанка = Перечисления.ПрограммыБанка.iBank2 Тогда
...
Процедура ПоместитьПакетЭДВКонверт(Конверт, АдресЗипКонтейнера) Экспорт
	...
	Выборка = ПолучитьВыборкуЭДПоОтбору(Новый Структура("Документ", Конверт));
...
Функция ОпределитьНастройкиОбменаЭДПоИсточнику(
	...
	Если ЗначениеЗаполнено(ЭД) Тогда
		...
		Если ЗначениеЗаполнено(ФайлЭД) Тогда
			...
			ПодписиФайлаЭД = РаботаСЭП.ПолучитьЭлектронныеПодписи(ФайлЭД.ТекущаяВерсия);
			Пока ПодписиФайлаЭД.Следующий() Цикл
				МассивОтпечатков.Добавить(ПодписиФайлаЭД.Отпечаток);
			КонецЦикла;
				
			ПараметрыЭД.Вставить("УстановленныеПодписи", МассивОтпечатков);
		КонецЕсли;
...
Функция ПолучитьНастройкиОбменаЭДПоИД(ИДОрганизации, ИДКонтрагента, ТолькоСОператоромЭДО = Ложь) Экспорт
	...
	|	НастройкиОбменаЭД.СертификатОрганизацииДляРасшифровки,
	|	ВЫБОР
	...
	Для Каждого ЭлементСоответствие Из СоответствиеФайлПараметры Цикл
		Если Найти(ЭлементСоответствие.Ключ, ".p7s") > 0 Тогда
			...
			Если СертификатыПодписи.Количество() <> 0 Тогда
				...
				ПредставлениеПользователя = ЭлектроннаяПодписьКлиентСервер.ПолучитьПредставлениеПользователя(Сертификат.Субъект);
				ФайлЭД = ЭлектронныеДокументы.ПолучитьПодчиненныйФайл(ЭлектронныйДокумент);
				Если Не ЗначениеЗаполнено(ФайлЭД) Тогда
					Продолжить;
				КонецЕсли;
			...
			СтрокаПакетаЭД.ОбъектВладелец = ЭлектронныйДокумент.Документ;
			МассивВозврата.Добавить(ЭлектронныйДокумент.Документ);
		КонецЕсли;
	КонецЦикла;
...
Функция ОпределитьСостояниеВерсииПоСтатусуЭД(СсылкаНаЭД) Экспорт
	...
	Если СсылкаНаЭД = Неопределено Или СсылкаНаЭД = Справочники.СообщенияЭДО.ПустаяСсылка() Тогда
		...
		ИначеЕсли НастройкиОбмена <> Неопределено Тогда
			...
			Если МассивСтатусов.Количество() > 0 Тогда
				...
				ИначеЕсли ИндексТекущегоСтатуса + 1 = МассивСтатусов.Количество() Тогда
					Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СсылкаНаЭД, "ВидЭД") = Перечисления.ВидыЭД.ПлатежноеПоручение Тогда
				...
				Иначе
					...
					ИначеЕсли СледующийСтатус = Перечисления.СтатусыЭД.Доставлен
						...
						Если НастройкиОбмена.ВидЭД = Перечисления.ВидыЭД.ПлатежноеПоручение
								И НастройкиОбмена.ПрограммаБанка = Перечисления.ПрограммыБанка.iBank2 Тогда
...
Функция УстановленныеПодписиВалидны(ЭД) Экспорт
	...
	Если Не ЗначениеЗаполнено(ФайлЭД) Тогда
		...
		|Документ не обработан, так как не содержит подчиненные файлы.'");
...
Функция ОпределитьОбъектПривязки(СтруктураПараметровДокумента) Экспорт
	...
	ИначеЕсли СтруктураПараметровДокумента.ВидЭД = Перечисления.ВидыЭД.ТОРГ12Покупатель
		...
		ЭДВладелецСсылка = Справочники.СообщенияЭДО.ПолучитьСсылку(
		...
		Если ЭДВладелецСсылка.ПолучитьОбъект() <> Неопределено Тогда
			...
			ВозвращаемоеЗначение = ЭДВладелецСсылка.Документ;
		Иначе
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ЭлектронныйНомерДокумента) Тогда
			...
			ЗапросДокументовОснований.Текст =
				"ВЫБРАТЬ ПЕРВЫЕ 1
				|	СообщенияЭДО.Документ.Ссылка КАК Ссылка
				|ИЗ
				|	Справочник.СообщенияЭДО КАК СообщенияЭДО
				|ГДЕ
				|	(НЕ СообщенияЭДО.Документ ССЫЛКА Документ.ПакетЭД)
				|	И СообщенияЭДО.ПометкаУдаления = ЛОЖЬ
				|	И СообщенияЭДО.НомерЭД ПОДОБНО &Параметр
				|	И СообщенияЭДО.ВидЭД = &ВидЭД
				|	И СообщенияЭДО.НаправлениеЭД = &НаправлениеЭД";
...
Функция ПолучитьНовыеЭД(СоотвСоглашенийИСтруктурСертификатов = Неопределено, ПовторноеПолучение = Ложь) Экспорт
	...
	Для Каждого СтрокаРесурсовЭП Из МассивРесурсовЭП Цикл
		...
		Попытка
			НаборСообщений = РаботаСПочтовымиСообщениямиСлужебный.ЗагрузитьПочтовыеСообщения(УчетнаяЗаписьЭП);
		Исключение
			ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		...
		Для Каждого Сообщение Из НаборСообщений Цикл
			...
			Для Каждого ЭлементВложенийПисьма Из Сообщение.Вложения Цикл
				...
				ПрисоединенныйФайл = ЭлектронныеДокументы.ДобавитьФайл(ПакетЭД.Ссылка, Лев(ЭлементВложенийПисьма.Ключ,
					СтрДлина(ЭлементВложенийПисьма.Ключ) -4), Прав(ЭлементВложенийПисьма.Ключ, 3), ТекущаяДатаСеанса(),
					ТекущаяДатаСеанса(), АдресВХранилище, , , Справочники.СообщенияЭДО.ПолучитьСсылку());
	...
	Для Каждого СтрокаРесурсовКаталог Из МассивРесурсовКаталоги Цикл
		...
		Для Каждого ЭлементФайл Из МассивФайлов Цикл
			...
			АдресВХранилище = ПоместитьВоВременноеХранилище(ДвоичныеДанныеЭлемента);
			ПрисоединенныйФайл = ЭлектронныеДокументы.ДобавитьФайл(ПакетЭД.Ссылка, Лев(ЭлементФайл.Имя,
				СтрДлина(ЭлементФайл.Имя) -4), Прав(ЭлементФайл.Имя, 3), ТекущаяДатаСеанса(), ТекущаяДатаСеанса(),
				АдресВХранилище, , , Справочники.СообщенияЭДО.ПолучитьСсылку());
	...
	Для Каждого СтрокаРесурсовFTP Из МассивРесурсовFTP Цикл
		...
		Для Каждого ФайлFTP Из МассивФайловFTP Цикл
			...
			АдресВХранилище = ПоместитьВоВременноеХранилище(ДвоичныеДанныеЭлемента);
			ПрисоединенныйФайл = ЭлектронныеДокументы.ДобавитьФайл(ПакетЭД.Ссылка,
...
// Изменяет реквизиты элемента справочника СообщенияЭДО,
//
// Параметры:
...
Процедура ИзменитьПоСсылкеПрисоединенныйФайл(
	...
	ФайлОбъектИзменен = Ложь;
	ФайлЭД = ЭлектронныеДокументы.ПолучитьПодчиненныйФайл(ФайлОбъект.Ссылка);
	Если Не ЗначениеЗаполнено(ФайлЭД) Тогда
		Возврат;
	КонецЕсли;
	ФайлЭДОбъект = ФайлЭД.ПолучитьОбъект();
	ФайлЭДОбъектИзменен = Ложь;
	ТекущаяВерсияФайлаЭДОбъект = ФайлЭД.ТекущаяВерсия.ПолучитьОбъект();
	ТекущаяВерсияФайлаЭДОбъектИзменен = Ложь;
	
	Если СтруктураЭД.Свойство("Контрагент") Тогда
		ФайлОбъект.Контрагент = СтруктураЭД.Контрагент;
		ФайлОбъектИзменен = Истина;
	КонецЕсли;
	...
	Если СтруктураЭД.Свойство("Автор") Тогда
		ФайлЭДОбъект.Автор = СтруктураЭД.Автор;
		ФайлЭДОбъектИзменен = Истина;
	КонецЕсли;
	...
	Если СтруктураЭД.Свойство("ЭлектронныйДокументВладелец") Тогда
		ФайлОбъект.Родитель = СтруктураЭД.ЭлектронныйДокументВладелец;
		ФайлОбъектИзменен = Истина;
	...
	Если СтруктураЭД.Свойство("Наименование") Тогда
		...
		ФайлОбъектИзменен = Истина;
		ФайлЭДОбъект.Наименование = СтруктураЭД.Наименование;
		ФайлЭДОбъектИзменен = Истина;
	КонецЕсли;
	
	Если СтруктураЭД.Свойство("Расширение") Тогда
		ТекущаяВерсияФайлаЭДОбъект.Расширение = СтруктураЭД.Расширение;
		ТекущаяВерсияФайлаЭДОбъектИзменен = Истина;
	КонецЕсли;
	
	Если СтруктураЭД.Свойство("ДатаМодификацииУниверсальная") Тогда
		ТекущаяВерсияФайлаЭДОбъект.ДатаМодификацииУниверсальная = СтруктураЭД.ДатаМодификацииУниверсальная;
		ТекущаяВерсияФайлаЭДОбъектИзменен = Истина;
	КонецЕсли;
	...
	Если ФайлОбъектИзменен Тогда
		ФайлОбъект.Записать();
	КонецЕсли;
	Если ФайлЭДОбъектИзменен Тогда
		ФайлЭДОбъект.Записать();
	КонецЕсли;
	Если ТекущаяВерсияФайлаЭДОбъектИзменен Тогда
		ТекущаяВерсияФайлаЭДОбъект.Записать();
	КонецЕсли;
...
// Возвращает структуру данных файла, полученную из одноимённой функции общего модуля ПрисоединенныеФайлы.
...
Функция ПолучитьДанныеФайла(Знач ПрисоединенныйФайл,
	...
	ДанныеФайла = ЭлектронныеДокументы.ПолучитьДанныеФайла(ПрисоединенныйФайл,
		ИдентификаторФормы, ПолучатьСсылкуНаДвоичныеДанные);
	Если ТипЗнч(ДанныеФайла) = Тип("Структура") Тогда
		СтруктураПараметров = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПрисоединенныйФайл, "ВидЭД, УникальныйИД, Наименование");
		Если ЗначениеЗаполнено(СтруктураПараметров.Наименование) Тогда
			ДанныеФайла.Наименование = СокрЛП(СтруктураПараметров.Наименование);
...
Процедура ЗаписатьОшибкуПоЭД(ВидОперации, ТекстОшибки) Экспорт
	...
	ШаблонСообщения = НСтр("ru = 'Выполнение операции: %1.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
// Только для внутреннего использования
Процедура ОбработатьОтветБанка(ДанныеОтвета, ЭД, НовыйЭД = Неопределено) Экспорт
...
//      ЭД - СправочникСсылка.СообщенияЭДО - электронный документ с запросом выписки,
...
//      ВыпискаБанка - СправочникСсылка.СообщенияЭДО - электронный документ с выпиской банка
//
Процедура ОтправитьЗапросВыпискиВБанк(СтруктураПараметров, АдресХранилища) Экспорт
	...
	МассивПодписей = ЭлектроннаяПодпись.ПолучитьВсеПодписи(ЭД, Новый УникальныйИдентификатор);
	...
	Попытка
		...
		Если НЕ ДанныеОтвета = Неопределено Тогда
			...
			ОбработатьОтветБанка(ДанныеОтвета, ЭД, ВыпискаБанка);
		КонецЕсли;
	...
	Исключение
		...
		ВидОперации    = НСтр("ru = 'Отправка ЭД в банк'");
		ЭлектронныеДокументыСлужебныйВызовСервера.ОбработатьИсключениеПоЭДНаСервере(ВидОперации,
																					ТекстОшибки,
																					ТекстСообщения);
	КонецПопытки;
	...
	Если ЗначениеЗаполнено(СсылкаНаЭД) Тогда
		...
		Если ЗначениеЗаполнено(РеквизитыЭД) Тогда
			...
			Если НастройкиОбмена.СпособОбмена <> Перечисления.СпособыОбменаЭД.ЧерезОператораЭДОТакском Тогда
				...
				ЗапросПоНастройкам.УстановитьПараметр("ИспользуетсяЭП",
					ЭлектронныеДокументыСлужебныйВызовСервера.ПолучитьЗначениеФункциональнойОпции("ИспользоватьЭлектронныеПодписи"));
...
Функция ВыборкаДопДанныеЭД(ЭДВладелец) Экспорт
	...
	Запрос.Текст = 
		...
		|	И СообщенияЭДО.ВидЭД = ЗНАЧЕНИЕ(Перечисление.ВидыЭД.ДопДанные)";
...
Функция ОпределитьНастройкиОбменаЭД(СтруктураПараметров, ПараметрыСертификатов = Неопределено) Экспорт
	...
	Если СтруктураПараметров.Свойство("НаправлениеЭД", НаправлениеЭД)
		...
		ФлагИспользованияЭП = ЭлектронныеДокументыСлужебныйВызовСервера.ПолучитьЗначениеФункциональнойОпции("ИспользоватьЭлектронныеПодписи");
		Запрос  = Новый Запрос;
		Запрос.УстановитьПараметр("НаправлениеЭД", НаправлениеЭД);
		Запрос.УстановитьПараметр("ВидЭД",         ВидЭД);
		...
		Если ЗначениеЗаполнено(Запрос.Текст) Тогда
			...
			Если НЕ ТЗ.Количество()=0 Тогда
				...
				Если ФлагИспользованияЭП И ПараметрыСертификатов <> Неопределено Тогда
				...
				Если ФлагИспользованияЭП Тогда
...
Функция СформироватьПрисоединенныеФайлы(МассивОбъектов, ПараметрыОбмена, ДопПараметры = "") Экспорт
	...
	Для Каждого СтруктураОбмена Из МассивСтруктурОбмена Цикл
		...
		ДобавленныйФайл = ЭлектронныеДокументы.ДобавитьФайл(
													...
													Неопределено,
													,
													Справочники.СообщенияЭДО.ПолучитьСсылку(УникальныйИдентификаторЭД));
		...
		Если Конверт.ДанныеЗашифрованы Тогда
			...
			Иначе
				...
				Если ПараметрыШифрования <> Неопределено Тогда
					...
					ПрофилиКриптографии = ЭлектроннаяПодпись.ОбщиеНастройки().ПрофилиНастроекКриптографии;
					ДвоичныеДанныеФайла = ПолучитьИзВременногоХранилища(ДанныеФайла.СсылкаНаДвоичныеДанныеФайла);
					
					ШифрованиеЗавершено = Ложь;
					Для Каждого Профиль Из ПрофилиКриптографии Цикл
						Попытка
							МенеджерКриптографии = Новый МенеджерКриптографии(
								Профиль.ПровайдерЭП,
								Профиль.ПутьМодуляКриптографии,
								Профиль.ТипПровайдераЭП);
							МенеджерКриптографии.АлгоритмПодписи     = Профиль.АлгоритмПодписи;
							МенеджерКриптографии.АлгоритмХеширования = Профиль.АлгоритмХеширования;
							МенеджерКриптографии.АлгоритмШифрования  = Профиль.АлгоритмШифрования;
							ЗашифрованныеДанные = МенеджерКриптографии.Зашифровать(ДвоичныеДанныеФайла, МассивСертификатов);
							ШифрованиеЗавершено = Истина;
							Прервать;
						Исключение
							// Пропускаем неподходящий криптопровайдер
						КонецПопытки;
					КонецЦикла;
					
					Если Не ШифрованиеЗавершено Тогда
...
Функция ЭтоПакетПроизвольногоЭД(МассивДокументов)
	...
	Для Каждого Документ Из МассивДокументов Цикл
		Если ТипЗнч(Документ.Документ) = Тип("ДокументСсылка.ПроизвольныйЭД")
			И Документ.Документ.Направление = Перечисления.НаправленияЭД.Исходящий Тогда
...
Функция СоздатьТекстОписиЭД(Конверт, СоответствиеВложений, СоответствиеФайлЭД, ЭтоПроизвольныйДокумент)
	...
	Если ЭтоПроизвольныйДокумент Тогда 
		Текст = Конверт.ЭлектронныеДокументы[0].ЭлектронныйДокумент.Документ.Текст;
...
Процедура ОтправитьПодтверждениеПоПакету(ПакетЭД, РесурсОтправителя, АдресОтправителя, ТипОтправки)
	...
	ВыборкаПрисоединенныхФайлов = ПолучитьВыборкуЭДПоОтбору(Новый Структура("Документ",ПакетЭД));
...
Функция НеобходимоПолучитьДвоичныеДанные(ДвоичныеДанные, ИмяФайла, СтруктураПараметров)
	...
	ЗапросПрисоединенногоФайла.Текст =
		...
		|	И ВЫРАЗИТЬ(СообщенияЭДО.Документ КАК Документ.ПакетЭД).Направление = &Направление
		|	И ВЫРАЗИТЬ(СообщенияЭДО.Документ КАК Документ.ПакетЭД).Получатель ПОДОБНО &Получатель
		|	И ВЫРАЗИТЬ(СообщенияЭДО.Документ КАК Документ.ПакетЭД).Отправитель ПОДОБНО &Отправитель
		|	И СообщенияЭДО.Наименование ПОДОБНО &Наименование
		|	И СообщенияЭДО.ПометкаУдаления = ЛОЖЬ";
...
Функция ПолучитьМассивАдресовСертификатовШифрования(ПрисоединенныйФайл) Экспорт
	...
	ПараметрыСоглашения = ОпределитьНастройкиОбменаЭДПоИсточнику(ПрисоединенныйФайл.Документ, , , ПрисоединенныйФайл);
...
Функция НайтиСоздатьПакетЭД(ТЗ_ПЭД, ПараметрыСоглашения)
	...
	Если НайденнаяСтрока = Неопределено Тогда
		...
		Если ЗначениеЗаполнено(СертификатШифрования)
			...
			СертификатШифрования   = ЭлектроннаяПодписьКлиентСервер.ПолучитьПредставлениеПользователя(
...
Функция ПолучитьПоследнююВерсиюЭДПоВладельцу(СсылкаНаВладельца)
	...
	ЗапросЭД.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	СообщенияЭДО.НомерВерсииЭД КАК НомерВерсии
		|ИЗ
		|	Справочник.СообщенияЭДО КАК СообщенияЭДО
		|ГДЕ
		|	СообщенияЭДО.ПометкаУдаления = ЛОЖЬ
		|	И СообщенияЭДО.Документ = &СсылкаНаВладельца
		|
		|УПОРЯДОЧИТЬ ПО
		|	СообщенияЭДО.НомерВерсииЭД УБЫВ";
...
Процедура ОбработатьУдалениеЭлектронногоДокумента(СсылкаНаОбъект)
	...
	Если НЕ ЭлектронныеДокументыСлужебныйВызовСервера.ЭтоСлужебныйДокумент(СсылкаНаОбъект) Тогда
		...
		Пока ВыборкаДокументов.Следующий() Цикл
			...
			НоваяЗаписьНабора.ЭлектронныйДокумент = Справочники.СообщенияЭДО.ПустаяСсылка();
			НаборЗаписей.Записать();
		КонецЦикла;
	КонецЕсли;
...
//
// Параметры:
//  СсылкаНаЭД - ссылка на элемент справочника СообщенияЭДО.
//
Процедура ЗаписатьСобытиеЖурналаРегистрацииПоОбменуЭД(СсылкаНаЭД)
	
	Если ТипЗнч(СсылкаНаЭД) = Тип("СправочникСсылка.СообщенияЭДО") Тогда
		...
		МенеджерЗаписи.ВладелецЭД         = СсылкаНаЭД.Документ;
...
Процедура ПроверитьИзменениеПередЗаписьюДокументаВладельцаЭДПередЗаписью(Источник, Отказ) Экспорт
	...
	// ЭД делаем только когда есть действующее соглашение об обмене.
	Если (НЕ Источник.ДополнительныеСвойства.Свойство("ЕстьСоглашение") ИЛИ НЕ Источник.ДополнительныеСвойства.ЕстьСоглашение)
		И НЕ ТипИсточника = Тип("СправочникОбъект.ИсходящиеДокументы") Тогда
...
Процедура НазначитьНовуюВерсиюЭДПриЗаписиДокументаВладельцаПриЗаписи(Источник, Отказ) Экспорт
