...
Процедура ПриНастройкеВариантовОтчетов(Настройки) Экспорт
	...
	Отчеты.СведенияОПользователях.НастроитьВариантыОтчета(Настройки);
...
// Вызывается при переходе на новую версию конфигурации.
...
//
Процедура ПроверитьПраваПользователя(ПользовательИБ, РежимПроверки = "ПриЗаписи")
	...
	// Проверка на изменение состава ролей у администраторов
	Если РежимПроверки = "ПриЗаписи" И Не РазделениеВключено Тогда
		...
		Если ПрежнийПользовательИБ <> Неопределено Тогда
			 ПроверитьПраваПользователя(ПрежнийПользовательИБ, "ПередЗаписью");
		КонецЕсли;
	КонецЕсли;
			...
			Для каждого Право Из СвойстваНедоступнойРоли.Права Цикл
				...
				Если РежимПроверки = "ПередЗаписью" Тогда
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Попытка изменить состав ролей у пользователя ""%1"" с ролью
						           |""%2"",
						           |предоставляющей право ""%3"".'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Если НайденныеНедоступныеРоли.Количество() = 1 Тогда
		...
		Если РежимПроверки = "ПередЗаписью" Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Невозможно изменить состав ролей у пользователя ""%1"".'"),
				ПользовательИБ.ПолноеИмя);
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Пользователю ""%1"" не может быть назначена роль
				           |""%2"".'"),
				ПользовательИБ.ПолноеИмя,
	...
	Иначе
		...
		Иначе
			...
			КонецЦикла;
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Пользователю ""%1"" не могут быть назначены роли:%2.'"), 
				ПользовательИБ.ПолноеИмя,
				Роли);
		КонецЕсли;
		ВызватьИсключение ТекстСообщения;
