...
//       Cм. РассылкаОтчетов.ИнициализироватьОтчет().
			...
			Если Не Выборка.ЭтоГруппа И Не ЗначениеЗаполнено(Выборка.РежимСовместимостиРазрешений) Тогда
				...
				Исключение
					...
					ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Не удалось определить режим совместимости разрешений для дополнительного отчета или обработки %1 по причине %2. Дополнительный отчет или обработка будет заблокирован и для него будет установлен режим совместимости разрешений ""Версия 2.1.3"".'"),
						Объект.Наименование, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
					
					ЗаписьЖурналаРегистрации(
						НСтр("ru = 'ДополнительныеОтчетыИОбработки.ЗаполнениеРежимаСовместимостиРазрешений'"),
...
Процедура СформироватьПодменюКомандЗаполнения(Форма, Параметры)
	...
	Если Подменю = Неопределено Тогда
		Подменю = Элементы.Вставить("ПодменюДополнительныхОбработокЗаполнения", Тип("ГруппаФормы"), Форма.КоманднаяПанель);
	Иначе
