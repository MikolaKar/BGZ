...
Процедура ПередЗавершениемРаботыСистемы(Параметры) Экспорт
	...
	Если ПараметрыПриЗапускеИЗавершенииПрограммы.Свойство("ДействияПередЗавершениемРаботыСистемыВыполненыДО") Тогда
		ПараметрыПриЗапускеИЗавершенииПрограммы.Удалить("ДействияПередЗавершениемРаботыСистемыВыполненыДО");
		Возврат;
	КонецЕсли;
	
	ОткрытьФормуВопроса =
		РаботаСПрограммой.ОткрытьФормуВопросаПриЗавершенииПрограммы();
		
	ОбработчикЗакрытия = 
		Новый ОписаниеОповещения("ОбработкаПодтвержденияЗавершенияРаботы", ЭтотОбъект, Параметры);
		
	Если ОткрытьФормуВопроса Тогда
		Параметры.Отказ = Истина;
		РежимОткрытияОкна = РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс;
		ОткрытьФорму("ОбщаяФорма.ВопросПриЗавершенииРаботы",,,,,, ОбработчикЗакрытия, РежимОткрытияОкна);
	Иначе
		ДопПараметры = Новый Структура;
		ДопПараметры.Вставить("ЗакрытьПрограмму", Истина);
		ДопПараметры.Вставить("ЗакрытьПринудительно", Ложь);
		ВыполнитьОбработкуОповещения(ОбработчикЗакрытия, ДопПараметры);
	КонецЕсли;
	
КонецПроцедуры

Процедура ОбработкаПодтвержденияЗавершенияРаботы(Результат, Параметры) Экспорт
	...
	Если Результат.Свойство("БольшеНеЗадаватьЭтотВопрос") И Результат.БольшеНеЗадаватьЭтотВопрос Тогда
		ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекСохранить("НастройкаВыходаИзПрограммы",, Ложь);
	КонецЕсли;	
	
	Если Не Результат.ЗакрытьПрограмму Тогда
		Параметры.Отказ = Истина;
		Возврат;
	КонецЕсли;	
	
	// ФайловыеФункции
	ОчищатьПриЗавершенииРаботы = ФайловыеФункцииКлиентПовтИсп.
		ПолучитьПерсональныеНастройкиРаботыСФайлами().ОчищатьПриЗавершенииРаботы;
	Если ОчищатьПриЗавершенииРаботы Тогда 
		ФайловыеФункцииКлиент.ОчиститьОсновнойРабочийКаталогПриЗавершенииРаботы();
	КонецЕсли;	
	// Конец ФайловыеФункции
	
	// Удаление временных сохраненных поисков
	ТекущийПользователь = ПользователиКлиентСервер.ТекущийПользователь();
	Делопроизводство.УдалитьВременныеСохраненныеПоиски(ТекущийПользователь);
	// Конец удаление временных сохраненных поисков
	
	ПараметрыПриЗапускеИЗавершенииПрограммы.Вставить("ДействияПередЗавершениемРаботыСистемыВыполненыДО");
	
	Если Не Результат.Свойство("ЗакрытьПринудительно") Или Результат.ЗакрытьПринудительно Тогда
