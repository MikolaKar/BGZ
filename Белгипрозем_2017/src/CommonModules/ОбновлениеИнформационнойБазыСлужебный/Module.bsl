...
// Для внутреннего использования.
//
Функция ВыполнитьИтерациюОбновления(Знач ИтерацияОбновления, Знач ХодВыполненияОбработчиков, Знач ОперативноеОбновление = Ложь, Знач ВФоне = Ложь) Экспорт
	...
	// Для предотвращения неоднозначности с переходом с другой конфигурации.
	Если ТекущаяВерсияИБ = "0.0.0.0" Тогда
		УстановитьВерсиюИБ(ИдентификаторБиблиотеки, ТекущаяВерсияИБ, ИтерацияОбновления.ЭтоОсновнаяКонфигурация);
	КонецЕсли;
	...
	Для Каждого Версия Из ВыполняемыеОбработчики.Строки Цикл
		...
		Для Каждого Обработчик Из Версия.Строки Цикл
			...
			ДополнительныеПараметры = Новый Структура("ЗаписыватьВЖурнал, ИдентификаторБиблиотеки, ХодВыполненияОбработчиков, ВФоне",
				ЗаписыватьВЖурнал, ИдентификаторБиблиотеки, ХодВыполненияОбработчиков, ВФоне);
...
Функция ОбработчикиПереходаСДругойПрограммы(ПредыдущееИмяКонфигурации) 
	...
	Возврат ОбработчикиПерехода.НайтиСтроки(Новый Структура("ПредыдущееИмяКонфигурации", ПредыдущееИмяКонфигурации));
...
Процедура ПерейтиСДругойПрограммы()
	...
	ПредыдущаяВерсияКонфигурации = Запрос.Выполнить().Выгрузить()[0].Версия;
	...
	НоваяЗапись.Версия = ПредыдущаяВерсияКонфигурации;
...
Процедура ЗапланироватьОтложенноеОбновление(ИтерацииОбновления)
	...
	ДеревоОбработчиков = ВыполненныеОбработчикиПрошлыхВерсий();
	СведенияОбОбновлении = НовыеСведенияОбОбновлении();
...
Функция СведенияОбОбновленииИнформационнойБазы() Экспорт
	...
	Если ТипЗнч(СведенияОбОбновленииИБ) = Тип("Структура") Тогда
		Если СведенияОбОбновленииИБ.Количество() = 1 Тогда
			СведенияОбОбновленииИБ = Неопределено;
		Иначе
...
Процедура ЗаписатьСведенияОбОбновленииИнформационнойБазы(Знач СведенияОбОбновлении) Экспорт
	...
	СтароеЗначение = Константы.СведенияОбОбновленииИБ.Получить().Получить();
	
	Если ТипЗнч(СтароеЗначение) = Тип("Структура") И СтароеЗначение.Свойство("ЛегальнаяВерсия") Тогда
		НовоеЗначение.Вставить("ЛегальнаяВерсия", СтароеЗначение.ЛегальнаяВерсия);
	КонецЕсли;
...
Функция ТребуетсяПроверитьЛегальностьПолученияОбновления() Экспорт
	...
	Иначе
		УстановитьПривилегированныйРежим(Истина);
		Свойства = Константы.СведенияОбОбновленииИБ.Получить().Получить();
		Если ТипЗнч(Свойства) = Тип("Структура") И Свойства.Свойство("ЛегальнаяВерсия") Тогда
			ЛегальнаяВерсия = Свойства.ЛегальнаяВерсия;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаписатьПодтверждениеЛегальностиПолученияОбновлений() Экспорт
	...
	СтароеЗначение = Константы.СведенияОбОбновленииИБ.Получить().Получить();
	Если ТипЗнч(СтароеЗначение) = Тип("Структура") Тогда
		НовоеЗначение = СтароеЗначение;
	Иначе
		НовоеЗначение = Новый Структура;
	КонецЕсли;
	
	НовоеЗначение.Вставить("ЛегальнаяВерсия", Метаданные.Версия);
...
Функция ПоказатьОписаниеИзмененийСистемы()
	...
	// функционал по отображению описанияю изменений не используется
	Возврат Ложь;
