...
Функция ПолучитьСписокЕжедневныхОтчетов(Сообщение, Ответ, ИмяТипа) Экспорт
	...
	Попытка 
		...
		СоответствиеРеквизитов.Вставить("Пользователь", "user#DMUser");
		...
		Если Отбор.Количество() > 0 Тогда 
			...
			Для Каждого СтрокаОтбора Из Отбор Цикл
				Если СтрокаОтбора.Ключ = "Пользователь" Тогда 
					Запрос.Текст = Запрос.Текст 
			...
			Для Каждого Условие Из Сообщение.query.conditions Цикл
				...
				Если ВРег(Условие.property) = ВРег("byUser") Тогда
					Если ОбработкаЗапросовXDTO.ПроверитьТип(Условие.value, "DMObjectID") Тогда
						Отбор.Вставить("Пользователь", ОбработкаЗапросовXDTO.ПолучитьСсылкуПоObjectID(Условие.value));
					Иначе
						Отбор.Вставить("Пользователь", ПараметрыСеанса.ТекущийПользователь);
					КонецЕсли;
...
Функция СоздатьЗаписьФактическихТрудозатрат(Сообщение) Экспорт
	...
	Попытка
		...
		Если ОбъектXDTO.Установлено("user") Тогда
			...
			ОбработкаЗапросовXDTO.ЗаполнитьРеквизитИзСвойстваXDTO(Узел, ПараметрыОтчета.Пользователь, ОбъектXDTO, "user");
		КонецЕсли;
...
Процедура ПолучитьДанныеЕжедневногоОтчета(Объект, ОбъектXDTO, НаборКолонок = Неопределено)
	...
	Работы = УчетВремениВызовСервера.ПолучитьРаботыЗаДень(Объект.Дата, Объект.Пользователь);
...
Процедура ЗаполнитьДанныеЕжедневногоОтчета(Узел, ДокументОбъект, ОбъектXDTO)
	...
	ОбработкаЗапросовXDTO.ЗаполнитьРеквизитИзСвойстваXDTO(Узел, ДокументОбъект.Пользователь, ОбъектXDTO, "user");
	...
	Если Не ЗначениеЗаполнено(ДокументОбъект.Подразделение) Тогда
		ДокументОбъект.Подразделение = РаботаСПользователями.ПолучитьПодразделение(ДокументОбъект.Пользователь);
	КонецЕсли;
