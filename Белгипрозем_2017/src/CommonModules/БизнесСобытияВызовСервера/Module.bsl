...
Процедура ОбработкаПроизошедшихБизнесСобытий() Экспорт
	...
	Пока Истина Цикл // В цикле получаем порциями по 100 необработанные события, пока есть что обрабатывать
		...
		Запрос.Текст = 
			...
			|	ПроизошедшиеБизнесСобытия.Дата КАК Дата,
		...
		Для Каждого Строка Из Таблица Цикл
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(НСтр("ru='Обработка бизнес-событий'"), УровеньЖурналаРегистрации.Ошибка, 
					Метаданные.Справочники.ПроизошедшиеБизнесСобытия, Строка.Ссылка, СообщениеОбОшибке);
				
				Попытка
					СобытиеОбъект = Строка.Ссылка.ПолучитьОбъект();
					СобытиеОбъект.ЧислоПопытокОбработки = СобытиеОбъект.ЧислоПопытокОбработки + 1;
				...
				Для Каждого ОбработчикСсылка Из СработавшиеПользовательскиеОбработчики Цикл
					...
					Попытка
						...
						ВыполняемыйКодОтменыОбработки = ОбщегоНазначения.ПолучитьЗначениеРеквизита(ОбработчикСсылка, 
					...
					Исключение
						...
						СообщениеОбОшибке = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
						ЗаписьЖурналаРегистрации(НСтр("ru='Обработка бизнес-событий. Выполнение кода отмены обработки.'"), 
	...
	Если ДатаПоследнейОчисткиБизнесСобытий = Дата("00010101000000") Тогда
		НужнаОчистка = Истина;
	Иначе
		Если ТекущаяДата() - ДатаПоследнейОчисткиБизнесСобытий > ПериодОчистки Тогда
			НужнаОчистка = Истина;
		КонецЕсли;		
	КонецЕсли;	
	...
	ДатаОтсечения = ТекущаяДата() - СрокХраненияБизнесСобытий * 86400; 
	...
	Запрос.Текст = 
		...
		|	И ПроизошедшиеБизнесСобытия.Дата < &ДатаОтсечения
		|
		|УПОРЯДОЧИТЬ ПО
		|	ПроизошедшиеБизнесСобытия.Дата";
	...
	Константы.ДатаПоследнейОчисткиБизнесСобытий.Установить(ТекущаяДата());
...
Функция ШаблонПодходитДляАвтозапускаБизнесПроцессаПоДокументу(ШаблонСсылка, ПредметСсылка, 
	...
	Если ВидСобытия = Справочники.ВидыБизнесСобытий.СозданиеКорреспондента Тогда
	...
	Если ТипЗнч(ПредметСсылка) = Тип("СправочникСсылка.Файлы")
		...
		ВладелецФайла = ОбщегоНазначения.ПолучитьЗначениеРеквизита(ПредметСсылка, "ВладелецФайла");
		...
		Если ТипЗнч(ВладелецФайла) = Тип("СправочникСсылка.ПапкиФайлов") Тогда
			...
			Пока ЗначениеЗаполнено(Папка) Цикл
				...
				Папка = ОбщегоНазначения.ПолучитьЗначениеРеквизита(Папка, "Родитель");
	...
	Если ТипЗнч(ПредметСсылка) = Тип("СправочникСсылка.ВнутренниеДокументы")
		...
		Папка = ОбщегоНазначения.ПолучитьЗначениеРеквизита(ПредметСсылка, "Папка");
	...
	Если (ТипЗнч(Подписчик) = Тип("СправочникСсылка.ВидыВнутреннихДокументов")
		...
		ВидДокумента = ОбщегоНазначения.ПолучитьЗначениеРеквизита(ПредметСсылка, "ВидДокумента");
		
		Если ВидДокумента = Подписчик Тогда
			Возврат Истина;
		КонецЕсли;	
...
Процедура ОбработатьСобытиеАвтоматическийЗапускБизнесПроцессов(Событие)
	...
	Если ИспользоватьУчетПоОрганизациям И ДелопроизводствоКлиентСервер.ЭтоСсылкаНаДокумент(Событие.Источник) Тогда
		...
		Организация = ОбщегоНазначения.ПолучитьЗначениеРеквизита(Событие.Источник, "Организация");
...
Процедура БизнесСобытияПередЗаписьюБизнесПроцессаПередЗаписью(Источник, Отказ) Экспорт
	...
	Реквизиты = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(Источник.Ссылка, "Стартован, Завершен, Состояние");
...
Процедура БизнесСобытияПередЗаписьюОбъектаПередЗаписью(Источник, Отказ) Экспорт
	...
	Реквизиты = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(Источник.Ссылка, "ПометкаУдаления");
