...
Функция ОбработатьУниверсальноеСообщение(Сообщение) Экспорт
	...
	Попытка
		...
		ИначеЕсли ПроверитьТип(Сообщение, "DMAddFileRequest") Тогда
			Возврат ОбработкаЗапросовXDTOФайлы.ДобавитьФайлВДокументСДиска(Сообщение);
	...
	Попытка 
		...
		Для Каждого ОбъектИд Из Сообщение.objectIds Цикл
			...
			ИначеЕсли ОбъектИд.type = "DMCorrespondent" Тогда
				Объект = ОбработкаЗапросовXDTOДокументы.ПолучитьКорреспондента(Узел, ОбъектИд, Сообщение.columnSet);
				
			ИначеЕсли ОбъектИд.type = "DMDailyReport" Тогда
				Объект = ОбработкаЗапросовXDTOУчетВремени.ПолучитьЕжедневныйОтчет(ОбъектИд);
				
			ИначеЕсли ОбъектИд.type = "DMActivity" Тогда
				Объект = ПолучитьМероприятие(ОбъектИд);
...
// Получает структуру ссылки на внешний объект по объекту Документооборота
//
// Параметры:
...
Функция ПолучитьСсылкуНаВнешнийОбъект(Узел, Объект) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		Иначе // тип в виде "СправочникСсылка.Контрагенты" требует преобразования
			Тип = СтрЗаменить(Выборка.ТипВнешнегоОбъекта, "СправочникСсылка.", "Справочник");
			Тип = СтрЗаменить(Выборка.ТипВнешнегоОбъекта, "ДокументСсылка.", "Документ");
...
// Возвращает объект XDTO мероприятия по ObjectID
//
// Параметры:
//   ОбъектИд - ОбъектXDTO типа DMObjectId
// Возвращаемое значение:
//   ОбъектXDTO запрошенного типа или DMError
// 
Функция ПолучитьМероприятие(ОбъектИД) Экспорт
	
	Попытка 
		
		Ссылка = ПолучитьСсылкуПоObjectID(ОбъектИд);
		
		Объект = СоздатьОбъект(ОбъектИд.type);
		Объект.objectID = ПолучитьObjectIDПоСсылке(Ссылка);
		Объект.name = Строка(Ссылка);
		
		Объект.beginDate = Ссылка.ДатаНачала;
		Объект.endDate = Ссылка.ДатаОкончания;
		Объект.venue = Ссылка.МестоПроведения; 
		...
		ОбработкаЗапросовXDTOФайлы.ЗаполнитьСписокФайловИзМассива(Объект, МассивФайлов, НаборКолонокФайлов);
...
Функция ПроверитьТип(ОбъектXDTO, Тип) Экспорт
	...
	Возврат (ОбъектXDTO.Тип() = ФабрикаXDTO.Тип("http://www.1c.ru/dm", Тип));
...
Функция СоответствиеТипов() Экспорт
	...
	ДобавитьСтроку(Таблица, "DMActivity",				"Справочник.Мероприятия");
	ДобавитьСтроку(Таблица, "DMDiscussionTopic",		"Справочник.ТемыОбсуждений");
	ДобавитьСтроку(Таблица, "DMDiscussionMessage",		"Справочник.СообщенияОбсуждений");
	...
	ДобавитьСтроку(Таблица, "DMUserGroup",  			"Справочник.ГруппыПользователей");
	...
	ДобавитьСтроку(Таблица, "DMCorrespondent",  		"Справочник.Корреспонденты");
	...
	ДобавитьСтроку(Таблица, "DMObjectPropertyValueHierarchy", 	"Справочник.ЗначенияСвойствОбъектовИерархия");
	...
	ДобавитьСтроку(Таблица, "DMBusinessProcessRoutingType",	"Перечисление.ВариантыМаршрутизацииЗадач");
	ДобавитьСтроку(Таблица, "DMPredecessorsStageConsiderationCondition", "Перечисление.УсловияРассмотренияПредшественниковЭтапа");
...
Функция ПолучитьНастройки(Сообщение) Экспорт
	...
	Попытка 
		...
		Ответ.useDigitalSignatures = РаботаСФайламиВызовСервера.ПолучитьИспользоватьЭлектронныеПодписиИШифрование();
...
Функция ПолучитьСписокФайловПоВладельцу(Сообщение) Экспорт
	...
	Попытка 
		...
		Для Каждого Владелец Из Сообщение.owners Цикл
			...
			МассивОбъектовДО = РаботаСФайламиВызовСервера.ПолучитьВсеПодчиненныеФайлы(ВладелецФайла, Ложь);
...
Процедура ПолучитьНаборДополнительныхРеквизитовОбъектаДО(ОбъектДО, ОбъектXDTO) Экспорт
	...
	НаборЗначенийСвойствОбъекта = УправлениеСвойствами.ПолучитьЗначенияСвойств(
		ОбъектДО.Ссылка, Истина, Ложь, НаборСвойствОбъекта);
	...
	Для Каждого Элемент Из Результат Цикл
		...
		ДополнительноеСвойство.name = Элемент.Значение.Свойство.Наименование;
		ДополнительноеСвойство.objectId = ПолучитьObjectIDПоСсылке(Элемент.Значение.Свойство);
		
		Для Каждого ТипРеквизита Из Элемент.Значение.ТипРеквизита.Типы() Цикл
			
			ОбъектТипXDTO = СоздатьОбъект("DMType");
			
			Если ТипРеквизита = Тип("Дата") Тогда
				ОбъектТипXDTO.xdtoClassName = "date";
				ОбъектТипXDTO.presentation = "Дата";
				
			ИначеЕсли ТипРеквизита = Тип("Число") Тогда
				ОбъектТипXDTO.xdtoClassName = "integer";
				ОбъектТипXDTO.presentation = "Число";
				
			ИначеЕсли ТипРеквизита = Тип("Строка") Тогда
				ОбъектТипXDTO.xdtoClassName = "string";
				ОбъектТипXDTO.presentation = "Строка";
				
			ИначеЕсли ТипРеквизита = Тип("Булево") Тогда
				ОбъектТипXDTO.xdtoClassName = "boolean";
				ОбъектТипXDTO.presentation = "Логическое значение";
			...
			ИначеЕсли ТипРеквизита = Тип("СправочникСсылка.ЗначенияСвойствОбъектов") Тогда
				...
				ОбъектТипXDTO.presentation = "Значение свойства объекта";
			...
			ИначеЕсли ТипРеквизита = Тип("СправочникСсылка.ЗначенияСвойствОбъектовИерархия") Тогда
				...
				ОбъектТипXDTO.presentation = "Значение свойства объекта (иерархия)";
				
			ИначеЕсли ТипРеквизита = Тип("СправочникСсылка.Корреспонденты") Тогда
				ОбъектТипXDTO.xdtoClassName = "DMCorrespondent";
				ОбъектТипXDTO.presentation = "Корреспондент";
				
			ИначеЕсли ТипРеквизита = Тип("СправочникСсылка.Пользователи") Тогда
				ОбъектТипXDTO.xdtoClassName = "DMUser";
				ОбъектТипXDTO.presentation = "Пользователь";
				
			ИначеЕсли ТипРеквизита = Тип("СправочникСсылка.ФизическиеЛица") Тогда
				ОбъектТипXDTO.xdtoClassName = "DMPrivatePerson";
				ОбъектТипXDTO.presentation = "Физическое лицо";
		...
		Если ЗначениеЗаполнено(Элемент.Значение.Значение) Тогда
			Если ТипЗнч(Элемент.Значение.Значение) = Тип("Дата") 
			 ...
			 Или ТипЗнч(Элемент.Значение.Значение) = Тип("Булево") Тогда
				...
				ДополнительноеСвойство.propertySimpleValue = Элемент.Значение.Значение;		
			Иначе
				ОбъектЗначениеXDTO = СоздатьОбъект("DMObject");
				ОбъектЗначениеXDTO.name = Строка(Элемент.Значение.Значение);
				ОбъектЗначениеXDTO.objectId = ПолучитьObjectIDПоСсылке(Элемент.Значение.Значение);
	...
	Для Каждого ДополнительныйРеквизит Из ОбъектXDTO.additionalProperties Цикл
		...
		Если ЗначениеЗаполнено(ОбъектСвойство) Тогда
			...
			ИначеЕсли ДополнительныйРеквизит.Установлено("propertyObjectValue") Тогда
				...
				Если Не ЗначениеЗаполнено(ЗначениеСвойстваXDTO.objectId.Id) И 
					Не ЗначениеЗаполнено(ЗначениеСвойстваXDTO.objectId.type) 
					И ЗначениеСвойстваXDTO.Установлено("externalObject") Тогда 
					ЗначениеСвойства = НайтиСоздатьВнешнийОбъект(Узел, ЗначениеСвойстваXDTO);
				Иначе
					ЗначениеСвойства = ПолучитьСсылкуПоObjectID(ЗначениеСвойстваXDTO.objectId);
...
Функция Создать(Сообщение)
	...
	ИначеЕсли ПроверитьТип(Сообщение.object, "DMCorrespondent") Тогда
		Объект = ОбработкаЗапросовXDTOДокументы.СоздатьКорреспондента(Сообщение);
...
Функция ЗаписатьОбъекты(Сообщение)
	...
	Для Каждого Объект Из Сообщение.objects Цикл
		...
		ИначеЕсли ПроверитьТип(Объект, "DMCorrespondent") Тогда
			Объект = ОбработкаЗапросовXDTOДокументы.ИзменитьКорреспондента(Узел, Объект);
...
Функция ПолучитьНовыйОбъект(Сообщение)
	...
	Если Сообщение.type = "DMIncomingDocument" Тогда 
		Ответ = ОбработкаЗапросовXDTOДокументы.ПолучитьНовыйВходящийДокумент(Узел, Сообщение.columnSet);
		
	ИначеЕсли Сообщение.type = "DMOutgoingDocument" Тогда 
		Ответ = ОбработкаЗапросовXDTOДокументы.ПолучитьНовыйИсходящийДокумент(Узел, Сообщение.columnSet);
		
	ИначеЕсли Сообщение.type = "DMInternalDocument" Тогда 
		Ответ = ОбработкаЗапросовXDTOДокументы.ПолучитьНовыйВнутреннийДокумент(Узел, Сообщение.columnSet);
	...
	ИначеЕсли Сообщение.type = "DMCorrespondent" Тогда 
		Ответ = ОбработкаЗапросовXDTOДокументы.ПолучитьНовогоКорреспондента(Узел, Сообщение.columnSet);
...
Функция НайтиСоздатьВнешнийОбъект(Узел, ЗначениеСвойстваXDTO)
	...
	Если ПроверитьТип(ЗначениеСвойстваXDTO, "DMCorrespondent") Тогда 
		...
		СсылкаДО = ОбработкаЗапросовXDTOДокументы.НайтиСоздатьКорреспондента(Узел, ЗначениеСвойстваXDTO);
	...
	ИначеЕсли ПроверитьТип(ЗначениеСвойстваXDTO, "DMLegalPrivatePerson") Тогда
		...
		НаименованиеПеречисления = ЗначениеСвойстваXDTO.Name;
		
		Для Каждого ЗначениеПеречисления Из Перечисления.ЮрФизЛицо Цикл
			Если Строка(ЗначениеПеречисления) = НаименованиеПеречисления Тогда  
...
//
Функция СоответствиеСвойствXDTOРеквизитамПоиска(Тип)
	...
	ИначеЕсли Тип = "DMInternalDocument" Тогда
		...
		Результат.Вставить("correspondent", "Корреспондент");
	...
	Попытка
		...
		Если Сообщение.query <> Неопределено Тогда 
			...
			Для Каждого Условие Из Сообщение.query.conditions Цикл
				...
				ИначеЕсли ВРег(Условие.property) = ВРег("correspondent")
					И Сообщение.Type = "DMOutgoingDocument" Тогда
					УсловиеОтбора = УсловияОтбора.Добавить();
				...
				ИначеЕсли Сообщение.type = "DMRelationType" Тогда 
					...
					Если ДокументИз <> Неопределено Тогда 
						НастройкиСвязи = СвязиДокументов.ПолучитьНастройкиСвязи(ДокументИз);
						УсловиеОтбора = УсловияОтбора.Добавить();
						УсловиеОтбора.Поле = "ОбъектВыборки.Ссылка";
						УсловиеОтбора.ОператорСравнения = "IN";
						УсловиеОтбора.Значение = НастройкиСвязи.ВыгрузитьКолонку("ТипСвязи");
					КонецЕсли;	
					
					Если ДокументИз <> Неопределено И ДокументНа <> Неопределено Тогда 
		...
		Для каждого ИмяСвойства из НаборКолонок Цикл
			...
			ИначеЕсли ИмяСвойства = "correspondent" И Сообщение.type = "DMOutgoingDocument" Тогда
				ТекстСоединений = ТекстСоединений + "
				|ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ИсходящиеДокументы.Получатели КАК ПервыеПолучатели
			...
			Иначе // свойство, получаемое через точку
		...
		|%ТекстСоединений%
		|ГДЕ НЕ ОбъектВыборки.ПометкаУдаления
		|";
		...
		Если Сообщение.type = "DMFile" Тогда
			...
			|	ИЛИ ОбъектВыборки.ВладелецФайла ССЫЛКА Справочник.Корреспонденты
			|)
			|";
		...
		Для каждого УсловиеОтбора из УсловияОтбора Цикл
			...
			ИначеЕсли УсловиеОтбора.Поле = "anyCorrespondent" Тогда
				...
				|	ИЗ Справочник.ИсходящиеДокументы.Получатели
		...
		Если Найти(Сообщение.type,"Template") <> 0 Тогда
