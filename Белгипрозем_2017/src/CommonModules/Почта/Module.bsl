...
Функция ИнтернетПочтаУстановитьСоединение(
	...
	СообщениеОбОшибке = "";
	ПараметрыУчетнойЗаписи = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(УчетнаяЗапись, "ИспользоватьДляОтправки, ИспользоватьДляПолучения");
...
Функция ПолучитьСтруктуруИнтернетПочтовогоВложения(ИнтернетПочтовоеВложение) Экспорт
	...
	ИначеЕсли ТипЗнч(ИнтернетПочтовоеВложение.Данные) = Тип("ИнтернетПочтовоеСообщение") Тогда
		...
		Результат.Вставить("Данные",
			ПолучитьСтруктуруПочтовогоСообщенияИзПисьма(ИнтернетПочтовоеВложение.Данные));
