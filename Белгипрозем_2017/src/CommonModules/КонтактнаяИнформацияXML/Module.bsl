...
Функция XSLT_КонтрольПредставленияИКомментария(Знач Текст, Знач Представление = Неопределено, Знач Комментарий = Неопределено)
	...
	Если Комментарий <> Неопределено Тогда
		...
		|      <xsl:choose>
		|        <xsl:when test=""text()=''"">" + НормализованнаяСтрокаXML(Комментарий) + "</xsl:when>
		|        <xsl:otherwise>
		|          <xsl:value-of select="".""/>
		|        </xsl:otherwise>
		|      </xsl:choose>
...
//
// Параметры:
//     Текст - Строка - Корректируемяя строка XML
//
// Возвращаемое значение:
//     Строка - Нормализовання строка
//
...
// Подготовливает строку для включения в текст XML, убирая спецсимволы
//
// Параметры:
//     Текст - Строка - Корректируемяя строка XML
//
