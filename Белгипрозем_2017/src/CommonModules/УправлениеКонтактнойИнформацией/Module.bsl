...
// 1С-Минск
	ЗаполнитьРеквизитыТабличнойЧастиДляАдресаРБ(СтрокаТабличнойЧасти, Источник);
	Возврат;
// 1С-Минск
...
//    Источник             - объект XDTO с контактной информацией
//
Процедура ЗаполнитьРеквизитыТабличнойЧастиДляАдресаРБ(СтрокаТабличнойЧасти, Источник)

	// Умолчания
	СтрокаТабличнойЧасти.Страна = "";
	СтрокаТабличнойЧасти.Регион = "";  	// Область из (СубъектРФ)
	СтрокаТабличнойЧасти.Район  = "";  	// Район из (СвРайМО.Район)
	СтрокаТабличнойЧасти.Сельсовет = "";// Сельсовет из (Город)
	СтрокаТабличнойЧасти.Город  = "";   // Нас. пункт из (НаселПункт)
	...
	Если ЭтоАдрес И Адрес.Состав <> Неопределено Тогда 
		...
		Если АдресРФ <> Неопределено Тогда
			// Российский адрес
			СтрокаТабличнойЧасти.Регион = АдресРФ.СубъектРФ;
			СвРайМО = АдресРФ.СвРайМО;
			Если ТипЗнч(СвРайМО) = Тип("ОбъектXDTO") Тогда
				СтрокаТабличнойЧасти.Район  = СвРайМО.Район;
			КонецЕсли;
			СтрокаТабличнойЧасти.Сельсовет  = АдресРФ.Город;
