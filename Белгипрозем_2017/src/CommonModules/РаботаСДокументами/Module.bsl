	...
	Для Каждого Строка Из Ответственные Цикл
		...
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			...
			Если ЗначениеЗаполнено(Документ.РегистрационныйНомер) Тогда 
				ТелоПисьма = ТелоПисьма + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = ' № %1'"),
			...
			КорреспондентыДляСписков = РегистрыСведений.ОбщиеРеквизитыДокументов.ПолучитьОбщийРеквизитДокумента(
				Документ.Ссылка, "КорреспондентыДляСписков");
			
			Если Документ.Корреспонденты.Количество() > 1 Тогда 
				ТелоПисьма = ТелоПисьма + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = ', корреспонденты %1'"),
					КорреспондентыДляСписков);
					
			ИначеЕсли Документ.Корреспонденты.Количество() > 0 Тогда 
				ТелоПисьма = ТелоПисьма + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = ', корреспондент %1'"),
					КорреспондентыДляСписков);
			...
			ТелоПисьма = ТелоПисьма + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				Формат(НайденнаяСтрока.НоваяДатаОкончания, "ДФ=dd.MM.yyyy"));
		...
		Исключение
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при отправке уведомления об автоматическом продлении договоров: %1.'", КодОсновногоЯзыка),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			УровеньВажностиСобытия = УровеньЖурналаРегистрации.Ошибка;
		КонецПопытки;
		...
		Если ПустаяСтрока(ТекстСообщения) Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Уведомление об автоматическом продлении договоров успешно отправлено на адрес %1.'", КодОсновногоЯзыка),
				ПочтовыйАдрес);
			УровеньВажностиСобытия = УровеньЖурналаРегистрации.Информация;
		КонецЕсли;
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Документы.Автоматическое продление договоров'", КодОсновногоЯзыка),
