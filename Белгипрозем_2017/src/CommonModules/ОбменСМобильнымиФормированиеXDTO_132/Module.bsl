...
Процедура ПолучитьXDTOИзОбъекта(
	...
	ИначеЕсли ТипЗнч(Объект) = Тип("ДокументСсылка.ВходящееПисьмо") Тогда
		...
		XDTOВходящееПисьмо = ПолучитьXDTOИзВходящегоПисьма(Объект, МобильныйКлиент);
	...
	ИначеЕсли ТипЗнч(Объект) = Тип("ДокументСсылка.ИсходящееПисьмо") Тогда
		...
		XDTOИсходящееПисьмо = ПолучитьXDTOИзИсходящегоПисьма(Объект, МобильныйКлиент);
	...
	ИначеЕсли ТипЗнч(Объект) = Тип("РегистрСведенийНаборЗаписей.СведенияОПрочтении") Тогда
		...
		XDTOСостояниеПрочтения = ПолучитьXDTOИзСведенийОПрочтении(Объект);
	...
	ИначеЕсли ТипЗнч(Объект) = Тип("СправочникСсылка.ЗаписиРабочегоКалендаря") Тогда
		...
		XDTOЗаписьРабочегоКалендаря = ПолучитьXDTOПоЗаписиРабочегоКалендаря(Объект);
	...
	ИначеЕсли ТипЗнч(Объект) = Тип("РегистрСведенийНаборЗаписей.ИсторияСобытийЗадач") Тогда
		...
		Объект = Объект.Отбор.Задача.Значение;
...
Функция ПолучитьXDTOИзПапкиПисем(СсылкаНаПапку, МобильныйКлиент)
	...
	XDTOПапкаПисем = ОбменСМобильными_132.СоздатьОбъект("MailFolder");
...
Функция ПолучитьXDTOИзУчетнойЗаписи(СсылкаУчетнаяЗапись, МобильныйКлиент)
	...
	XDTOУчетнаяЗапись = ОбменСМобильными_132.СоздатьОбъект("MailAccount");
...
Функция ПолучитьXDTOИзВходящегоПисьма(СсылкаВходящееПисьмо, МобильныйКлиент) 
	
	Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		СсылкаВходящееПисьмо, 
		"ТекстПисьмаПростойТекстХранилище, ОтправительАдресат, ВидМаршрутизации");
	
	МассивXDTO = Новый Массив;
	
	XDTOВходящееПисьмо = ОбменСМобильными_132.СоздатьОбъект("IncomingMail");
...
Функция ПолучитьXDTOИзИсходящегоПисьма(СсылкаИсходящееПисьмо, МобильныйКлиент) 
	
	Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		СсылкаИсходящееПисьмо,
		"ПодготовленоКОтправке, ДатаОтправки, ТекстХранилище, ПисьмоОснование, ТипОтвета, ВидМаршрутизации");
	
	XDTOИсходящееПисьмо = ОбменСМобильными_132.СоздатьОбъект("OutgoingMail");
	...
	XDTOИсходящееПисьмо.baseMailId = УникальныйИдентификаторПустой();				
	Если ЗначениеЗаполнено(Реквизиты.ПисьмоОснование) И ЗначениеЗаполнено(Реквизиты.ТипОтвета) Тогда	
		Если ТипЗнч(Реквизиты.ПисьмоОснование) = Тип("ДокументСсылка.ВходящееПисьмо") Тогда
			XDTOИсходящееПисьмо.baseMailType = "IncomingMail";
		Иначе
			XDTOИсходящееПисьмо.baseMailType = "OutgoingMail";
		КонецЕсли;
		XDTOИсходящееПисьмо.baseMailId = Реквизиты.ПисьмоОснование.УникальныйИдентификатор();
...
Функция ПолучитьXDTOИзФайла(СсылкаФайл, МобильныйКлиент, ПараметрыСинхронизации, ВыгрузитьПринудительноПолностью = Ложь) 
	...
	ОбъектФайл = ОбменСМобильными_132.СоздатьОбъект("File");
	...
	ОбъектФайл.ownerId    = ДанныеФайла.Владелец.УникальныйИдентификатор();
	...
	Если ТипЗнч(ДанныеФайла.Владелец) = Тип("ДокументСсылка.ВходящееПисьмо") Тогда
		Если Не СинхронизироватьПочту Тогда
			Возврат Неопределено;
		КонецЕсли;
		ОбъектФайл.ownerType = "IncomingMail";
	...
	ИначеЕсли ТипЗнч(ДанныеФайла.Владелец) = Тип("ДокументСсылка.ИсходящееПисьмо") Тогда
		...
		ОбъектФайл.ownerType = "OutgoingMail";
	...
	ИначеЕсли ЗначениеЗаполнено(ДанныеФайла.Владелец) Тогда
		...
		ОбъектФайл.ownerType = "ActionTarget";

	Иначе
		ОбъектФайл.ownerType = "";
...
Функция ПолучитьXDTOИзСведенийОПрочтении(НаборЗаписей)
		
	XDTOСостояниеПрочтения = ОбменСМобильными_132.СоздатьОбъект("ObjectReadState");
...
Функция ПолучитьXDTOУдалениеОбъектаПоСсылке(Ссылка)
	...
	XDTOУдалениеОбъекта = ОбменСМобильными_132.СоздатьОбъект("ObjectDeletion");
...
Функция ПолучитьXDTOИзСинхронизацияПапокПисемСМобильнымКлиентом(Объект)
	...
	XDTOСинхронизируемыеПапки = ОбменСМобильными_132.СоздатьОбъект("MailFolderSynchronizationSettings");
	НаборПапки = РегистрыСведений.СинхронизацияПапокПисемСМобильнымКлиентом.СоздатьНаборЗаписей();
	НаборПапки.Отбор.Пользователь.Значение = Объект.Отбор.Пользователь.Значение;
...
Функция ПолучитьXDTOИзЗаписиРегистраПапкиПисем(ЗаписьРегистра)
	...
	XDTOПисьмоВПапке = ОбменСМобильными_132.СоздатьОбъект("ObjectInFolder");
...
Функция ПолучитьXDTOПоШаблонуТекстаПисьма(СсылкаШаблонТекста)
	...
	XDTOШаблонТекста = ОбменСМобильными_132.СоздатьОбъект("MailTextTemplate");
...
Функция ПолучитьXDTOИзНастроекШаблоновПисем(МобильныйКлиент)
	...
	XDTOНастройкиПодписи = ОбменСМобильными_132.СоздатьОбъект("MailFooterSettings");
...
Функция ПолучитьXDTOИзМаксимальногоРазмераФайла(Объект, ПараметрыСинхронизации, МобильныйКлиент)
	...
	XDTOМаксимальныйРазмерФайла = ОбменСМобильными_132.СоздатьОбъект("MaxFileSizeToDownloadSetting");
...
Функция ПолучитьXDTOПоСведениямОбОшибке(ИнформацияОбОшибке)
	...
	XDTOОшибка = ОбменСМобильными_132.СоздатьОбъект("LogRecord");
...
Функция ПолучитьXDTOПоПредметуПроцесса(СсылкаНаОбъект) 
	...
	XDTOОбщийПредметПроцесса = ОбменСМобильными_132.СоздатьОбъект("ActionTarget");
...
Функция ПолучитьXDTOПоЗаписиРабочегоКалендаря(Объект)

	XDTO = ОбменСМобильными_132.СоздатьОбъект("CalendarEvent");
	...
	Если Объект.ТипЗаписиКалендаря = Перечисления.ТипЗаписиКалендаря.ПовторяющеесяСобытие Тогда
		...
		Для Каждого ДеньПовторения Из Объект.ПовторениеПоДням Цикл
			...
			XDTOДеньПовторения = ОбменСМобильными_132.СоздатьОбъект("RecurrenceDay");
		...
		Для Каждого ИсключениеПовторения Из Объект.ИсключенияПовторения Цикл
			...
			XDTOИсключениеПовторения = ОбменСМобильными_132.СоздатьОбъект("RecurrenceException");
...
Функция ПолучитьXDTOПоНастройкеРабочегоКалендаря(НаборЗаписей)
	...
	XDTO = ОбменСМобильными_132.СоздатьОбъект("ObjectColor");
...
Функция ПолучитьXDTOПоПользователю(СсылкаПользователь)
	...
	Если Не ЗначениеЗаполнено(Реквизиты.Наименование) Тогда
		Возврат Неопределено;	
	КонецЕсли;
	XDTOПользователь = ОбменСМобильными_132.СоздатьОбъект("User");
...
Функция ПолучитьXDTOПоРоли(СсылкаРоль)
	...
	Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		СсылкаРоль,
		"Наименование");
	XDTOРоль = ОбменСМобильными_132.СоздатьОбъект("Role");
...
Процедура ПодготовитьЗадачуКВыгрузке(СсылкаЗадача, ОбъектыXDTO, ПараметрыСинхронизации, МобильныйКлиент)
	...
	XDTOЗадача = ОбменСМобильными_132.СоздатьОбъект("Task");
...
Процедура ДобавитьВариантИсполнения(
	...
	ВариантВыполнения = ОбменСМобильными_132.СоздатьОбъект("TaskExecutionResultVariant");
...
Процедура ДобавитьПредметыКЗадаче(XDTOЗадача, ОбъектыXDTO, СсылкаЗадача, ПараметрыСинхронизации, МобильныйКлиент)
	...
	Для Каждого ПредметЗадачи Из СсылкаЗадача.Предметы Цикл
		...
		XDTOПрикрепленныйПредмет = ОбменСМобильными_132.СоздатьОбъект("AttachedProcessTarget");
		...
		ИначеЕсли ТипЗнч(ПредметЗадачи.Предмет) = Тип("ДокументСсылка.ВходящееПисьмо") Тогда
			...
			XDTO = ПолучитьXDTOИзВходящегоПисьма(ПредметЗадачи.Предмет, МобильныйКлиент);
		...
		ИначеЕсли ТипЗнч(ПредметЗадачи.Предмет) = Тип("ДокументСсылка.ИсходящееПисьмо") Тогда
			...
			XDTO = ПолучитьXDTOИзИсходящегоПисьма(ПредметЗадачи.Предмет, МобильныйКлиент);
...
Функция ПолучитьXDTOПоПроцессуИсполнение(Объект, МассивXDTO, МобильныйКлиент) 
	...
	XDTOИсполнение = ОбменСМобильными_132.СоздатьОбъект("ExecutionProcess");
	...
	Для Каждого Исполнитель Из Объект.Исполнители Цикл
		Если ТипЗнч(Исполнитель.Исполнитель) = Тип("СправочникСсылка.Пользователи") Тогда
			XDTOИсполнитель = ОбменСМобильными_132.СоздатьОбъект("User");
		ИначеЕсли ТипЗнч(Исполнитель.Исполнитель) = Тип("СправочникСсылка.РолиИсполнителей") Тогда
			XDTOИсполнитель = ОбменСМобильными_132.СоздатьОбъект("Role");
		КонецЕсли;
...
Процедура ПолучитьXDTOВсехПодчиненныхФайлов(Владелец, ОбъектыXDTO, ПараметрыСинхронизации, МобильныйКлиент)
	...
	Для Каждого Файл Из Файлы Цикл
		XDTOФайл = ПолучитьXDTOИзФайла(Файл, МобильныйКлиент, ПараметрыСинхронизации, ПараметрыСинхронизации);
