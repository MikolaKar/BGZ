...
Функция СформироватьПодписиДляЗанесениюВБазу(ФайлСсылка, МассивФайловПодписей, УникальныйИдентификатор = Неопределено) Экспорт
	...
	Для Каждого Элемент Из МассивФайловПодписей Цикл
		...
		Если Сертификаты.Количество() <> 0 Тогда
			...
			ДанныеПодписи.Вставить("ОбъектСсылка", ФайлСсылка);
			...
			ДанныеПодписи.Вставить("Комментарий", Элемент.Комментарий);
			ДанныеПодписи.Вставить("ИмяФайлаПодписи", ИмяФайлаПодписи);
...
Процедура ОчиститьОсновнойРабочийКаталогПриЗавершенииРаботы() Экспорт
	...
	// ОчищатьВсе = Истина.
	РаботаСФайламиСлужебныйКлиент.ОчиститьРабочийКаталог(Неопределено, РазмерФайловВРабочемКаталоге, 0, Истина);
...
Процедура ВыбратьКартинкуИПоместитьВХранилище(ОбработчикРезультата, УникальныйИдентификатор) Экспорт
	...
	ПоказатьПредупреждение(ОписаниеОповещения, НСтр("ru='Выберите файл фотографии. 
...
// продолжение
Процедура ВыбратьКартинкуИПоместитьВХранилищеПродолжение(Параметры) Экспорт  
