...
Функция ЗаполнитьРегистрационныйШтамп(Объект, МакетПечати, ТабличныйДокумент, ДанныеОПоложении, ЗаголовокПриложения)
	...
	Макет = УправлениеПечатью.ПолучитьМакет("ОбщийМакет." + МакетПечати);
	...
	Если НЕ КартинкаШтрихкода = Неопределено 
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьУчетПоОрганизациям") Тогда
			...
			КонецЕсли;
		Иначе
			Надпись.Текст = Константы.НаименованиеПредприятия.Получить();
		КонецЕсли;
...
Функция ПолучитьШтрихкодНаНаклейке(ПараметрКоманды, ЗаголовокПриложения) Экспорт
	...
	Если ДанныеОШтрихкоде = Неопределено Тогда
		ТекстСообщения = НСтр("ru = 'Для файлов, находящихся в папке ""Шаблоны файлов"", а также для файлов, прикрепленных к должности или корреспонденту,
