...
// Заполнить корреспондента или контактное лицо данными личного адресата и заменить ссылки в письмах
Функция ЗаполнитьКонтактИЗаменитьСсылки(ЛичныйАдресат, Контакт, УникальныйИдентификатор = Неопределено,
	СтрокаОшибки = "") Экспорт
	
	Если ТипЗнч(Контакт) = Тип("СправочникСсылка.Корреспонденты") Тогда
			...
			Для Каждого КонтактИнфоАдресата Из ЛичныйАдресат.КонтактнаяИнформация Цикл
				...
				Для Каждого КонтактИнфоКорреспондента Из КорреспондентОбъект.КонтактнаяИнформация Цикл
					...
					Если Строка(КонтактИнфоКорреспондента.Вид) = Строка(КонтактИнфоАдресата.Вид) Тогда
						...
						Если НРег(КонтактИнфоКорреспондента.Представление) <> НРег(КонтактИнфоАдресата.Представление) Тогда
							СтрокаОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Нельзя выполнить преобразование, т.к. у личного адресата и корреспондента
									|разные адреса Email (%1 и %2).
									|Выберите другого корреспондента или создайте нового.'"),
									КонтактИнфоАдресата.Представление, КонтактИнфоКорреспондента.Представление);
							Возврат Ложь;
						КонецЕсли;	
