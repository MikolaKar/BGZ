...
Процедура ОбработатьXDTO(ОбъектXDTO, МобильныйКлиент, ПисьмаКОтправке, СвязиКУстановке) Экспорт
	...
	ИначеЕсли ОбъектXDTO.Тип().Имя = "Control" Тогда
		ЗаписатьКонтроль(ОбъектXDTO, МобильныйКлиент);
...
Процедура ПодключитьНовоеУстройство(ОбъектXDTO, МобильныйКлиент)
	...
	ОбменСМобильнымиРегистрацияИзменений.ЗарегистрироватьКонтрольДляПервичнойЗагрузки(УзелОбмена);
...
Процедура ОтключитьУстройство(МобильныйКлиент)
	...
	РегистрыСведений.ПротоколРаботыСМобильнымиКлиентами.ДобавитьИнформацию(
		НСтр("ru = 'Начало отключения мобильного клиента'"),
		Ложь,
		МобильныйКлиент);
...
Функция ПолучитьСсылкуИзXDTO(XDTOСсылка)

	Если XDTOСсылка = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;

	Если XDTOСсылка.type = "IncomingMail" Тогда
		Возврат Документы.ВходящееПисьмо.ПолучитьСсылку(XDTOСсылка.id);

	ИначеЕсли XDTOСсылка.type = "OutgoingMail" Тогда
		Возврат Документы.ИсходящееПисьмо.ПолучитьСсылку(XDTOСсылка.id);

	ИначеЕсли XDTOСсылка.type = "User" Тогда
		Возврат Справочники.Пользователи.ПолучитьСсылку(XDTOСсылка.id);

	ИначеЕсли XDTOСсылка.type = "Role" Тогда
		Возврат Справочники.РолиИсполнителей.ПолучитьСсылку(XDTOСсылка.id);

	ИначеЕсли XDTOСсылка.type = "ExecutionProcess" Тогда
		Возврат БизнесПроцессы.Исполнение.ПолучитьСсылку(XDTOСсылка.id);

	ИначеЕсли XDTOСсылка.type = "Task" Тогда
		Возврат Задачи.ЗадачаИсполнителя.ПолучитьСсылку(XDTOСсылка.id);

	ИначеЕсли XDTOСсылка.type = "CalendarEvent" Тогда
		Возврат Справочники.ЗаписиРабочегоКалендаря.ПолучитьСсылку(XDTOСсылка.id);

	ИначеЕсли XDTOСсылка.type = "File" Тогда
		Возврат Справочники.Файлы.ПолучитьСсылку(XDTOСсылка.id);

	ИначеЕсли XDTOСсылка.type = "MailThread" Тогда
		Возврат Справочники.ВеткиПереписки.ПолучитьСсылку(XDTOСсылка.id);

	ИначеЕсли XDTOСсылка.type = "Control" Тогда
		Возврат Справочники.Контроль.ПолучитьСсылку(XDTOСсылка.id);

	ИначеЕсли XDTOСсылка.type = "Addressee" Тогда
		Возврат Справочники.АдресатыПочтовыхСообщений.ПолучитьСсылку(XDTOСсылка.id);

	ИначеЕсли XDTOСсылка.type = "MailTextTemplate" Тогда
		ОбъектСсылка = Справочники.ШаблоныТекстовПисем.ПолучитьСсылку(XDTOСсылка.id);
...
Процедура ОбработатьУдалениеОбъекта(ОбъектXDTO, МобильныйКлиент)
	...
	ОбъектСсылка = ПолучитьСсылкуИзXDTO(ОбъектXDTO);
...
Процедура ЗаписатьФайл(ОбъектXDTO)
	...
	Если ОбъектXDTO.owner.type = "IncomingMail" Тогда
		ВладелецФайла = Документы.ВходящееПисьмо.ПолучитьСсылку(ОбъектXDTO.owner.id);

	ИначеЕсли ОбъектXDTO.owner.type = "OutgoingMail" Тогда
		ВладелецФайла = Документы.ИсходящееПисьмо.ПолучитьСсылку(ОбъектXDTO.owner.id);
...
Процедура ЗаписатьИсходящееПисьмо(ОбъектXDTO, ПисьмаКОтправке, СвязиКУстановке)
	...
	Если ОбъектXDTO.baseMail <> Неопределено Тогда
		ПисьмоОснованиеСсылка = Неопределено;
		Если ОбъектXDTO.baseMail.type = "IncomingMail" Тогда
			ПисьмоОснованиеСсылка = Документы.ВходящееПисьмо.ПолучитьСсылку(ОбъектXDTO.baseMail.id);
		ИначеЕсли ОбъектXDTO.baseMail.type = "OutgoingMail" Тогда
			ПисьмоОснованиеСсылка = Документы.ИсходящееПисьмо.ПолучитьСсылку(ОбъектXDTO.baseMail.id);
		КонецЕсли;
		...
		Если ПисьмоОснованиеСсылка <> Неопределено Тогда
			...
			Если ТипОтвета = Перечисления.ТипыОтвета.ОтветНаПисьмо Тогда
				РегистрыСведений.КешИнформацииОбОбъектах.УстановитьПризнак(ПисьмоОснованиеСсылка, "ОтправленОтвет", Истина);

			ИначеЕсли ТипОтвета = Перечисления.ТипыОтвета.ПересылкаПисьма Тогда
				РегистрыСведений.КешИнформацииОбОбъектах.УстановитьПризнак(ПисьмоОснованиеСсылка, "Переслан", Истина);
...
Процедура ЗаписатьАдресатаПочтовыхСообщений(ОбъектXDTO)
	...
	ПротоколированиеРаботыПользователей.ЗаписатьИзменение(Адресат.Ссылка);
	ИсторияРаботыПользователя.Добавить(ПолучитьНавигационнуюСсылку(Адресат.Ссылка));
...
Процедура ЗаписатьПеренаправлениеЗадачи(ОбъектXDTO, МобильныйКлиент)
	...
	Если ОбъектXDTO.newPerformer.type = "User" Тогда
		НовыйИсполнитель = Справочники.Пользователи.ПолучитьСсылку(ОбъектXDTO.newPerformer.id);
	ИначеЕсли ОбъектXDTO.newPerformer.type = "Role" Тогда
		НовыйИсполнитель = Справочники.РолиИсполнителей.ПолучитьСсылку(ОбъектXDTO.newPerformer.id);
	Иначе
		ОбменСМобильными.ВыполнитьРегистрациюЭлементаДанных(ЗадачаСсылка);
		Возврат;
	КонецЕсли;
...
Процедура ЗаписатьКонтроль(ОбъектXDTO, МобильныйКлиент)
	
	УстановитьПривилегированныйРежим(Истина);

	КонтрольСсылка                        = Справочники.Контроль.ПолучитьСсылку(ОбъектXDTO.id);
	...
	Если ОбъектНайденНаСервере 
		И КонтрольОбъект.ВерсияДанных <> ОбъектXDTO.dataVersion 
		И ЗначениеЗаполнено(ОбъектXDTO.dataVersion) Тогда

		ОбменСМобильными.ВыполнитьРегистрациюЭлементаДанных(КонтрольОбъект.Ссылка);
		ТекстПредупреждения = 
			"warning:" + 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Контроль ""%1"" не был передан, т.к. он был изменен на сервере. Настройка контроля на данном устройстве заменена версией с сервера.'"),
				ОбъектXDTO.name);
	...
	КонтрольОбъект.Автор               = МобильныйКлиент.Пользователь;
	КонтрольОбъект.ДатаПостановкиНаКонтроль = ОбъектXDTO.controlStartDate;
	КонтрольОбъект.ПоставленНаКонтроль = ОбъектXDTO.active;
	КонтрольОбъект.ДатаСнятияСКонтроля = ОбъектXDTO.controlEndDate;
	КонтрольОбъект.СнятСКонтроля       = ОбъектXDTO.deactivated;

	Если Не ОбъектНайденНаСервере Тогда
		СсылкаИсточник = ПолучитьСсылкуИзXDTO(ОбъектXDTO.source);
		Если СсылкаИсточник <> Неопределено Тогда
			Если ОбъектXDTO.source.type = "IncomingMail"
				Или ОбъектXDTO.source.type = "OutgoingMail" Тогда
	...
	Если ОбъектXDTO.controller <> Неопределено Тогда
		КонтрольОбъект.Контролер = ПолучитьСсылкуИзXDTO(ОбъектXDTO.controller);
	Иначе
		КонтрольОбъект.Контролер = МобильныйКлиент.Пользователь;
	КонецЕсли;

	КонтрольОбъект.Описание = ОбъектXDTO.description;

	Если Не ОбъектНайденНаСервере Тогда

		СсылкаПредмет = ПолучитьСсылкуИзXDTO(ОбъектXDTO.target);

		Если СсылкаПредмет <> Неопределено Тогда

			Если ОбъектXDTO.target.type = "IncomingMail"
			 Или ОбъектXDTO.target.type = "OutgoingMail" Тогда
	...
	КонтрольОбъект.СрокИсполнения = ОбъектXDTO.deadline;
	...
	Для Каждого XDTOИсполнитель Из ОбъектXDTO.performers Цикл
		...
		Для Каждого Строка Из КонтрольОбъект.Исполнители Цикл
			...
			Если Строка.Исполнитель <> ПолучитьСсылкуИзXDTO(XDTOИсполнитель.performer) Тогда
				Продолжить;
			КонецЕсли;

			Строка.ДатаКонтроля      = XDTOИсполнитель.controlEndDate;
			Строка.Проконтролировано = XDTOИсполнитель.taskControlled;
		...
		НоваяСтрока.Ответственный  = XDTOИсполнитель.main;
		НоваяСтрока.Исполнитель    = ПолучитьСсылкуИзXDTO(XDTOИсполнитель.performer);
		НоваяСтрока.Исполнено      = XDTOИсполнитель.taskIsExecuted;
		НоваяСтрока.ДатаИсполнения = XDTOИсполнитель.taskExecutionDate;

		Если Не ОбъектНайденНаСервере Тогда
			НоваяСтрока.Источник = ПолучитьСсылкуИзXDTO(XDTOИсполнитель.source);
		КонецЕсли;

		НоваяСтрока.ДатаКонтроля      = XDTOИсполнитель.controlEndDate;
		НоваяСтрока.Проконтролировано = XDTOИсполнитель.taskControlled;
	...
	Если НеобходимоЗарегистрироватьОбъектСразу Тогда
		ОбменСМобильными.ВыполнитьРегистрациюЭлементаДанных(КонтрольСсылка);
	КонецЕсли;
