...
Функция Выгрузить(Документ) Экспорт
	...
	Иначе
		ЗаписьXML.ЗаписатьАтрибут("from_organization", РаботаСоСтроками.УдалитьНедопустимыеСимволыXML(Константы.НаименованиеПредприятия.Получить()));
	КонецЕсли;	
			...
			ЗаписьXML.ЗаписатьНачалоЭлемента("Author");
				...
				Если ТипЗнч(Документ) = Тип("СправочникСсылка.ВходящиеДокументы") Тогда
					...
					Иначе
						...
						Если ПустаяСтрока(Документ.Отправитель.ПолноеНаименование) Тогда
						...
						Иначе
							ЗаписьXML.ЗаписатьАтрибут("organization_string", РаботаСоСтроками.УдалитьНедопустимыеСимволыXML(Строка(Документ.Отправитель.ПолноеНаименование)));
						КонецЕсли;
				...
				Иначе
					...
					ЗаписьXML.ЗаписатьНачалоЭлемента("OrganizationWithSign");
					Если ПолучитьФункциональнуюОпцию("ИспользоватьУчетПоОрганизациям") Тогда
						Если ПустаяСтрока(Документ.Организация.ПолноеНаименование) Тогда
							ЗаписьXML.ЗаписатьАтрибут("organization_string", РаботаСоСтроками.УдалитьНедопустимыеСимволыXML(Строка(Документ.Организация.Наименование)));
						Иначе	
							ЗаписьXML.ЗаписатьАтрибут("organization_string", РаботаСоСтроками.УдалитьНедопустимыеСимволыXML(Строка(Документ.Организация.ПолноеНаименование)));
						КонецЕсли;	
					Иначе
						ЗаписьXML.ЗаписатьАтрибут("organization_string", РаботаСоСтроками.УдалитьНедопустимыеСимволыXML(Константы.НаименованиеПредприятия.Получить()));
					КонецЕсли;	
			...
			Если НРег(Путь) = НРег("\Header\Document") Тогда
				Пока Файл.ПрочитатьАтрибут() Цикл
					...
					ИначеЕсли Файл.Имя = "type" Тогда
						...
						КонецЕсли;
						ДополнительноеСодержание = ДополнительноеСодержание + НСтр("ru = 'Тип документа у корреспондента: '");
					...
					ИначеЕсли Файл.Имя = "kind" Тогда
						Если Не ПустаяСтрока(ДополнительноеСодержание) Тогда
							ДополнительноеСодержание = ДополнительноеСодержание + Символы.ВК;
						КонецЕсли;
						ДополнительноеСодержание = ДополнительноеСодержание + НСтр("ru = 'Вид документа у корреспондента: '") + Файл.Значение;
					КонецЕсли;	
				КонецЦикла;
			КонецЕсли;
		...
		ИначеЕсли Файл.ТипУзла = ТипУзлаXML.Текст Тогда	
			...
			Если НРег(Путь) = НРег("\Header\Document\Confident") И ОграничениеДоступаКДокументу Тогда
				Если Не ПустаяСтрока(ДополнительноеСодержание) Тогда
					ДополнительноеСодержание = ДополнительноеСодержание + Символы.ВК;
				КонецЕсли;	
				ДополнительноеСодержание = ДополнительноеСодержание + НСтр("ru = 'Гриф корреспондента: '");
