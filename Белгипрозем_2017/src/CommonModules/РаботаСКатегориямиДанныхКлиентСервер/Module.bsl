...
Процедура ОтключитьКатегорииКакПараметры(Список, МаксимальноеКоличествоПараметров) Экспорт
	...
	ИмяПараметраИЛИ = "КатегорияИЛИ";
	ИмяПараметраИИЛИ = "КатегорияИИЛИ";
	ИмяПараметраНаличиеКатегорий = "НаличиеКатегорий";
	ПараметрНаличиеКатегорий = Список.Параметры.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных(ИмяПараметраНаличиеКатегорий)); 
	Если ПараметрНаличиеКатегорий <> Неопределено Тогда
		ПараметрНаличиеКатегорий.Использование = Ложь;
	КонецЕсли;
	Список.Параметры.УстановитьЗначениеПараметра("КатегорияИЛИ", Неопределено);
	Список.Параметры.УстановитьЗначениеПараметра("КатегорияИИЛИ", Неопределено);
	Список.Параметры.УстановитьЗначениеПараметра("КатегорияИЛИПередана", Ложь);
	Список.Параметры.УстановитьЗначениеПараметра("КатегорияИИЛИПередана", Ложь);
