...
Функция ПолучитьСообщениеОбОшибке(КодОшибки, СтороннееОписаниеОшибки = "") Экспорт
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	СообщенияЭДО.ДатаСоздания > &ДатаСоздания
		|	И НЕ СообщенияЭДО.Наименование ПОДОБНО ""%2AL%""
		|	И СообщенияЭДО.ТипЭлементаВерсииЭД = ЗНАЧЕНИЕ(Перечисление.ТипыЭлементовВерсииЭД.ПервичныйЭД)
		|	И СообщенияЭДО.СоглашениеЭД.СпособОбменаЭД = ЗНАЧЕНИЕ(Перечисление.СпособыОбменаЭД.ЧерезОператораЭДОТакском)";
