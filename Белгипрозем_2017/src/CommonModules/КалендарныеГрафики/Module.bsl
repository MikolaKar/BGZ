...
//1С-Минск	
    ...
    Если РазныеГода Тогда
        ...
        Пока ТекГод <> ГодОкончания Цикл
            ДатаКонецГода = Дата(ТекГод, 12, 31, 0, 0, 0); 
		    МассивДатНачала.Добавить(ДатаКонецГода);
	...
	|	И (КалендарныеГрафики.Год >= &ГодДатыНачала И КалендарныеГрафики.Год <= &ГодДатыОкончания
    ...
    КоличествоДнейДатыОкончания = 0;
    Для й=1 По ТаблицаДней.Количество()-1 Цикл
        КоличествоДнейДатыОкончания = КоличествоДнейДатыОкончания + ТаблицаДней[й].КоличествоДней;
    КонецЦикла; 
	...
	Возврат Сторно*(КоличествоДнейДатыОкончания - КоличествоДнейДатыНачала);
//1С-Минск	
...
Функция БлижайшийРабочийДень(Календарь="", ДляДаты) Экспорт
	ИскДата = '00010101000000';
	
	Если Не ЗначениеЗаполнено(Календарь) Тогда
		Календарь = Справочники.Календари.НайтиПоНаименованию("Производственный календарь");
	КонецЕсли; 
	
	Отбор = Новый Структура("Календарь", Календарь);
	Отбор.Вставить("Год", Год(ДляДаты)); 
	Отбор.Вставить("ДатаГрафика", ДляДаты); 
	
	СтруктураРесурсов = РегистрыСведений.КалендарныеГрафики.Получить(Отбор);
	Если СтруктураРесурсов.ДеньВключенВГрафик Тогда
		// это рабочий день
		Возврат ДляДаты;
	КонецЕсли; 
	
	// продолжение поиска рабочего дня
	Сч = 7; // для ограничения количества дней сплошных выходных
	ТекДата = ДляДаты;
	Пока Сч > 0 Цикл
		ТекДата = ТекДата + 24*3600;
		
		Отбор = Новый Структура("Календарь", Календарь);
		Отбор.Вставить("Год", Год(ТекДата)); 
		Отбор.Вставить("ДатаГрафика", ТекДата); 
	...
	Сообщение.Текст = "Не найдена ближайшая рабочая дата для "+Формат(ДляДаты, "ДФ=dd.MM.yyyy")+" Проверьте календарь.";
	Сообщение.Сообщить(); 
	Возврат ИскДата;
КонецФункции 
 
Функция ПредыдущийРабочийДень(Календарь="", ДляДаты) Экспорт
	
	Если Не ЗначениеЗаполнено(Календарь) Тогда
		График = ГрафикиРаботы.ПолучитьОсновнойГрафикРаботы();
		Календарь = График.Календарь;
	КонецЕсли; 
	...
	Сообщение = Новый СообщениеПользователю;
	Сообщение.Текст = "Не найдена предыдущая рабочая дата для "+Формат(ДляДаты, "ДФ=dd.MM.yyyy")+" Проверьте календарь.";
	Сообщение.Сообщить(); 
