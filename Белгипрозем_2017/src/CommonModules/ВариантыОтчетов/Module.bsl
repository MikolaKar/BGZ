...
Функция ОтчетыТекущегоПользователя() Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки") Тогда
		...
		Модуль.ПриДобавленииДополнительныхОтчетовДоступныхТекущемуПользователю(ДоступныеОтчеты);
	КонецЕсли;
...
Функция ДеревоНастроекВариантовОтчетовКонфигурации() Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("СтандартныеПодсистемы.ДатыЗапретаИзменения") Тогда
		...
		Модуль.ПриНастройкеВариантовОтчетов(ДеревоВариантов);
	КонецЕсли;
...
Функция СформироватьИнформациюОбОтчетеПоПолномуИмени(ОтчетПолноеИмя) Экспорт
	...
	Если Результат.ОтчетМетаданные = Неопределено Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки") Тогда
			...
			Модуль = ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ДополнительныеОтчетыИОбработки");
			Модуль.ПриОпределенииТипаИСсылкиЕслиОтчетДополнительный(Результат);
...
// Записывает в журнал регистрации событие отмены процедуры и фиксирует транзакцию.
//
Процедура ОтменаПроцедуры(ПредставлениеПроцедуры, Причина)
	ОтменитьТранзакцию();
	ИнформацияПоВарианту(Неопределено, НСтр("ru = 'Отмена процедуры ""%1"":%2'"), ПредставлениеПроцедуры, Символы.ПС + "  - " + Причина);
КонецПроцедуры
...
Процедура ФормаОтчетаПриСозданииНаСервере(ЭтаФорма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("СтандартныеПодсистемы.РассылкаОтчетов") Тогда
		Модуль = ОбщегоНазначенияКлиентСервер.ОбщийМодуль("РассылкаОтчетов");
		Модуль.ФормаОтчетаДобавитьКоманды(ЭтаФорма, Отказ, СтандартнаяОбработка);
	КонецЕсли;
...
Функция ПолучитьОбъектДополнительногоОтчета(ОтчетСсылка)
	...
	Если ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки") Тогда
		Модуль = ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ДополнительныеОтчетыИОбработки");
		ОтчетОбъект = Модуль.ПолучитьОбъектВнешнейОбработки(ОтчетСсылка);
	КонецЕсли;
...
Процедура ОбновитьОбщиеДанные(ПараметрыОбновления) Экспорт
	...
	Обработчики = ПараметрыОбновления.РазделенныеОбработчики;
	...
	Кэш.Вставить("ПараметрыОбновления", ПараметрыОбновления);
	Кэш.Вставить("ТребуетсяУстановитьМонопольныйРежим", Ложь);
	
	////////////////////////////////////////////////////////////////////////////////
	// 1. Заменить устаревшие ключи вариантов отчетов на актуальные.
	АктуализироватьКлючиПредопределенных(Кэш);
	
	Если Кэш.ТребуетсяУстановитьМонопольныйРежим Тогда
...
//
Процедура ОбновитьРазделенныеДанные(ПараметрыОбновления) Экспорт
	...
	ПометитьНаУдалениеВариантыУдаленныхОтчетов(Кэш, Ложь);
...
Процедура АктуализироватьКлючиПредопределенных(Кэш)
	...
	Если Выборка.Количество() > 0 И НЕ Кэш.ПараметрыОбновления.МонопольныйРежим Тогда
		Кэш.ТребуетсяУстановитьМонопольныйРежим = Истина;
		ОтменаПроцедуры(ПредставлениеПроцедуры, НСтр("ru = 'Требуется установка монопольного режима.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьНастройкиПредопределенных(Кэш)
	...
	Пока ВариантВыборка.Следующий() Цикл
		...
		Если ВариантПометкаУдаления <> ВариантВыборка.ПометкаУдаления Тогда
			Если НЕ Кэш.ПараметрыОбновления.МонопольныйРежим Тогда
				Кэш.ТребуетсяУстановитьМонопольныйРежим = Истина;
				ОтменаПроцедуры(ПредставлениеПроцедуры, НСтр("ru = 'Требуется установка монопольного режима.'"));
				Возврат;
			КонецЕсли;
	...
	Если Кэш.ПараметрыОбновления.МонопольныйРежим Тогда
