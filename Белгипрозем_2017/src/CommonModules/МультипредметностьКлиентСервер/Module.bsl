...
Процедура ЗаполнитьТаблицуПредметовФормы(Объект) Экспорт
	...
	Для Каждого СтрокаПредмета Из Объект.Предметы Цикл
		...
		Если СтрокаПредмета.Предмет <> Неопределено Тогда
			Попытка
				ПометкаУдаления = ОбщегоНазначения.ПолучитьЗначениеРеквизита(СтрокаПредмета.Предмет,"ПометкаУдаления");
			...
			КонецПопытки;
		КонецЕсли;
		СтрокаПредмета.Картинка = ИндексКартинкиРолиПредмета(СтрокаПредмета.РольПредмета, ПометкаУдаления);
	КонецЦикла;
	...
	Иначе 
		...
		Для Каждого СтрокаПредмета Из Предметы Цикл
			Если ЗначениеЗаполнено(СтрокаПредмета.Предмет) Тогда
				Если ТолькоОсновные И СтрокаПредмета.РольПредмета <> ПредопределенноеЗначение("Перечисление.РолиПредметов.Основной") Тогда
					Продолжить;
				КонецЕсли;
		...
		Если ТипЗнч(СтрокаПредмета) = Тип("Структура") Тогда
			Если ЗначениеЗаполнено(СтрокаПредмета.Предмет) Тогда
				СтрокаПредметов = СтрокаПредметов + ?(ПустаяСтрока(СтрокаПредметов),"",", ") 
					+ ?(Кавычки,"""","") + ОбщегоНазначенияДокументооборот.ПредметСтрокой(СтрокаПредмета.Предмет, СтрокаПредмета.ИмяПредмета) + ?(Кавычки,"""","");
			КонецЕсли;
		Иначе
			Если ЗначениеЗаполнено(СтрокаПредмета) Тогда
				СтрокаПредметов = СтрокаПредметов + ?(ПустаяСтрока(СтрокаПредметов),"",", ") 
					+ ?(Кавычки,"""","") + ОбщегоНазначенияДокументооборот.ПредметСтрокой(СтрокаПредмета) + ?(Кавычки,"""","");
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьНаименованиеСПредметами(ШаблонНаименования, Предметы) Экспорт
	...
	ПредставлениеПредметов = ПредметыСтрокой(Предметы, Истина, Ложь, Истина);
...
Функция ПолучитьМассивПредметовОбъекта(ОбъектСсылка, ТипыПредметов = Неопределено, Основные = Неопределено) Экспорт
	...
	ИначеЕсли ОбщегоНазначения.ЭтоСсылка(ТипЗнч(ОбъектСсылка)) Тогда
...
//	ЭтоКомплексныйПроцесс - признак копмлексного процесса
//
...
Процедура ИзменитьПредметПоРезультатуВыбора(Форма, Объект, Результат, СтрокаПредмета = Неопределено) Экспорт
	...
	Для Каждого РезультатВыбора Из ДанныеВыбора Цикл
			...
			Если ЗначениеЗаполнено(СтрокаДанных.ИсходноеИмяПредмета) Тогда
				...
				Если ЭтоКомплексныйПроцесс Тогда
					...
					Для Каждого СтрокаШаблона Из Объект.Этапы Цикл
						ШаблоныЭтапов.Добавить(СтрокаШаблона.ШаблонБизнесПроцесса);
					КонецЦикла;
					МультипредметностьВызовСервера.ЗаменитьИмяПредметаВШаблонахПроцесса(
						Объект.Ссылка, ШаблоныЭтапов, СтрокаДанных.ИсходноеИмяПредмета, СтрокаДанных.ИмяПредмета);
				КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(СтрокаДанных.Предмет) Тогда
			ПометкаУдаления = Ложь;
		Иначе
			ПометкаУдаления = ОбщегоНазначения.ПолучитьЗначениеРеквизита(СтрокаДанных.Предмет,"ПометкаУдаления");
		КонецЕсли;
		...
		Если ЭтоКомплексныйПроцесс Тогда
			...
			Если Добавление Тогда
				МультипредметностьВызовСервера.ДобавитьПредметВШаблоныПроцесса(
					Объект.Ссылка, ШаблоныЭтапов, Объект.ПредметыЗадач, СтруктураПредмета);
			Иначе
				РолиПредметовЭтапов = Новый Массив;
				
				МультипредметностьВызовСервера.ОбновитьШаблоныПодДаннымПредметовЗадач(
					Объект.Ссылка, ШаблоныЭтапов, Объект.ПредметыЗадач, СтруктураПредмета, РолиПредметовЭтапов);
