...
Процедура ДополнитьСоответствиеТипов(Таблица) Экспорт
	...
	ОбработкаЗапросовXDTO.ДобавитьСтроку(Таблица, "DMComplexBusinessProcessTemplate", "Справочник.ШаблоныКомплексныхБизнесПроцессов");
	ОбработкаЗапросовXDTO.ДобавитьСтроку(Таблица, "DMComplexBusinessProcess", "БизнесПроцесс.КомплексныйПроцесс");
	
	ОбработкаЗапросовXDTO.ДобавитьСтроку(Таблица, "DMBusinessProcessInvitationTemplate", "Справочник.ШаблоныПриглашения");
	ОбработкаЗапросовXDTO.ДобавитьСтроку(Таблица, "DMBusinessProcessInvitation", "БизнесПроцесс.Приглашение");
	ОбработкаЗапросовXDTO.ДобавитьСтроку(Таблица, "DMBusinessProcessInvitationTaskInvitation", "Задача.ЗадачаИсполнителя");
	ОбработкаЗапросовXDTO.ДобавитьСтроку(Таблица, "DMBusinessProcessInvitationTaskAcquaint", "Задача.ЗадачаИсполнителя");
	ОбработкаЗапросовXDTO.ДобавитьСтроку(Таблица, "DMBusinessProcessInvitationTaskNotify", "Задача.ЗадачаИсполнителя");
...
Функция ПолучитьБизнесПроцесс(Узел, ОбъектИд, ОбъектБизнесПроцесс) Экспорт
	...
	Если ОбъектИд.type = "DMBusinessProcessInvitation" Тогда
		Ответ = ОбработкаЗапросовXDTOКорп.ПолучитьБППриглашение(ОбъектБизнесПроцесс);
...
Функция ПолучитьHTMLПредставлениеФормыЗадачи(СсылкаНаЗадачу, ИмяФормы) Экспорт
	...
	Иначе
		ОсновнойОбъектАдресации = ?(ЗначениеЗаполнено(СсылкаНаЗадачу.ОсновнойОбъектАдресации), Строка(СсылкаНаЗадачу.ОсновнойОбъектАдресации), "");
		ДополнительныйОбъектАдресации = ?(ЗначениеЗаполнено(СсылкаНаЗадачу.ДополнительныйОбъектАдресации), Строка(СсылкаНаЗадачу.ДополнительныйОбъектАдресации), "");
		СтрокаИсполнитель = Строка(СсылкаНаЗадачу.РольИсполнителя) + "(" + ОсновнойОбъектАдресации + ", " + ДополнительныйОбъектАдресации + ")";
		СтрокаИсполнитель = СтрЗаменить(СтрокаИсполнитель, "(, )", "");
		СтрокаИсполнитель = СтрЗаменить(СтрокаИсполнитель, ", )", ")");
