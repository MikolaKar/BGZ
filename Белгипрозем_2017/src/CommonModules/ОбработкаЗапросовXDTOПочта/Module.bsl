...
Функция ПолучитьНовоеПисьмо(Сообщение) Экспорт
	...
	Если ДелопроизводствоКлиентСервер.ЭтоКорреспондент(Предмет) Тогда
		СписокПочтовыхАдресов = ПолучитьСписокПочтовыхАдресовКорреспондента(Предмет);
...
Функция ПолучитьДеревоПисем(Сообщение) Экспорт
	...
	Попытка
		...
		Для каждого Элемент Из МассивОбъектовДляОтбора Цикл
			Если ТипЗнч(Элемент) = Тип("СправочникСсылка.Корреспонденты") Тогда
				ЗаполнитьПерепискуСКорреспондентом(Ответ.followers, Элемент);
			Иначе
...
Функция ПолучитьАдреснуюКнигу(Сообщение) Экспорт
	...
	Попытка
		...
		ИначеЕсли  ВРег(Сообщение.addressBookType) = ВРег("correspondents") Тогда
			ЗаполнитьКорреспондентов(Ответ, Сообщение.parentID);
		КонецЕсли;
...
//
// Параметры:
//   ТипОбъектов - Строка - тип объекта, например, "Корреспонденты"
...
// Заполняет ответ на запрос DMGetAddressBookRequest деревом пользователей по подразделениям 
//
// Параметры:
...
Процедура ЗаполнитьПользователейВПодразделениях(Ответ, РодительID)
	...
	Запрос.Текст = 
		...
		|				И (СведенияОПользователях.Подразделение = &Родитель)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		|		СведенияОПользователях.Пользователь,
		|		СведенияОПользователях.Пользователь.ПредставлениеВПерепискеСРангом,
		|		ЛОЖЬ
		|	ИЗ
		|		РегистрСведений.СведенияОПользователях КАК СведенияОПользователях
		|	ГДЕ
		|		СведенияОПользователях.Подразделение = &Родитель
		|		И НЕ СведенияОПользователях.Пользователь.Недействителен) КАК Данные
		|
		|УПОРЯДОЧИТЬ ПО
		|	ЭтоГруппа УБЫВ,
		|	Наименование";
...
// Заполняет ответ на запрос DMGetAddressBookRequest деревом пользователей по группам пользователей .
//
// Параметры:
//   Ответ - ОбъектXDTO типа DMGetAddressBookResponse, неявно возвращаемое значение
//   РодительID - Строка - идентификатор группы в справочнике ГруппыПользователей 
//
Процедура ЗаполнитьПользователейВГруппах(Ответ, РодительID)
	
	Запрос = Новый Запрос;
	
	Родитель = Справочники.ГруппыПользователей.ПустаяСсылка();
	Если НЕ ПустаяСтрока(Строка(РодительID)) Тогда
		Родитель = Справочники.ГруппыПользователей.ПолучитьСсылку(Новый УникальныйИдентификатор(РодительID));
	КонецЕсли;
	
	Если РодительID = Строка(Справочники.ГруппыПользователей.ВсеПользователи.УникальныйИдентификатор()) Тогда
		Запрос.Текст = 
			"ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	Данные.Ссылка,
			|	Данные.Наименование КАК Наименование,
	...
	Иначе
		Запрос.Текст = 
			...
			|		И ГруппыПользователей.Ссылка <> ЗНАЧЕНИЕ(Справочник.ГруппыПользователей.ВсеПользователи)
			|	
			|	ОБЪЕДИНИТЬ ВСЕ
			|	
			|	ВЫБРАТЬ
			|		ГруппыПользователейСостав.Пользователь,
			|		ГруппыПользователейСостав.Пользователь.ПредставлениеВПерепискеСРангом,
			|		ЛОЖЬ
			|	ИЗ
			|		Справочник.ГруппыПользователей.Состав КАК ГруппыПользователейСостав
			|	ГДЕ
			|		ГруппыПользователейСостав.Ссылка = &Родитель
			|	
			|	ОБЪЕДИНИТЬ ВСЕ
			|	
			|	ВЫБРАТЬ РАЗЛИЧНЫЕ
			...
			|			ПО ИсполнителиЗадач.Исполнитель = ГруппыПользователейСостав.Пользователь
			|				И (ГруппыПользователейСостав.Ссылка = &Родитель)) КАК Данные
			|
			|УПОРЯДОЧИТЬ ПО
			|	ЭтоГруппа УБЫВ,
			|	Наименование";
	...
	Если НЕ ЗначениеЗаполнено(Родитель) Тогда
		ЭлементXDTO = ОбработкаЗапросовXDTO.СоздатьОбъект("DMAddressBookItem");
		ОбъектXDTO = ОбработкаЗапросовXDTO.СоздатьОбъект("DMObject");
		ОбъектXDTO.name = Справочники.ГруппыПользователей.ВсеПользователи.Наименование;
		ОбъектXDTO.objectID = ОбработкаЗапросовXDTO.СоздатьОбъект("DMObjectID");
		ОбъектXDTO.objectID.id = Строка(Справочники.ГруппыПользователей.ВсеПользователи.УникальныйИдентификатор());
...
// Заполняет ответ на запрос DMGetAddressBookRequest деревом корреспондентов.
//
// Параметры:
//   Ответ - ОбъектXDTO типа DMGetAddressBookResponse, неявно возвращаемое значение
//   РодительID - Строка - идентификатор группы в справочнике Корреспонденты 
//
Процедура ЗаполнитьКорреспондентов(Ответ, РодительID)
	
	Запрос = Новый Запрос;
	
	Родитель = Справочники.Корреспонденты.ПустаяСсылка();
	Если НЕ ПустаяСтрока(Строка(РодительID)) Тогда
		Родитель = Справочники.Корреспонденты.ПолучитьСсылку(Новый УникальныйИдентификатор(РодительID));
	КонецЕсли;
	
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	СправочникКорреспонденты.Ссылка,
	|	СправочникКорреспонденты.Наименование КАК Наименование,
	|	СправочникКорреспонденты.ЭтоГруппа
	|ИЗ
	|	Справочник.Корреспонденты КАК СправочникКорреспонденты
	|ГДЕ
	|	НЕ СправочникКорреспонденты.ПометкаУдаления
	|	И СправочникКорреспонденты.Родитель = &Родитель
	|
	|УПОРЯДОЧИТЬ ПО
	|	Наименование";
	Запрос.УстановитьПараметр("Родитель", Родитель);
	...
	ЭлектронныеАдреса = ПолучитьЭлектронныеАдреса("Корреспонденты",Справочники.ВидыКонтактнойИнформации.EmailКорреспондента);
		...
		Для Каждого Файл Из ОбъектXDTO.files Цикл
			...
			Иначе
				...
				ФайлСсылка = РаботаСФайламиВызовСервера.СоздатьФайлСВерсией(
					...
					Файл.name,
					Файл.extension,
					Файл.modificationDate,
					Файл.modificationDateUniversal,
					Файл.size,
...
//
Функция ТаблицаПолучателейВСтроку(Знач ТаблицаПолучателей)
...
// Возвращает список адресов электронной почты указанного корреспондента
//
// Параметры:
...
//         * Контакт - ссылка на корреспондента
//         * ОтображаемоеИмя - представление корреспондента
...
Процедура ПередатьОбщиеРеквизитыПисьма(Узел, СсылкаНаПисьмо, Ответ, Предмет = Неопределено)
	...
	Если СсылкаНаПисьмо.Предмет <> Неопределено Тогда
		...
		ВнешнийОбъект = ОбработкаЗапросовXDTO.ПолучитьСсылкуНаВнешнийОбъект(Узел, СсылкаНаПисьмо.Предмет);
		Если ЗначениеЗаполнено(ВнешнийОбъект) Тогда
			...
			ПредметXDTO.externalObject = ВнешнийОбъектXDTO;
		КонецЕсли;
		Ответ.target = ПредметXDTO;
	КонецЕсли;
	
	// Не нужно получать представление для новых исходящих писем.
...
Процедура ЗаполнитьПредметПисьма(Узел, ПисьмоXDTO, СсылкаНаПредмет)
	...
	ВнешнийОбъект = ОбработкаЗапросовXDTO.ПолучитьСсылкуНаВнешнийОбъект(Узел, СсылкаНаПредмет);
	Если ЗначениеЗаполнено(ВнешнийОбъект) Тогда
		ВнешнийОбъектXDTO = ОбработкаЗапросовXDTO.СоздатьОбъект("ExternalObject");
		ВнешнийОбъектXDTO.id = ВнешнийОбъект.id;
		ВнешнийОбъектXDTO.type = ВнешнийОбъект.type;
...
// Заполняет переписку с указанным корреспондентом
//
// Параметры:
//   СтрокиXDTO - СписокXDTO - список документов для заполнения, неявно возвращаемый параметр
//   Корреспондент - СправочникСсылка.Корреспонденты
//
Процедура ЗаполнитьПерепискуСКорреспондентом(СтрокиXDTO, Корреспондент)
	...
	Для Каждого Строка Из ТаблДокументов Цикл
		...
		ЗаполнитьПодчиненныеДокументыПоКорреспонденту(СтрокаXDTO.followers, Строка);
...
// Добавляет подчиненные строки к документам переписки с Корреспондентом
//
// Параметры:
//   СтрокиXDTO - СписокXDTO - список документов для заполнения, неявно возвращаемый параметр
//   Строка - Строка выборки документов с реквизитом Ссылка
//
Процедура ЗаполнитьПодчиненныеДокументыПоКорреспонденту(СтрокиXDTO, СтрокаДерева)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.ВходящиеДокументы")  Тогда
			СтруктураРеквизитов = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(Ссылка,"Отправитель, Подписал");
			СтрокаXDTO.sent = ПолучитьПризнакОтправлен(
				Выборка.Ссылка, СтруктураРеквизитов.Отправитель, СтруктураРеквизитов.Подписал);
		КонецЕсли;
		
		ЗаполнитьПодчиненныеДокументыПоКорреспонденту(СтрокаXDTO.followers, Выборка);
...
//   Получатель - СправочникСсылка.Корреспонденты
