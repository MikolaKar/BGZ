...
Процедура СформироватьЗаголовокФормы()
	...
	Иначе
		ЭтаФорма.Заголовок = НСтр("ru = 'Ежедневный отчет за'") + " " + Формат(Объект.Дата, "ДФ=dd.MM.yyyy");
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УправлениеСвойствами.ПриСозданииНаСервере(ЭтаФорма, Объект, "ГруппаМиниАнкета");
...
Процедура ОбработкаВыбораРаботы(РезультатВыбора, Строка)
	...
	Если Объект.СпособУказанияВремени = ЗначениеПеречисленияВремяНачала И Не ЗначениеЗаполнено(Строка.Начало) Тогда 
