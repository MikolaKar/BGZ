...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Не Ссылка.Пустая() Тогда
		...
		ЗначенияРеквизитов = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(Ссылка, "ПометкаУдаления, Папка");
...
Процедура ЗаполнитьСтруктуруРеквизитовПоВнутреннемуДокументу(СтруктураРеквизитов, Знач Документ)
	...
	РеквизитыДокумента = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(Документ, "Заголовок, Проект, ВидДокумента.ВестиУчетПоКорреспондентам");
	...
	Если РеквизитыДокумента.ВидДокументаВестиУчетПоКорреспондентам = Истина Тогда 
		СтруктураРеквизитов.Вставить("ПолучателиПисьма", Новый Массив);
		
		Для Каждого КорреспондентыСтрока Из Документ.Корреспонденты Цикл 
			
			Если ЗначениеЗаполнено(КорреспондентыСтрока.КонтактноеЛицо) Тогда
				АдресатАдрес = ПолучитьАдресЭлектроннойПочты(КорреспондентыСтрока.КонтактноеЛицо);
				
				Если ЗначениеЗаполнено(АдресатАдрес) Тогда
					АдресатПредставление = Строка(КорреспондентыСтрока.КонтактноеЛицо);
			...
			Если ЗначениеЗаполнено(КорреспондентыСтрока.ПодписалОтКорреспондента) Тогда
				АдресатАдрес = ПолучитьАдресЭлектроннойПочты(КорреспондентыСтрока.ПодписалОтКорреспондента);
				
				Если ЗначениеЗаполнено(АдресатАдрес) Тогда
					АдресатПредставление = Строка(КорреспондентыСтрока.ПодписалОтКорреспондента);
					Адресат = ВстроеннаяПочтаСерверПовтИсп.ПолучитьПочтовогоАдресата(
						АдресатАдрес, АдресатПредставление);
			...
			Если ЗначениеЗаполнено(КорреспондентыСтрока.Корреспондент) Тогда
				АдресатАдрес = ПолучитьАдресЭлектроннойПочты(КорреспондентыСтрока.Корреспондент);
				
				Если ЗначениеЗаполнено(АдресатАдрес) Тогда
					АдресатПредставление = Строка(КорреспондентыСтрока.Корреспондент);
					Адресат = ВстроеннаяПочтаСерверПовтИсп.ПолучитьПочтовогоАдресата(
						АдресатАдрес, АдресатПредставление);
...
Процедура ЗаполнитьСтруктуруРеквизитовПоВходящемуДокументу(СтруктураРеквизитов, Знач Документ)
	...
	РеквизитыДокумента = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(Документ, "Заголовок, Подписал, Проект");
	СтруктураРеквизитов.Вставить("Тема", РеквизитыДокумента.Заголовок);
	СтруктураРеквизитов.Вставить("ПолучателиПисьма", Новый Массив);
	СтруктураРеквизитов.Вставить("Проект", РеквизитыДокумента.Проект);
	...
	Если ЗначениеЗаполнено(РеквизитыДокумента.Подписал) Тогда
		...
		АдресатАдрес = ПолучитьАдресЭлектроннойПочты(РеквизитыДокумента.Подписал);
...
Процедура ЗаполнитьСтруктуруРеквизитовПоИсходящемуДокументу(СтруктураРеквизитов, Знач Документ)
	...
	РеквизитыДокумента = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(Документ, "Заголовок, Проект");
	СтруктураРеквизитов.Вставить("Тема", РеквизитыДокумента.Заголовок);
	СтруктураРеквизитов.Вставить("ПолучателиПисьма", Новый Массив);
	СтруктураРеквизитов.Вставить("Проект", РеквизитыДокумента.Проект);
	СтруктураРеквизитов.Вставить("Предмет", Неопределено);
	...
	Для каждого ПолучателиСтрока Из Документ.Получатели Цикл
		...
		АдресКонтактноеЛицо = ПолучитьАдресЭлектроннойПочты(КонтактноеЛицо);
		Если ЗначениеЗаполнено(АдресКонтактноеЛицо) Тогда
			
			Контакт = КонтактноеЛицо;
			Адрес = АдресКонтактноеЛицо;
			
		Иначе
			
			Корреспондент = ПолучателиСтрока.Получатель;
			АдресКорреспондент = ПолучитьАдресЭлектроннойПочты(Корреспондент);
			
			Если ЗначениеЗаполнено(АдресКорреспондент) Тогда
				Контакт = Корреспондент;
				Адрес = АдресКорреспондент;
			КонецЕсли;
		...
		Если ЗначениеЗаполнено(Адрес) Тогда
			...
			Адресат = ВстроеннаяПочтаСерверПовтИсп.ПолучитьПочтовогоАдресата(Адрес, Строка(Контакт));
			СтруктураАдресата = Новый Структура;
			СтруктураАдресата.Вставить("Адресат", Адресат);
...
Процедура ЗаполнитьСтруктуруРеквизитовПоПроекту(СтруктураРеквизитов, Знач Проект)
	...
	РеквизитыПроекта = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(
	...
	Пока ПроектнаяКоманда.Следующий() Цикл
		...
		Адрес = ПолучитьАдресЭлектроннойПочты(ПроектнаяКоманда.Исполнитель);
...
Процедура ЗаполнитьСтруктуруРеквизитовПоФайлу(СтруктураРеквизитов, Знач Файл)
	...
	РеквизитыФайла = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(Файл, "Наименование, Проект");
...
Процедура ЗаполнитьСтруктуруРеквизитовПоЗадачеИсполнителя(СтруктураРеквизитов, Знач ЗадачаИсполнителя)
	...
	РеквизитыЗадачиИсполнителя = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(
...
Процедура ЗаполнитьСтруктуруРеквизитовПоСообщению(СтруктураРеквизитов, Знач Сообщение)
	...
	ВладелецСообщения = ОбщегоНазначения.ПолучитьЗначениеРеквизита(Сообщение, "ВладелецСообщения");
...
Процедура ЗаполнитьСтруктуруРеквизитовПоУведомлениюПрограммы(
	...
	Описание = ОбщегоНазначения.ПолучитьЗначениеРеквизита(УведомлениеПрограммы, "Описание");
	СтруктураРеквизитов.Вставить("Тема", СтрПолучитьСтроку(Описание,1));
	СтруктураРеквизитов.Вставить("Тема", СтрПолучитьСтроку(Описание,1));
...
Процедура ЗаполнитьСтруктуруРеквизитовПоКонтролюОбъектов(СтруктураРеквизитов, Знач Контроль)
	...
	ПредметКонтроля = ОбщегоНазначения.ПолучитьЗначениеРеквизита(Контроль, "Предмет");	
	Если ЗначениеЗаполнено(ПредметКонтроля) Тогда 
		...
		ИначеЕсли ТипЗнч(ПредметКонтроля) = Тип("СправочникСсылка.ВеткиПереписки") Тогда 
			...
			КорневоеПисьмо = ОбщегоНазначения.ПолучитьЗначениеРеквизита(ПредметКонтроля, "КорневоеПисьмо");
...
Функция ПолучитьАдресЭлектроннойПочты(Корреспондент)
