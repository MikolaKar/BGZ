...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СпособРедактированияТекстаПисьмаПолеВвода = Перечисления.СпособРедактированияТекстаПисьма.ПолеВвода;
	...
	Иначе
		Если СпособРедактированияТекстаПисьма = Перечисления.СпособРедактированияТекстаПисьма.ПолеВвода Тогда
			Элементы.ТекстПисьма.АктивизироватьПоУмолчанию = Истина;
		Иначе
			Элементы.ТекстПисьмаКакТекстовыйДокумент.АктивизироватьПоУмолчанию = Истина;
		КонецЕсли;
	КонецЕсли;
	...
	Иначе
		...
		Иначе
			Элементы.ГруппаСтраницаПолеТекстовогоДокумента.Видимость = Истина;
			Элементы.ГруппаСтраницыТекстаПисьма.ТекущаяСтраница = Элементы.ГруппаСтраницаПолеТекстовогоДокумента;
			Элементы.ФормаНеПереноситьСтроки.Пометка = Истина;
		КонецЕсли;
	КонецЕсли;
	...
	Если ПисьмоОтправлено Тогда
		...
		Элементы.ПроверитьОрфографию1.Доступность = Ложь;
		...
		Элементы.ФормаНеПереноситьСтроки.Доступность = Ложь;
...
Процедура ВычислитьСпособРедактированияТекста()
	...
	СпособРедактированияТекстаПисьма =
		ВстроеннаяПочтаСерверПовтИсп.ПолучитьПерсональнуюНастройку("СпособРедактированияТекстаПисьма");
		
	Если СпособРедактированияТекстаПисьма = Перечисления.СпособРедактированияТекстаПисьма.Авто Тогда
			
		ЧислоСимволовПисьмаДляСпособаРедактирования =
			ВстроеннаяПочтаСерверПовтИсп.ПолучитьПерсональнуюНастройку("ЧислоСимволовПисьмаДляСпособаРедактирования");
			
		Если СтрДлина(ТекстПисьма) > ЧислоСимволовПисьмаДляСпособаРедактирования Тогда
			СпособРедактированияТекстаПисьма = Перечисления.СпособРедактированияТекстаПисьма.ПолеТекстовогоДокумента;
		Иначе
	...
	Если Объект.ТипТекста = Перечисления.ТипыТекстовПочтовыхСообщений.HTML Тогда
		...
		Элементы.ФормаНеПереноситьСтроки.Пометка = Ложь;
	...
	Если Параметры.Свойство("ЗначенияЗаполнения")
		...
		Иначе
			...
			Если ФорматОтветныхИПересылаемыхПисем = ПредопределенноеЗначение("Перечисление.ФорматыОтветныхИПересылаемыхПисем.ФорматИсходногоПисьма") Тогда
				ТипТекста = ОбщегоНазначения.ПолучитьЗначениеРеквизита(Письмо, "ТипТекста");
	...
	ИначеЕсли Параметры.Свойство("Файлы") Тогда
		...
		Если ТипЗнч(Параметры.Файлы) = Тип("Массив") И Параметры.Файлы.Количество() > 0 Тогда
			...
			Для каждого Файл Из Параметры.Файлы Цикл
				...
				ДанныеФайла = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(Файл,
				...
				ВложенияСтрока.Представление = ДанныеФайла.Наименование + ДанныеФайла.ТекущаяВерсияРасширение;
	...
	Если Параметры.Свойство("ВосстановлениеПисьма")
		...
		Если Найти(Параметры.ВосстановлениеПисьма.Расширение, "txt") <> 0 Тогда
			...
			Если Объект.ТипТекста = Перечисления.ТипыТекстовПочтовыхСообщений.HTML Тогда
				...
				СпособРедактированияТекстаПисьма = ПредопределенноеЗначение("Перечисление.СпособРедактированияТекстаПисьма.ПолеВвода");
...
Процедура ПриОткрытии(Отказ)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка)
		И Объект.ТипТекста = ПредопределенноеЗначение("Перечисление.ТипыТекстовПочтовыхСообщений.HTML") Тогда
		
		ПодключитьОбработчикОжидания("УстановитьПозициюHTMLВНачало", 0.3, Истина);
	...
	Для Каждого Строка Из СписокДобавленныхФайлов Цикл
		...
		Иначе
			...
			ВложениеПисьмаСсылка = РаботаСФайламиВызовСервера.СоздатьФайлСВерсией(
				ТекущийОбъект.Ссылка, // ВладелецФайла
				ИмяФайлаИнфо.Имя, // ИмяБезРасширения,
				ИмяФайлаИнфо.Расширение, // РасширениеБезТочки
				ОписаниеФайла.ВремяИзменения,
				ВремяИзмененияУниверсальное,
				ОписаниеФайла.Размер,
				ОписаниеФайла.Адрес, //АдресВременногоХранилищаФайла
				АдресВременногоХранилищаТекста,
				Ложь, // ЭтоВебКлиент
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_Файл" И Параметр.Событие = "ДанныеФайлаИзменены"
		...
		Если Объект.ТипТекста = ПредопределенноеЗначение("Перечисление.ТипыТекстовПочтовыхСообщений.ПростойТекст") Тогда
			Если СпособРедактированияТекстаПисьма = ПредопределенноеЗначение("Перечисление.СпособРедактированияТекстаПисьма.ПолеВвода") Тогда
...
&НаКлиенте
Процедура ПересылкаПисьмаСтрокаНажатие(Элемент, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	Если ПересылкаПисьма <> Неопределено Тогда
		ПоказатьЗначение(,ПересылкаПисьма);
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура ОтправленоВОтветНаСтрокаНажатие(Элемент, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	Если ЗначениеЗаполнено(ПисьмоОтправленоВОтветНа) Тогда
		ПоказатьЗначение(,ПисьмоОтправленоВОтветНа);
	ИначеЕсли ЗначениеЗаполнено(Объект.ПисьмоОснование) Тогда
		ПоказатьЗначение(,Объект.ПисьмоОснование);
	КонецЕсли;
...
Процедура ПолученОтветСтрокаНажатие(Элемент, СтандартнаяОбработка)
	...
	Если ПолученОтвет.Количество() = 1 Тогда
		ПоказатьЗначение(,ПолученОтвет[0].Значение);
	Иначе
		ВстроеннаяПочтаКлиент.ОткрытьДеревоПереписки(Объект.Ссылка);
	КонецЕсли;
...
Процедура ПредметОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Тип") Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ПослеВводаПредмета", ЭтотОбъект);
...
Процедура ПредметНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ЗавершениеНачалаВыбораПредмета", 
		ЭтотОбъект, 
...
Процедура УчетнаяЗаписьПроверитьПараметрыПочты()
	...
	Если ЗначениеЗаполнено(Объект.УчетнаяЗапись)
		...
		И ЗначениеЗаполнено(Объект.УчетнаяЗапись.СерверИсходящейПочтыSMTP) Тогда
...
Процедура ОпределитьВидимостьДоступностьЭлементовФормы()
	...
	Если ПисьмоОтправлено Тогда
		...
		Элементы.ФормаРедактироватьЦитирование.Видимость = Ложь;
		Элементы.ФормаРедактироватьЦитирование.Доступность = Ложь;
	...
	Иначе
		...
		Элементы.ФормаРедактироватьЦитирование.Видимость = Истина;
		Элементы.ФормаРедактироватьЦитирование.Доступность = Истина;
...
Процедура ВыбратьИзАдреснойКниги(ПараметрМножественныйВыбор = Неопределено)
	...
	ПараметрыОткрытия.Вставить("ТекущийКонтакт", Контакт);
	ПараметрыОткрытия.Вставить("ЗаполненыПараметрыEmailУчетнойЗаписи", ЗаполненыПараметрыEmailУчетнойЗаписи);
	...
	Если Не МножественныйВыбор Тогда
		...
		Если Результат.Количество() > 0 Тогда
			...
			Если ТипЗнч(Результат[0].Контакт) = Тип("СправочникСсылка.Пользователи") Тогда
				Представление = ОбщегоНазначения.ПолучитьЗначениеРеквизита(
					Результат[0].Контакт,
...
&НаСервере
Функция ВПолучателяхУжеЕстьКонтакт(Контакт, Адрес, НайденнаяСтрока)
	
	Для Каждого Строка Из Получатели Цикл
		
		Если ЗначениеЗаполнено(Строка.Адресат) Тогда
			
			Если НРег(Строка.Адресат.Адрес) = НРег(Адрес)
				И Строка.Контакт = Контакт Тогда
			...
			Если Строка.Адресат.Адрес = Адрес
				И Строка.Контакт <> Контакт Тогда
				
				НайденнаяСтрока = Строка;
				Возврат "ТотЖеАдресДругойКонтакт";
...
&НаСервере
Функция ЗанестиВПолучателиВыбранныеСтроки(Результат, ИдТекущаяСтрока, 
	ПараметрыЗаполненияТекущейСтроки, ТипАдреса)
	...
	Для Индекс = 0 По Результат.Количество() - 1 Цикл
		...
		КодВозврата = ВПолучателяхУжеЕстьКонтакт(Строка.Контакт, Строка.Адрес, НайденнаяСтрока);
		...
		Если КодВозврата = "ТотЖеАдресДругойКонтакт" Тогда // сообщим об ошибке, но строку все же добавим
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Адрес ""%1"" встречается в адресатах письма дважды (""%2"" и ""%3"")'"),
					Строка.Адрес, Строка.Контакт, НайденнаяСтрока.Контакт);
		...
		СведенияОбАдресате = ВстроеннаяПочтаСервер.ПолучитьПредставлениеИКонтактАдресата(Адресат);
		НоваяСтрока.Контакт = СведенияОбАдресате.Контакт;
		НоваяСтрока.Представление = СведенияОбАдресате.Представление;
		НоваяСтрока.Адрес = Строка.Адрес;
		НоваяСтрока.Внешний = СведенияОбАдресате.Внешний;
		НоваяСтрока.ВидМаршрутизации = СведенияОбАдресате.ВидМаршрутизации;
		НоваяСтрока.РедактированиеЗавершено = Истина;
		Если ЭтоТекущаяСтрока Тогда
			ПараметрыЗаполненияТекущейСтроки = Новый Структура("Адресат, Контакт, Представление, Адрес, Внешний, ВидМаршрутизации",
				НоваяСтрока.Адресат, НоваяСтрока.Контакт, НоваяСтрока.Представление, НоваяСтрока.Адрес, НоваяСтрока.Внешний, НоваяСтрока.ВидМаршрутизации);
		КонецЕсли;	
...
&НаКлиенте
Процедура ЗавершениеСозданияКонтактногоЛица(Корреспондент, Параметры) Экспорт
...
Процедура ВставитьТекущуюДатуИВремя(Команда)
	...
	ИначеЕсли СпособРедактированияТекстаПисьма = ПредопределенноеЗначение("Перечисление.СпособРедактированияТекстаПисьма.ПолеВвода") Тогда
	...
	ИначеЕсли СпособРедактированияТекстаПисьма = ПредопределенноеЗначение("Перечисление.СпособРедактированияТекстаПисьма.ПолеТекстовогоДокумента") Тогда
			
		Элементы.ТекстПисьмаКакТекстовыйДокумент.ВыделенныйТекст = 
			Элементы.ТекстПисьмаКакТекстовыйДокумент.ВыделенныйТекст + СтрокаВставки;
			
		Если ЭтаФорма.ТекущийЭлемент <> Элементы.ТекстПисьмаКакТекстовыйДокумент Тогда
			ЭтаФорма.ТекущийЭлемент = Элементы.ТекстПисьмаКакТекстовыйДокумент;
		КонецЕсли;
...
Процедура СохранитьВложениеКак(Команда)
	...
	Если Элементы.Вложения.ВыделенныеСтроки.Количество() > 1 Тогда
		...
		Если СписокФайловДляВыгрузки.Количество() > 0 Тогда
			...
			ОткрытьФорму(
				"Справочник.Файлы.Форма.ВыгрузкаФайловНаДиск", 
				ПараметрыФормы,
				ЭтаФорма,,,,,
				РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
		КонецЕсли;
...
Процедура ДобавитьПолучателя(ТипАдреса, Адресат, СоответствиеДанныхАдресатов = Неопределено)
	...
	Если ЗначениеЗаполнено(Адресат) Тогда
		...
		Иначе	
			НоваяСтрока.Адрес = ОбщегоНазначения.ПолучитьЗначениеРеквизита(Адресат, "Адрес");
		КонецЕсли;
	КонецЕсли;
	НоваяСтрока.РедактированиеЗавершено = Истина;
	Если СоответствиеДанныхАдресатов <> Неопределено Тогда 
...
Процедура ЗаполнитьПолучателейКомуКопия()
	...
	ТаблицаЗначений = ВстроеннаяПочтаСервер.ПолучитьТаблицуПолучателейКомуКопияСкрытаяУИсходящегоПисьма(Объект.Ссылка);
...
Процедура ПолучателиПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если Получатели.Количество() >= 2 Тогда
		...
		Если Получатели.Количество() = 2 И ДобавлятьАвтораИсходногоПисьмаПриПересылке = Истина Тогда
			ПредыдущаяСтрокаТипАдреса = НСтр("ru='Кому:'");
		КонецЕсли;	
...
Процедура ОтобразитьСвязиПисьма()
	...
	Если ЗначениеЗаполнено(ПисьмоОтправленоВОтветНа) Тогда
		...
		Если ВстроеннаяПочтаКлиентСервер.ЭтоВходящееПисьмо(ПисьмоОтправленоВОтветНа) Тогда
			...
			ОтправленоВОтветНаСтрока =
				ОбщегоНазначения.ПолучитьЗначениеРеквизита(ПисьмоОтправленоВОтветНа, "Тема");
		...
		Элементы.ОтправленоВОтветНаСтрока.Гиперссылка = Истина;
	...
	ИначеЕсли ЗначениеЗаполнено(Объект.ПисьмоОснование) 
		...
		Если ВстроеннаяПочтаКлиентСервер.ЭтоВходящееПисьмо(Объект.ПисьмоОснование) Тогда
			...
			ОтправленоВОтветНаСтрока =
				ОбщегоНазначения.ПолучитьЗначениеРеквизита(Объект.ПисьмоОснование, "Тема");
	...
	Иначе
		...
		Элементы.ОтправленоВОтветНаСтрока.Гиперссылка = Ложь;
		Элементы.ОтправленоВОтветНаСтрока.Доступность = Ложь;
	...
	ИначеЕсли КоличествоОтветов = 1 Тогда
		...
		Если ВстроеннаяПочтаКлиентСервер.ЭтоВходящееПисьмо(Ответ) Тогда
			ПолученОтветСтрока = ОбщегоНазначения.ПолучитьЗначениеРеквизита(
				Ответ,
				"Тема");
	...
	Если ПересланоПисьмом.Количество() > 0 Тогда
		...
		Элементы.ПересланоПисьмомСтрока.Доступность = Истина;
	КонецЕсли;
	
	// Обработка связи ПересылкаПисьма
	ПересылкаПисьмаСтрока = НСтр("ru = 'Нет'");
	Элементы.ПересылкаПисьмаСтрока.Гиперссылка = Ложь;
	Элементы.ПересылкаПисьмаСтрока.Доступность = Ложь;
	Если ПересылкаПисьма <> Неопределено Тогда
		Реквизиты = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(
			ПересылкаПисьма, "Тема");
		ПересылкаПисьмаСтрока = Реквизиты.Тема;
		Элементы.ПересылкаПисьмаСтрока.Гиперссылка = Истина;
		Элементы.ПересылкаПисьмаСтрока.Доступность = Истина;
	КонецЕсли;
...
Процедура УстановитьТекущийЭлементТекст1()
	...
	Если Объект.ТипТекста = ПредопределенноеЗначение("Перечисление.ТипыТекстовПочтовыхСообщений.ПростойТекст") Тогда
		...
		Если СпособРедактированияТекстаПисьма = СпособРедактированияТекстаПисьмаПолеВвода Тогда
			ТекущийЭлемент = Элементы.ТекстПисьма;
		Иначе
			ТекущийЭлемент = Элементы.ТекстПисьмаКакТекстовыйДокумент;
		КонецЕсли;	
...
Процедура ВывестиВидМаршрутизации(Форма)
	...
	Если Не ПисьмоОтправлено Тогда 
		...
		Если Объект.ВидМаршрутизации = ВидМаршрутизацииВнутренняя Тогда 
			Форма.Элементы.ДекорацияВидМаршрутизации.Картинка = Форма.КартинкаВнутренняяМаршрутизация;
			Форма.Элементы.ДекорацияВидМаршрутизации.Подсказка = НСтр("ru = 'Доставка внутренняя'");
		Иначе
			Форма.Элементы.ДекорацияВидМаршрутизации.Картинка = Форма.КартинкаВнешняяМаршрутизация;
			Форма.Элементы.ДекорацияВидМаршрутизации.Подсказка = НСтр("ru = 'Доставка через почтовый сервер'");
		КонецЕсли;
...
Процедура ВывестиВидАдресатов(Форма)
	...
	Если ВнешниеАдресаты.Количество() > 0 Тогда 
		Форма.Элементы.ДекорацияВнешний.Картинка = Форма.КартинкаВнешнийАдресат;
		Форма.Элементы.ДекорацияВнешний.Подсказка = НСтр("ru = 'Это внешнее письмо'");
	Иначе
		Форма.Элементы.ДекорацияВнешний.Картинка = Форма.КартинкаВнутреннийАдресат;
		Форма.Элементы.ДекорацияВнешний.Подсказка = НСтр("ru = 'Это внутреннее письмо'");
	КонецЕсли;
...
&НаКлиенте
Процедура РедактироватьЦитирования(Команда)
	...
	ПараметрыФормы = Новый Структура("ТекстПисьма", ТекстПисьма);
	ОписаниеОповещения = Новый ОписаниеОповещения("ЗавершениеРедактированияЦитирования", ЭтотОбъект);
	ОткрытьФорму(
		"Документ.ИсходящееПисьмо.Форма.ФормаРедактированияЦитирования", 
		ПараметрыФормы,
		ЭтаФорма,,,,
		ОписаниеОповещения,
		РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
...
Процедура ЗавершениеВставкиШаблонаТекста(ШаблонТекста, Параметры) Экспорт
	...
	ИначеЕсли СпособРедактированияТекстаПисьма = ПредопределенноеЗначение("Перечисление.СпособРедактированияТекстаПисьма.ПолеТекстовогоДокумента") Тогда
		...
		Если ЭтаФорма.ТекущийЭлемент <> Элементы.ТекстПисьмаКакТекстовыйДокумент Тогда
			Элементы.ТекстПисьмаКакТекстовыйДокумент.ВыделенныйТекст = Элементы.ТекстПисьмаКакТекстовыйДокумент.ВыделенныйТекст + ШаблонТекста;
			ЭтаФорма.ТекущийЭлемент = Элементы.ТекстПисьмаКакТекстовыйДокумент;
		Иначе
			Элементы.ТекстПисьмаКакТекстовыйДокумент.ВыделенныйТекст = ШаблонТекста;
		КонецЕсли;
...
&НаКлиенте
Процедура НеПереноситьСтроки(Команда)
	
	Если Объект.ТипТекста = ПредопределенноеЗначение("Перечисление.ТипыТекстовПочтовыхСообщений.HTML") Тогда
		
		ТекстВопроса = НСтр("ru = 'Преобразование письма в текст удалит форматирование, вставленные изображения, таблицы, гиперссылки и другие элементы оформления.
			|Продолжить?'");     			
		ОписаниеОповещения = Новый ОписаниеОповещения("ЗавершениеНеПереноситьСтроки", ЭтотОбъект);	
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ОКОтмена, 0);
	...
	ПродолжитьНеПереноситьСтроки();
	
КонецПроцедуры

&НаКлиенте
Процедура ЗавершениеНеПереноситьСтроки(Ответ, Параметры) Экспорт
...
&НаКлиенте
Процедура ПродолжитьНеПереноситьСтроки()
	
	СпособРедактированияТекстаПисьма = ПредопределенноеЗначение("Перечисление.СпособРедактированияТекстаПисьма.ПолеТекстовогоДокумента");
...
Процедура ЗавершениеЦветТекста(Цвет, Параметры) Экспорт
	...
	Если Цвет <> Неопределено Тогда
		...
		СтрокаЦвета = Неопределено;
		Если ПолучитьСтрокуЦвета(Цвет, СтрокаЦвета) Тогда
			HTMLДокумент = Элементы.ТекстПисьмаHTML.Документ; 
			HTMLДокумент.execCommand("foreColor", Ложь, СтрокаЦвета );
			Модифицированность = Истина;
		КонецЕсли;	
...
Процедура ЗавершениеЦветФона(Цвет, Параметры) Экспорт
	...
	Если Цвет <> Неопределено Тогда
		...
		Если ПолучитьСтрокуЦвета(Цвет, СтрокаЦвета) Тогда
			HTMLДокумент = Элементы.ТекстПисьмаHTML.Документ; 
			HTMLДокумент.execCommand("backColor", Ложь, СтрокаЦвета );
			Модифицированность = Истина;
		КонецЕсли;	
...
Процедура ТекстПисьмаHTMLДокументСформирован(Элемент)
	...
	Если Объект.ТипТекста = ПредопределенноеЗначение("Перечисление.ТипыТекстовПочтовыхСообщений.HTML")
		И ПустаяСтрока(ТекстПисьмаПриОткрытии) Тогда
...
Процедура ЗавершениеДобавленияВМоиКонтакты(ГруппаИзбранного, Параметры) Экспорт
	...
	Если ГруппаИзбранного <> Неопределено Тогда
		...
		Если ЧислоДобавленных <> 0 Тогда
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Адресаты (%1 шт) добавлены в группу моих контактов ""%2""'"), ЧислоДобавленных, ГруппаИзбранного);
...
Процедура ПроектПриИзменении(Элемент)
	...
	Иначе
		...
		Если Объект.Проект = НачальноеЗначениеПроект Тогда
			Возврат;
		КонецЕсли;
		
		ПараметрыЗаписи = Новый Структура("ЗаменитьПроектПереписки", Истина);
		Если Записать(ПараметрыЗаписи) Тогда
		...
		Иначе
			...
			Если СписокОшибокПриЗаписи.Количество() > 0 Тогда
				...
				ПараметрыОткрытия = Новый Структура;
				ПараметрыОткрытия.Вставить("Заголовок", НСтр("ru = 'Проект не установлен'"));
				
				ПараметрыОткрытия.Вставить("ПояснениеОшибки", 
					НСтр("ru ='Не удалось установить проект, так как некоторые письма в переписке заблокированы'"));
...
#Если Не ВебКлиент Тогда
	...
	Иначе
		...
		Если Объект.ТипТекста = ПредопределенноеЗначение("Перечисление.ТипыТекстовПочтовыхСообщений.ПростойТекст") Тогда
			...
			Если СпособРедактированияТекстаПисьма = 
				ПредопределенноеЗначение("Перечисление.СпособРедактированияТекстаПисьма.ПолеВвода") Тогда
