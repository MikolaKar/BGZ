...
Процедура ПриОткрытии(Отказ)
	...
	Если Прав(ПапкаДляЭкспортаПолная, 1) <> ОбщегоНазначенияКлиентСервер.РазделительПути() Тогда
		ПапкаДляЭкспортаПолная = ПапкаДляЭкспортаПолная + ОбщегоНазначенияКлиентСервер.РазделительПути();
	КонецЕсли;	
