...
Процедура ЗаписатьНаКлиенте(ЗакрытьПослеЗаписи)
	...
	РаботаВБезопасномРежимеКлиент.ПрименитьЗапросыНаИспользованиеВнешнихРесурсов(Запросы, ЭтаФорма, Новый ОписаниеОповещения("ПродолжитьЗаписьНаКлиенте", ЭтотОбъект, ЗакрытьПослеЗаписи));
КонецПроцедуры
...
Процедура УстановитьВидимостьДоступность(Регистрация = Ложь)
	...
	Если КоличествоРазрешений = 0 Тогда
		Элементы.СтраницаРазрешенияВерсия_2_1_3.Видимость = Ложь;
		Элементы.СтраницаРазрешенияВерсия_2_1_3.Заголовок = НСтр("ru = 'Разрешения'");
		Элементы.СтраницаРазрешенияВерсия_2_2_2.Видимость = Ложь;
		Элементы.СтраницаРазрешенияВерсия_2_2_2.Заголовок = НСтр("ru = 'Разрешения'");
	Иначе
		КоличествоВидимыхЗакладок = КоличествоВидимыхЗакладок + 1;
		...
		Если Объект.РежимСовместимостиРазрешений = Перечисления.РежимыСовместимостиРазрешенийДополнительныхОтчетовИОбработок.Версия_2_1_3 Тогда
			Элементы.СтраницаРазрешенияВерсия_2_1_3.Видимость = Истина;
			Элементы.СтраницаРазрешенияВерсия_2_1_3.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		ИначеЕсли Объект.РежимСовместимостиРазрешений = Перечисления.РежимыСовместимостиРазрешенийДополнительныхОтчетовИОбработок.Версия_2_2_2 Тогда
			Элементы.СтраницаРазрешенияВерсия_2_1_3.Видимость = Ложь;
			Элементы.СтраницаРазрешенияВерсия_2_1_3.Заголовок = НСтр("ru = 'Разрешения'");
			Элементы.СтраницаРазрешенияВерсия_2_2_2.Видимость = Истина;
			Элементы.СтраницаРазрешенияВерсия_2_2_2.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Разрешения (%1)'"),
				Формат(КоличествоРазрешений, "ЧГ="));
		КонецЕсли;
		СформироватьСписокРазрешений();
...
Функция ПолучитьТаблицыРазрешений()
	...
	Результат = РаботаВБезопасномРежимеСлужебный.ТаблицыРазрешений();
