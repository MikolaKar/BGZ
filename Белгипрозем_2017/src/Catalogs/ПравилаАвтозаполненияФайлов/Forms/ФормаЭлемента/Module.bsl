...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТипыВладельца = Метаданные.Справочники.ПравилаАвтозаполненияФайлов.Реквизиты.ВладелецФайла.Тип.Типы();
	Для Каждого Тип Из ТипыВладельца Цикл
		Если Тип = Тип("СправочникСсылка.ВидыВнутреннихДокументов") Тогда
			Представление = НСтр("ru = 'Внутренний документ'");
		ИначеЕсли Тип = Тип("СправочникСсылка.ВидыИсходящихДокументов") Тогда
			Представление = НСтр("ru = 'Исходящий документ'");	
		КонецЕсли;
		Элементы.ТипВладельцаФайла.СписокВыбора.Добавить(Строка(Тип), Представление);
	КонецЦикла;
	...
	Элементы.ВладелецФайла.ТолькоПросмотр = ПустаяСтрока(ТипВладельцаФайла);
	Элементы.ВладелецФайла.ОтметкаНезаполненного = НЕ Элементы.ВладелецФайла.ТолькоПросмотр И НЕ ЗначениеЗаполнено(Объект.ВладелецФайла);
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ДанныеФайла = РаботаСФайламиВызовСервера.ПолучитьДанныеФайла(ТекущийОбъект.ШаблонФайла.Ссылка);
...
Процедура ДанныеДляАвтозаполненияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если НРег(ЭтаФорма.Элементы.ТипШаблона.ТекстРедактирования) = "txt" ИЛИ
		НРег(ЭтаФорма.Элементы.ТипШаблона.ТекстРедактирования) = "html" ИЛИ
		НРег(ЭтаФорма.Элементы.ТипШаблона.ТекстРедактирования) = "htm" Тогда
	...
	ПараметрыОткрытияФормы.Вставить("ТипДокумента", Элементы.ТипВладельцаФайла.ТекстРедактирования);
...
Процедура ПроверитьПравилоПродолжениеПослеВыбораДокумента(Результат, Параметры) Экспорт
	...
	Если РезультатЗаполненияФайла.Свойство("ДвоичныеДанныеЗаполненногоФайла")
		...
		ОткрытьФайлПриложением(ПутьКФайлу);	
...
&НаКлиенте
Процедура ОткрытьФайлПриложением(ПолноеИмяФайла)
...
Процедура ШаблонФайлаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.Файлы") Тогда
		...
		РасширениеФайла = НРег(ОбщегоНазначения.ПолучитьЗначениеРеквизита(ВыбранноеЗначение, "ТекущаяВерсияРасширение"));
	Иначе
		Возврат;
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	#Если НЕ ВебКлиент Тогда
		Если НЕ ВыполнятьЗаполнениеФайловНаСервере Тогда
			РасширениеШаблона = НРег(ЭтаФорма.Элементы.ТипШаблона.ТекстРедактирования);
...
Процедура ПриОткрытии(Отказ)
	#Если ВебКлиент Тогда
		ЭтаФорма.Элементы.ФормаПроверитьПравила.Видимость = Ложь;
	#КонецЕсли
	Если НРег(ЭтаФорма.Элементы.ТипШаблона.ТекстРедактирования) = "txt" ИЛИ
		НРег(ЭтаФорма.Элементы.ТипШаблона.ТекстРедактирования) = "html" ИЛИ
		НРег(ЭтаФорма.Элементы.ТипШаблона.ТекстРедактирования) = "htm" Тогда
		
		ЭтаФорма.ПодчиненныеЭлементы.ДанныеДляАвтозаполнения.ПодчиненныеЭлементы.ДанныеДляАвтозаполненияТермДляЗамены.Видимость = Ложь;
	Иначе
		ЭтаФорма.ПодчиненныеЭлементы.ДанныеДляАвтозаполнения.ПодчиненныеЭлементы.ДанныеДляАвтозаполненияТермДляЗамены.Видимость = Истина;
	КонецЕсли;	
КонецПроцедуры
...
Процедура ДанныеДляАвтозаполненияПередУдалением(Элемент, Отказ)
	...
	ТекстВопроса = НСтр("ru = 'Удалить выбранные настройки автозаполения (" + Элемент.ВыделенныеСтроки.Количество() + ")?'");
