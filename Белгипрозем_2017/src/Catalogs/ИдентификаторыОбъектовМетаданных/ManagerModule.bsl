				...
				Если Не Объект.ПометкаУдаления Тогда
					...
					Для каждого Строка Из Строки Цикл
						...
						Если Строка.Ссылка <> Ссылка
						   И ТаблицаЗагружаемых.Найти(Ссылка, "Ссылка") = Неопределено Тогда
...
Функция ТаблицаПереименованияДляТекущейВерсии() Экспорт
	...
	КоллекцииБезКлючаОбъектаМетаданных = СвойстваКоллекцийОбъектовМетаданных(Истина).НайтиСтроки(Отбор);
...
// Только для внутреннего использования.
Функция СвойстваКоллекцийОбъектовМетаданных(БезИдентификаторовКоллекций = Ложь)
	...
	Для каждого Строка Из СвойстваКоллекцийОбъектовМетаданных Цикл
		...
		Если БезИдентификаторовКоллекций Тогда
			Продолжить;
		КонецЕсли;
	...
	Если Не БезИдентификаторовКоллекций Тогда
...
Процедура ОбновитьКэшИдентификаторов(Выгрузка, ЕстьИзменения, ТолькоПроверка);
	...
	Для каждого Свойства Из НайденныеСтроки Цикл
		...
		Если НЕ Свойства.БезДанных
		   И НЕ Метаданные.Константы.Содержит(Свойства.ОбъектМетаданных) Тогда
