...
Функция ЗаписатьСервер()
	...
	Попытка
		...
		Для каждого Эл Из ТаблицаПрав Цикл
			...
			Строка.ЧтениеПапокИФайлов = ПреобразоватьПравоВБулево(Эл.ПравоЧтения);
			Строка.ДобавлениеПапокИФайлов = ПреобразоватьПравоВБулево(Эл.ПравоДобавления); 
			Строка.ИзменениеПапокИФайлов = ПреобразоватьПравоВБулево(Эл.ПравоИзменения);
			Строка.ИзменениеПапок = ПреобразоватьПравоВБулево(Эл.ПравоИзмененияПапок);
			Строка.ПометкаУдаленияПапокИФайлов = ПреобразоватьПравоВБулево(Эл.ПравоУдаленияДанных);
...
Процедура ВыбратьПользователей()
	...
	ПараметрыФормы.Вставить("ТекущаяСтрока", ?(Элементы.ТаблицаПрав.ТекущиеДанные = Неопределено, Неопределено, Элементы.ТаблицаПрав.ТекущиеДанные.Пользователь));
...
Процедура ДобавитьПравоВТаблицуПрав(ОписаниеПрава)
	...
	// Право ЧтениеПапокИФайлов
	Если ОписаниеПрава.ЧтениеПапокИФайлов = Ложь Тогда
		// Запрещено
		СтрокаПрав.ПравоЧтения = 2;
	ИначеЕсли ОписаниеПрава.ЧтениеПапокИФайлов = Истина Тогда
		// Разрешено
	...
	// Право ДобавлениеПапокИФайлов
	Если ОписаниеПрава.ДобавлениеПапокИФайлов = Ложь Тогда
		// Запрещено
		СтрокаПрав.ПравоДобавления = 2;
	ИначеЕсли ОписаниеПрава.ДобавлениеПапокИФайлов = Истина Тогда
		// Разрешено
	...
	Если ОписаниеПрава.ИзменениеПапокИФайлов = Ложь Тогда
		// Запрещено
		СтрокаПрав.ПравоИзменения = 2;
	ИначеЕсли ОписаниеПрава.ИзменениеПапокИФайлов = Истина Тогда
		// Разрешено
	...
	// Право ПометкаУдаленияПапокИФайлов
	Если ОписаниеПрава.ПометкаУдаленияПапокИФайлов = Ложь Тогда
		// Запрещено
		СтрокаПрав.ПравоУдаленияДанных = 2;
	ИначеЕсли ОписаниеПрава.ПометкаУдаленияПапокИФайлов = Истина Тогда
		// Разрешено
