...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ФорматСодержитНомерСвязанного(Объект.ФорматНомера) 
	 Или Объект.НезависимаяНумерацияПоСвязанномуДокументу Тогда 
		Элементы.ТипСвязи.Доступность = Истина;
		Элементы.ТипСвязи.АвтоОтметкаНезаполненного = Истина;
	Иначе	
		Элементы.ТипСвязи.Доступность = Ложь;
		Элементы.ТипСвязи.АвтоОтметкаНезаполненного = Ложь;
	КонецЕсли;
	
	СформироватьПримерНомера(Объект.ФорматНомера, Пример);
...
Процедура НастройкиНумерацииВидДокументаНачалоВыбораПродолжение(ВыбранныйЭлемент, Параметры) Экспорт
	...
	Если ВыбранныйЭлемент.Значение = "СправочникСсылка.ВидыВходящихДокументов" Тогда 
		ТекущиеДанные.ТипДокумента = ТипыОбъектовВходящиеДокументы;
	ИначеЕсли ВыбранныйЭлемент.Значение = "СправочникСсылка.ВидыИсходящихДокументов" Тогда 
		ТекущиеДанные.ТипДокумента = ТипыОбъектовИсходящиеДокументы;
	ИначеЕсли ВыбранныйЭлемент.Значение = "СправочникСсылка.ВидыВнутреннихДокументов" Тогда 
		ТекущиеДанные.ТипДокумента = ТипыОбъектовВнутренниеДокументы;
	КонецЕсли;	
...
Процедура НастройкиНумерацииТипДокументаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение = ТипыОбъектовВходящиеДокументы  Тогда 
		ИмяТипа = "СправочникСсылка.ВидыВходящихДокументов";
	ИначеЕсли ВыбранноеЗначение = ТипыОбъектовИсходящиеДокументы Тогда 
		ИмяТипа = "СправочникСсылка.ВидыИсходящихДокументов";
	ИначеЕсли ВыбранноеЗначение = ТипыОбъектовВнутренниеДокументы  Тогда 
...
&НаКлиенте
Процедура УстановитьДоступностьТипаСвязи()
	
	НезависимаяНумерацияПоСвязанномуДокументу = НезависимаяНумерация.НайтиПоЗначению(
		"НезависимаяНумерацияПоСвязанномуДокументу").Пометка;
	
	Если ФорматСодержитНомерСвязанного(Объект.ФорматНомера) 
	 Или НезависимаяНумерацияПоСвязанномуДокументу Тогда 
		Элементы.ТипСвязи.Доступность = Истина;
		Элементы.ТипСвязи.АвтоОтметкаНезаполненного = Истина;
	Иначе
...
Процедура СформироватьПримерНомера(ФорматНомера, Пример)
	
	ОписаниеОшибки = "";
	Если Не Нумерация.СформироватьПримерНомера(ФорматНомера, Пример, ОписаниеОшибки) Тогда 
...
Процедура УстановитьУчетПоНоменклатуреДел(ВидыДокументов)
	...
	Попытка
		Для Каждого ВидДокумента Из ВидыДокументов Цикл
			...
			ЗаблокироватьДанныеДляРедактирования(ВидДокументаОбъект.Ссылка);
...
Процедура ЗаполнитьНезависимуюНумерацию()
	...
	НезависимаяНумерация.Добавить("НезависимаяНумерацияПоСвязанномуДокументу", 	НСтр("ru = 'Связанному документу'"));
	...
	НезависимаяНумерация.Добавить("НезависимаяНумерацияПоВопросуДеятельности", 	НСтр("ru = 'Вопросу деятельности'"));
...
Процедура ЗаполнитьСлужебныеПоля()
	...
	СлужебныеПоля.Добавить(НСтр("ru = '[ИндексКорресп]'"), 	НСтр("ru = 'Индекс корреспондента'"));
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСвязиДокументов") Тогда 
