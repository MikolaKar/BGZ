...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПраваПоОбъекту = ДокументооборотПраваДоступа.ПолучитьПраваПоОбъекту(Проект);	
	РеквизитыПроекта.Вставить("ДанныеДоступныДляИзменения", ПраваПоОбъекту.Изменение);
...
Процедура ЗаполнитьДеревоЗадач()
	...
	Индекс = -1;
	РаботаСПроектамиКлиентСервер.НайтиЗадачуВДеревеПоСсылке(ДеревоЗадач.ПолучитьЭлементы(), ТекущаяПроектнаяЗадача, Индекс);
	Если Индекс > -1 Тогда
...
Процедура ЗаполнитьСтрокуДереваИзПроектнойЗадачи(ЭлементДерева, ПроектнаяЗадача)
	...
	Если ТипЗнч(ПроектнаяЗадача) = Тип("СправочникСсылка.ПроектныеЗадачи") Тогда 
		Реквизиты = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(ПроектнаяЗадача, СписокРеквизитов1);
	...
	Для Каждого СтрокаТаблЧасти Из Реквизиты.Предшественники.Выгрузить() Цикл
		Если ЗначениеЗаполнено(СтрокаТаблЧасти.Предшественник) И СтрокаТаблЧасти.Предшественник.ПометкаУдаления Тогда 
			Продолжить;
		КонецЕсли;
...
Процедура ИсполнителиСтрОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТекущиеДанные.Исполнители.Количество() = 1 И Не ЗначениеЗаполнено(ТекущиеДанные.ТекущийПланТрудозатраты) Тогда 
		...
		СтрокаИсполнители = ТекущиеДанные.Исполнители.Получить(0);
