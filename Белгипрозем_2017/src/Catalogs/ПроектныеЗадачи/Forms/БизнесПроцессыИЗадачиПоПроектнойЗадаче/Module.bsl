...
Процедура ЗаполнитьДеревоЗадач()
	...
	ДобавитьЗадачиПоПроектнойЗадаче(Дерево, Параметры.ПроектнаяЗадача);
	...
	ДобавитьВспомогательныеЗадачиПоПроектнойЗадаче(Дерево, Параметры.ПроектнаяЗадача);
...
&НаСервере
Процедура ДобавитьЗадачиПоПроектнойЗадаче(Дерево, ПроектнаяЗадача)
	...
	Для каждого МетаданныеБизнесПроцесса Из Метаданные.БизнесПроцессы Цикл
		...
		Запрос.Текст = 
			...
			|ГДЕ
			|   БизнесПроцессы.ПроектнаяЗадача = &ПроектнаяЗадача
...
&НаСервере
Процедура ДобавитьВспомогательныеЗадачиПоПроектнойЗадаче(Дерево, ПроектнаяЗадача)
	...
	Для каждого МетаданныеБизнесПроцесса Из Метаданные.БизнесПроцессы Цикл
		...
		Запрос.Текст = 
			...
			|	%БизнесПроцесс% КАК БизнесПроцессы
			|ГДЕ
			|   БизнесПроцессы.Предметы.Предмет = &ПроектнаяЗадача
			|   И БизнесПроцессы.ГлавнаяЗадача = ЗНАЧЕНИЕ(Задача.ЗадачаИсполнителя.ПустаяСсылка)
			|   И БизнесПроцессы.ПометкаУдаления = Ложь
			|
			|УПОРЯДОЧИТЬ ПО
