...
Функция ПечатьОбложки(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	//МиСофт+
	Макет = УправлениеПечатью.ПолучитьМакет("Справочник.ДелаХраненияДокументов.ОбложкаРБ");
	//МиСофт-
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	//МиСофт+
	|	Дела.МестоХранения КАК МестоХранения,
	//МиСофт-
	...
	Пока Выборка.Следующий() Цикл
		...
		//МиСофт+
		ОбластьОбложка.Параметры.Подразделение = СокрЛП(Выборка.МестоХранения);
		//МиСофт-
...
Функция ПечатьВнутреннейОписи(МассивОбъектов, ОбъектыПечати) Экспорт 
	...
	ТабДок.ОтображатьЗаголовки = Ложь;
	//Мисофт+
	Макет = УправлениеПечатью.ПолучитьМакет("Справочник.ДелаХраненияДокументов.ВнутренняяОписьРБ");
	//Мисофт-
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	Для Каждого Ссылка из МассивОбъектов Цикл
		...
		//Мисофт+
		ОбластьПодвал.Параметры.КоличествоДокументов = ?(Номер = 0, "", Строка(Номер) + " ( " + СокрЛП(Нрег(ЧислоПрописью(Номер, , ",,,с,,,,,0")))+")");
		ОбластьПодвал.Параметры.Дата = ТекущаяДата();
		ОбластьПодвал.Параметры.Расшифровка = Пользователи.ТекущийПользователь().ПредставлениеВДокументах;
		//Мисофт-
...
Функция ПечатьЛистаЗаверителя(МассивОбъектов, ОбъектыПечати) Экспорт	
	...
	Макет = УправлениеПечатью.ПолучитьМакет("Справочник.ДелаХраненияДокументов.ПФ_MXL_ЛистЗаверитель");
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Дела.НоменклатураДел.Индекс КАК НомерДела,
	...
	Пока Выборка.Следующий() Цикл
		...
		//МиСофт+
		ОбластьЛистЗаверитель.Параметры.КоличествоЛистов = ?(Выборка.КоличествоЛистов = 0, "", Строка(Выборка.КоличествоЛистов) + " (" + Нрег(СокрЛП(ЧислоПрописью(Выборка.КоличествоЛистов, , ",,,с,,,,,0")))+")");
		ОбластьЛистЗаверитель.Параметры.Дата = ТекущаяДата();
		ОбластьЛистЗаверитель.Параметры.Расшифровка = Пользователи.ТекущийПользователь().ПредставлениеВДокументах;
		//МиСофт-
...
Функция ПечатьКартыЗаместителя(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	ТабДок.ОтображатьЗаголовки = Ложь;
	//МиСофт+
	Макет = УправлениеПечатью.ПолучитьМакет("Справочник.ДелаХраненияДокументов.КартаЗаместительРБ");
	//МиСофт-	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	Пока Выборка.Следующий() Цикл
		...
		//МиСофт+
		ОбластьКартаЗаместитель.Параметры.Категория	  = Выборка.КатегорияДела;
		ОбластьКартаЗаместитель.Параметры.Расшифровка = ОбщегоНазначения.ТекущийПользователь().ПредставлениеВДокументах;
		ОбластьКартаЗаместитель.Параметры.Должность	  = РаботаСПользователями.ПолучитьДолжность(ОбщегоНазначения.ТекущийПользователь());
		//МиСофт-
