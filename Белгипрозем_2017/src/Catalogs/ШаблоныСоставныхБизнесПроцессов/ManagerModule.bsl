...
//     НастрокиШаблона
//
Функция ПолучитьСтруктуруШаблонаСоставныхБизнесПроцессов() Экспорт
	...
	ПараметрыПроцесса.Вставить("ТипШаблона");
	
	Шаблоны = Новый ТаблицаЗначений;
	Шаблоны.Колонки.Добавить("Шаблон");
	Шаблоны.Колонки.Добавить("Реквизит");
	ПараметрыПроцесса.Вставить("Шаблоны", Шаблоны);
...
//
Функция СоздатьШаблонСоставногоБизнесПроцесса(СтруктураШаблона) Экспорт
	
	НовыйШаблон = СоздатьЭлемент();
	ШаблоныБизнесПроцессов.ЗаполнитьШаблон(НовыйШаблон, СтруктураШаблона);
	
	Для Каждого Шаблон Из СтруктураШаблона.Шаблоны Цикл
		НовыйПодчиненныйШаблон = НовыйШаблон.Шаблоны.Добавить();
		ЗаполнитьЗначенияСвойств(НовыйПодчиненныйШаблон, Шаблон);
	КонецЦикла;
