...
Процедура ЗаполнитьДескрипторДоступа(ОбъектДоступа, ДескрипторДоступа) Экспорт
	...
	Если ЗначениеЗаполнено(ОбъектДоступа.Исполнитель) Тогда 
		Исполнитель = ОбъектДоступа.Исполнитель;
	Иначе
		Исполнитель = ОбъектДоступа.РольИсполнителя;
	КонецЕсли;	
		
	ДескрипторДоступа.ДобавитьПользователя(
		"Исполнитель",
		Исполнитель,
		ОбъектДоступа.ОсновнойОбъектАдресации,
		ОбъектДоступа.ДополнительныйОбъектАдресации);	
	...
	Для каждого Эл Из ДескрипторДоступа.Пользователи Цикл
		...
		Если ТипЗнч(Эл.Пользователь) = Тип("СправочникСсылка.Пользователи") Тогда
			...
			Если Эл.Ключ = "Автор" Или Эл.Ключ = "" Тогда
				...
				Если ПротоколРасчетаПрав <> Неопределено Тогда
					ПротоколРасчетаПрав.Добавить(НСтр("ru = 'Автор визы согласования'"));
				КонецЕсли;
				
			ИначеЕсли Эл.Ключ = "Исполнитель" Тогда
				
				ПраваПользователя = Новый Структура("Чтение, Добавление, Изменение, Удаление, УправлениеПравами",
					НайденноеЗначение.Чтение, Ложь, НайденноеЗначение.Чтение, Ложь, Ложь);
				
				Если ПротоколРасчетаПрав <> Неопределено Тогда
					ПротоколРасчетаПрав.Добавить(НСтр("ru = 'Исполнитель'"));
				КонецЕсли;
		...
		ИначеЕсли ТипЗнч(Эл.Пользователь) = Тип("СправочникСсылка.РолиИсполнителей") Тогда
			...
			Для каждого ИсполнительРоли Из ИсполнителиРоли Цикл
				...
				Если Эл.Ключ = "Исполнитель" Или Эл.Ключ = "" Тогда
...
Функция ПолучитьМассивАвторовВизПоДокументу(Документ) Экспорт
	...
	Результат.Колонки.Добавить("Исполнитель");
	Результат.Колонки.Добавить("ОсновнойОбъектАдресации");
	Результат.Колонки.Добавить("ДополнительныйОбъектАдресации");
	
	Выборка = РаботаСВизамиСогласования.ПолучитьАктивныеВизыДокумента(Документ);
	
	Для Каждого Элемент Из Выборка Цикл
		НоваяСтрока = Результат.Добавить();
		ЗаполнитьЗначенияСвойств(Элемент, НоваяСтрока);
	КонецЦикла;
