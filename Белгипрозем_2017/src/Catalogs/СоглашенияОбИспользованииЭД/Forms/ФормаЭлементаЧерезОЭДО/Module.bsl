...
Процедура СохранитьПриглашения(ТЗ, ДобавленоНовыхЗаписей)
	...
	|ГДЕ
	|	(Приглашения.Идентификатор = """"
	|			ИЛИ НЕ Приглашения.Идентификатор В
	|					(ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Пока НовыеПриглашения.Следующий() Цикл
			...
			Ошибка = Найти(НовыеПриглашения.Состояние, "Ошибка") > 0;
			Если ЗначениеЗаполнено(НовыеПриглашения.ИНН) Тогда
				Контрагент = ЭлектронныеДокументыПереопределяемый.СсылкаНаОбъектПоИННКПП("Корреспонденты", НовыеПриглашения.ИНН,
			...
			Если Контрагент = Неопределено Тогда
				...
				Если НЕ Ошибка Тогда
					Текст = НСтр("ru = 'В базе не найден участник:
					|Наименование: %1
					|ИНН: %2
					|КПП: %3
					|Статус: %4.'");
				...
				Иначе
					...
					Текст = НСтр("ru = '%1 в приглашении для участника:
					|Наименование: %2
					|Адрес ЭП: %3
					|ИНН: %4
					|КПП: %5
					|Статус: %6.'");
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Текст, ОписаниеОшибки, Наименование,
						НовыеПриглашения.Наименование, НовыеПриглашения.ИНН, НовыеПриглашения.КПП, НовыеПриглашения.Состояние);
				КонецЕсли;
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
				Продолжить;
			КонецЕсли;
			...
			ТекущаяЗаписьНабора.Статус = СтатусУчастникаПоСтроке(НовыеПриглашения.Состояние);
			Если Ошибка Тогда
				ТекстОписанияОшибки = НСтр("ru = '%1. Исправьте ошибку и повторите отправку приглашения.'");
				ТекстОписанияОшибки = СтрЗаменить(ТекстОписанияОшибки, "%1", НовыеПриглашения.ОписаниеОшибки);
			Иначе
				ТекстОписанияОшибки = "";
			КонецЕсли;
			ТекущаяЗаписьНабора.ОписаниеОшибки = ТекстОписанияОшибки;
...
Функция СписокСертификатовИсключения()
	...
	Запрос.Текст = 
		...
		|	И (СертификатыЭП.ПометкаУдаления
		|			ИЛИ СертификатыЭП.Отозван)";
...
Процедура ОткрытьФормуВыбораСертификатаЭП()
	...
	ОткрытьФорму("Справочник.СертификатыЭП.ФормаВыбора", ПараметрыФормы, ЭтаФорма);
...
Процедура УстановитьДоступностьЭлементовФормы()
	...
	ДоступнаРольНастройки = ЕстьПравоНастройки();
	
	Элементы.УчастникиОбменаЭД.КоманднаяПанель.Доступность = ДоступнаРольНастройки;
	Элементы.СертификатыПодписейОрганизации.КоманднаяПанель.Доступность = ДоступнаРольНастройки;
КонецПроцедуры

&НаСервере
Функция ЕстьПравоНастройки()
	Результат = ЭлектронныеДокументыПереопределяемый.ЕстьПравоНастройкиПараметровЭД();
	Возврат Результат;
КонецФункции
...
Процедура НадписьИдентификаторУчастникаОбменаЭДНажатие(Элемент)
	...
	Если Объект.СертификатыПодписейОрганизации.Количество() = 0 Тогда
		...
		Обработчик = Новый ОписаниеОповещения(
			"НадписьИдентификаторУчастникаОбменаЭДНажатиеПослеОтветаНаВопрос",
			ЭтотОбъект);
...
&НаКлиенте
Процедура НадписьИдентификаторУчастникаОбменаЭДНажатиеПослеОтветаНаВопрос(Ответ, ПараметрыВыполнения) Экспорт
...
Процедура НадписьЛичныйКабинетУчастникаОбменаЭДНажатие(Элемент)
	...
	Если Модифицированность Тогда
		ТекстВопроса = НСтр("ru = 'Выполнить действие можно только в записанном соглашении.
                             |Записать соглашение?'");
		Обработчик = Новый ОписаниеОповещения("НадписьЛичныйКабинетУчастникаОбменаЭДНажатиеПослеОтветаНаВопрос", ЭтотОбъект);
		ПоказатьВопрос(Обработчик, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
		Возврат;
	КонецЕсли;
	
	НадписьЛичныйКабинетУчастникаОбменаЭДНажатиеЗавершение();
	
КонецПроцедуры

&НаКлиенте
Процедура НадписьЛичныйКабинетУчастникаОбменаЭДНажатиеПослеОтветаНаВопрос(Ответ, ПараметрыВыполнения)
...
&НаКлиенте
Процедура НадписьЛичныйКабинетУчастникаОбменаЭДНажатиеЗавершение()
...
Процедура ЗарегистрироватьСертификат(СертификатЭП)
	...
	ПараметрыВыполнения.Вставить("СертификатЭП", СертификатЭП);
	
	Обработчик = Новый ОписаниеОповещения(
		"ЗарегистрироватьСертификатЗавершение",
		ЭтотОбъект,
		ПараметрыВыполнения);
...
&НаКлиенте
Процедура ЗарегистрироватьСертификатЗавершение(Результат, ПараметрыВыполнения) Экспорт
	
	Если Не Результат.Успешно ИЛИ Результат.Отказ = Истина Тогда
		Возврат;
	КонецЕсли;
	
	ПараметрыСертификата = ПараметрыВыполнения.ПараметрыСертификата;
	СертификатЭП = ПараметрыВыполнения.ПараметрыСертификата;
...
Процедура ОбновитьДанныеПоСтатусамУчастниковССервиса(Команда)
	...
	// Блок проверки на заполненность идентификатора организации.
	Если Не ЗначениеЗаполнено(Объект.ИдентификаторОрганизации) Тогда
		...
		ЭлектронныеДокументыКлиентСервер.ПолучитьТекстСообщения("Поле", "Заполнение", "Идентификатор организации"),
			Объект.Ссылка, "ИдентификаторОрганизации");
		Возврат;
	КонецЕсли;
...
Процедура ОрганизацияОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Объект.СертификатыПодписейОрганизации.Количество() > 0 Тогда
		...
		ТекстВопроса = НСтр("ru = 'При изменении организации будет очищен список сертификатов.
                             |Продолжить?'");
		ПараметрывВыполнения = Новый Структура("ВыбранноеЗначение", ВыбранноеЗначение);
		Обработчик = Новый ОписаниеОповещения("ОрганизацияОбработкаВыбораПослеОтветаНаВопрос", ЭтотОбъект);
		ПоказатьВопрос(Обработчик, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
		Возврат;
	КонецЕсли;
	
	ОрганизацияОбработкаВыбораЗавершение();
	
КонецПроцедуры

&НаКлиенте
Процедура ОрганизацияОбработкаВыбораПослеОтветаНаВопрос(Ответ, ПараметрыВыполнения) Экспорт
	
	Если Ответ = КодВозвратаДиалога.Нет Тогда
		Возврат;
	КонецЕсли;
	
	Объект.Организация = ПараметрыВыполнения.ВыбранноеЗначение;
	ОрганизацияОбработкаВыбораЗавершение();
	
КонецПроцедуры

&НаКлиенте
Процедура ОрганизацияОбработкаВыбораЗавершение() Экспорт
...
Процедура СертификатыПодписейОрганизацииПередНачаломИзменения(Элемент, Отказ)
	...
	Отказ = Истина;
	ОткрытьФорму("Справочник.СертификатыЭП.ФормаОбъекта", Новый Структура("Ключ", Элемент.ТекущиеДанные.Сертификат));
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
		...
		Если ТипВыбЗначения = Тип("СправочникСсылка.Корреспонденты") Тогда
			ДобавитьУчастникаОбменаЭД(Объект.Ссылка, ВыбранноеЗначение);
			Элементы.УчастникиОбменаЭД.Обновить();
		ИначеЕсли ТипВыбЗначения = Тип("СправочникСсылка.СертификатыЭП") Тогда
...
Процедура СертификатыПодписейОрганизацииВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Ключ", Элемент.ТекущиеДанные.Сертификат);
	ОткрытьФорму("Справочник.СертификатыЭП.Форма.ФормаЭлементаБЭД", ПараметрыФормы, ЭтаФорма);
