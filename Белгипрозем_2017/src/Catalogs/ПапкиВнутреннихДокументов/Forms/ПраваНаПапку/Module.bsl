...
Функция ЗаписатьСервер()
	...
	Попытка
		...
		Для каждого Эл Из ТаблицаПрав Цикл
			...
			Строка.ЧтениеПапокИВнутреннихДокументов = ПреобразоватьПравоВБулево(Эл.ПравоЧтения);
			Строка.ДобавлениеПапокИВнутреннихДокументов = ПреобразоватьПравоВБулево(Эл.ПравоДобавления); 
			Строка.ИзменениеПапокИВнутреннихДокументов = ПреобразоватьПравоВБулево(Эл.ПравоИзменения);
			Строка.ИзменениеПапокВнутреннихДокументов = ПреобразоватьПравоВБулево(Эл.ПравоИзмененияПапок);
			Строка.ПометкаУдаленияПапокИВнутреннихДокументов = ПреобразоватьПравоВБулево(Эл.ПравоУдаленияДанных);
...
Процедура ВыбратьПользователей()
	...
	ПараметрыФормы.Вставить("ТекущаяСтрока", 
		?(Элементы.ТаблицаПрав.ТекущиеДанные = Неопределено, 
...
Процедура ДобавитьПравоВТаблицуПрав(ОписаниеПрава)
	...
	// Право ЧтениеПапокИВнутреннихДокументов
	Если ОписаниеПрава.ЧтениеПапокИВнутреннихДокументов = Ложь Тогда
		// Запрещено
		СтрокаПрав.ПравоЧтения = 2;
	ИначеЕсли ОписаниеПрава.ЧтениеПапокИВнутреннихДокументов = Истина Тогда
		// Разрешено
	...
	// Право ДобавлениеПапокИВнутреннихДокументов
	Если ОписаниеПрава.ДобавлениеПапокИВнутреннихДокументов = Ложь Тогда
		// Запрещено
		СтрокаПрав.ПравоДобавления = 2;
	ИначеЕсли ОписаниеПрава.ДобавлениеПапокИВнутреннихДокументов = Истина Тогда
		// Разрешено
	...
	// Право ИзменениеПапокВнутреннихДокументов
	Если ОписаниеПрава.ИзменениеПапокВнутреннихДокументов = Ложь Тогда
		// Запрещено
		СтрокаПрав.ПравоИзмененияПапок = 2;
	ИначеЕсли ОписаниеПрава.ИзменениеПапокВнутреннихДокументов = Истина Тогда
		// Разрешено
	...
	// Право ИзменениеПапокИВнутреннихДокументов
	Если ОписаниеПрава.ИзменениеПапокИВнутреннихДокументов = Ложь Тогда
		// Запрещено
		СтрокаПрав.ПравоИзменения = 2;
	ИначеЕсли ОписаниеПрава.ИзменениеПапокИВнутреннихДокументов = Истина Тогда
		// Разрешено
	...
	// Право ПометкаУдаленияПапокИВнутреннихДокументов
	Если ОписаниеПрава.ПометкаУдаленияПапокИВнутреннихДокументов = Ложь Тогда
		// Запрещено
		СтрокаПрав.ПравоУдаленияДанных = 2;
	ИначеЕсли ОписаниеПрава.ПометкаУдаленияПапокИВнутреннихДокументов = Истина Тогда
		// Разрешено
