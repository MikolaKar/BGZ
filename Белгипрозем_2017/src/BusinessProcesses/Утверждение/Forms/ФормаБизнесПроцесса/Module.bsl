...
Процедура ПриОткрытии(Отказ)
	...
	Если Объект.Ссылка.Пустая() И Не Копирование Тогда 
		Если ШаблоныПоПредметам.Количество() > 1 Тогда 
			РезультатВыбора = ШаблоныБизнесПроцессовКлиент.ВыбратьШаблонБизнесПроцесса("ШаблоныУтверждения", ШаблоныПоПредметам);
...
Процедура ПередЗаписьюПриСтартеЗавершениеВопросаОбОбщемСроке(Результат, Параметры) Экспорт
	...
	Иначе	
		...
		Параметры.Удалить("СтартоватьПриНезарегистрированныхДокументах");
...
Процедура ЗаполнитьПоШаблону(Команда)
	...
	ШаблоныПоПредметам.ЗагрузитьЗначения(МультипредметностьВызовСервера.ПолучитьШаблоныПоПредметам(Предметы, "ШаблоныУтверждения", Ложь));
	
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ЗаполнитьПоШаблонуПродолжение",
		ЭтотОбъект);
	ШаблоныБизнесПроцессовКлиент.ВыбратьШаблонБизнесПроцесса("ШаблоныУтверждения", ШаблоныПоПредметам, ОписаниеОповещения);
	...
	Если ОбрабатываемыеПредметы.Количество() = 1 Тогда
		Если Объект.ПодписыватьЭП
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				ОбщегоНазначенияДокументооборот.ПредметСтрокой(ОбрабатываемыеПредметы[0]));
			...
			Для Каждого Предмет Из ПредметыБезВозможностиПодписи Цикл
				Если Не ПустаяСтрока(ПредметыБезВозможностиПодписиСтрокой) Тогда
					ПредметыБезВозможностиПодписиСтрокой = ПредметыБезВозможностиПодписиСтрокой + ", 
						|";
				КонецЕсли;
				ПредметыБезВозможностиПодписиСтрокой = ПредметыБезВозможностиПодписиСтрокой + ОбщегоНазначенияДокументооборот.ПредметСтрокой(Предмет);
			КонецЦикла;
