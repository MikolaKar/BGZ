...
&НаСервере
Процедура ОзнакомилсяСервер(ТребуетсяУточнение)
	
	УстановитьПривилегированныйРежим(Истина);
	БПОбъект = Объект.БизнесПроцесс.ПолучитьОбъект();
	ЗаблокироватьДанныеДляРедактирования(БПОбъект.Ссылка);
	БПОбъект.ОтправитьНаУточнение = ТребуетсяУточнение;	
	БПОбъект.Записать();
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.БизнесПроцесс.ВидВопроса = Перечисления.ВидыВопросовВыполненияЗадач.ПереносСрока Тогда
		КоличествоПереносов = ПереносСроковВыполненияЗадач.КоличествоПереносовСрокаПоЗадачеИЗаявкеНаПеренос(
			ПредметРассмотрения,
		...
		КонецЕсли;
	Иначе
		Элементы.ДекорацияСтатусРассмотрения.Видимость = Ложь;
	КонецЕсли;
	
	// Установка статуса выполнения
	Если Объект.Выполнена Тогда
		Если Объект.БизнесПроцесс.ОтправитьНаУточнение Тогда
...
&НаКлиенте
Процедура ВыполнитьЗадачу(ТребуетсяКомментарий)
	
	Если ТребуетсяКомментарий И НЕ ЗначениеЗаполнено(Объект.РезультатВыполнения) Тогда
