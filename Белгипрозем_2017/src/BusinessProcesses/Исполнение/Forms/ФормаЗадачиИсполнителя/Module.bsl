...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НайденнаяСтрока = Объект.БизнесПроцесс.Исполнители.Найти(Объект.Ссылка, "ЗадачаИсполнителя");	
	Если НайденнаяСтрока = Неопределено Тогда 
		СтруктураОтбора = Новый Структура("Исполнитель, ОсновнойОбъектАдресации, ДополнительныйОбъектАдресации",
			?(ЗначениеЗаполнено(Объект.Исполнитель), Объект.Исполнитель, Объект.РольИсполнителя),
			Объект.ОсновнойОбъектАдресации,
			Объект.ДополнительныйОбъектАдресации);
		
		НайденныеСтроки = Объект.БизнесПроцесс.Исполнители.НайтиСтроки(СтруктураОтбора);
	...
	Если НайденнаяСтрока <> Неопределено Тогда 
		РаботаСПроектами.ЗаполнитьТрудозатратыПланСтрокой(
			ЭтаФорма,
			НайденнаяСтрока.ТрудозатратыПланИсполнителя,
			Истина);
	КонецЕсли;
	...
	Если НайденнаяСтрока <> Неопределено Тогда 
		...
		Если ЗначениеЗаполнено(ЗадачаОтвИсполнителя.Исполнитель) Тогда 
			...
			Элементы.ОтветственныйИсполнительСтрокой.КнопкаОткрытия = Истина;
		Иначе
			ОтветственныйИсполнительСтрокой = Строка(ЗадачаОтвИсполнителя.РольИсполнителя);
			Если ЗначениеЗаполнено(ЗадачаОтвИсполнителя.ОсновнойОбъектАдресации) Тогда
				ОтветственныйИсполнительСтрокой = ОтветственныйИсполнительСтрокой + ", " + ЗадачаОтвИсполнителя.ОсновнойОбъектАдресации;
			КонецЕсли;	
			Если ЗначениеЗаполнено(ЗадачаОтвИсполнителя.ДополнительныйОбъектАдресации) Тогда
				ОтветственныйИсполнительСтрокой = ОтветственныйИсполнительСтрокой + ", " + ЗадачаОтвИсполнителя.ДополнительныйОбъектАдресации;
			КонецЕсли;	
			Элементы.ОтветственныйИсполнительСтрокой.КнопкаОткрытия = Ложь;
		КонецЕсли;	
...
&НаКлиенте
Процедура ОтветственныйИсполнительСтрокойОткрытие(Элемент, СтандартнаяОбработка)
