...
Процедура ОбработкаВыполнения(ЗадачаСсылка, БизнесПроцессСсылка, ТочкаМаршрутаБизнесПроцесса, ПараметрыВыполнения) Экспорт
	...
	Если ТочкаМаршрутаБизнесПроцесса = БизнесПроцессы.Регистрация.ТочкиМаршрута.Зарегистрировать Тогда
		...
		Если ПараметрыВыполнения.Свойство("РезультатРегистрации") Тогда	
			...
			КонецЕсли;
			ЗаблокироватьДанныеДляРедактирования(РегистрацияОбъект.Ссылка,, ПараметрыВыполнения.УникальныйИдентификаторФормыИзмененияПараметров);
			РегистрацияОбъект.ДополнительныеСвойства.Вставить("ТекущаяЗадача", ЗадачаСсылка);
			РегистрацияОбъект.ДополнительныеСвойства.Вставить(
				"РезультатРегистрации", ПараметрыВыполнения.РезультатРегистрации);
			РегистрацияОбъект.Записать();
			РазблокироватьДанныеДляРедактирования(РегистрацияОбъект.Ссылка, ПараметрыВыполнения.УникальныйИдентификаторФормыИзмененияПараметров);
	...
	ИначеЕсли ТочкаМаршрутаБизнесПроцесса = БизнесПроцессы.Регистрация.ТочкиМаршрута.Ознакомиться Тогда
		...
		Если ПараметрыВыполнения.Свойство("ПовторитьРегистрацию") Тогда
			...
			РегистрацияОбъект.ДополнительныеСвойства.Вставить("ТекущаяЗадача", ЗадачаСсылка);
			РегистрацияОбъект.ДополнительныеСвойства.Вставить(
				"ПовторитьРегистрацию", ПараметрыВыполнения.ПовторитьРегистрацию);
			РегистрацияОбъект.Записать();
			РазблокироватьДанныеДляРедактирования(РегистрацияОбъект.Ссылка, ПараметрыВыполнения.УникальныйИдентификаторФормыИзмененияПараметров);
...
Процедура ДополнитьТекстЗапросСпискаЗадач(ТекстЗапроса) Экспорт
	...
	ТекстЗаменыПоляНомерИтерации = "
		|	ВЫБОР
		|		КОГДА ЗадачаЗадачаИсполнителя.ТочкаМаршрута = ЗНАЧЕНИЕ(БизнесПроцесс.Регистрация.ТочкаМаршрута.Ознакомиться)
		|			ТОГДА РезультатыОзнакомлений.НомерИтерации
		|		ИНАЧЕ РезультатыРегистрации.НомерИтерации
	...
	ТекстЗаменыТаблиц = "
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ БизнесПроцесс.Регистрация.РезультатыОзнакомлений КАК РезультатыОзнакомлений
